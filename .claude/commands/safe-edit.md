...
- The existing diff-time agents (`mobile-twin-mirror`, `metadata-key-keeper`, `migration-coordinator`, `doc-hygiene-checker`, `shared-library-syncer`, `test-gap-checker`) still fire as part of the coder pass. The reviewer is **on top of** them, not instead of them.
