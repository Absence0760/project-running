...
1. **Semantic HTML.** `<button>` vs `<div onClick>`. Every interactive icon-only button needs `aria-label` or visually-hidden text. Walk `apps/web/src/lib/components/` + `apps/web/src/routes/`.
