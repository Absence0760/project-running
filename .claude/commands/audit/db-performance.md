...
8. **Materialized views + their refresh.** The repo currently has **none** — `mv_weekly_mileage` was dropped unread in `20270530_001` (decisions § 690). If one has since been added, confirm it has a reader at all (an unread matview on a pg_cron refresh is pure background spend, which is exactly how that one lived its whole life), then that its cadence vs staleness is sane, its base query is indexed, and it carries the unique index `REFRESH MATERIALIZED VIEW CONCURRENTLY` requires.
