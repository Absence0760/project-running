---
description: Audit every runs.metadata.<key> read/write across the codebase against docs/metadata.md registry
---

Sweep every `runs.metadata.<key>` access in the codebase and confirm each key is registered in `docs/metadata.md`.

## Goal

`runs.metadata` is a `jsonb` bag with no schema codegen. The only thing keeping cross-platform writers and readers in sync is the registry at `docs/metadata.md`. A typo (`activityType` vs `activity_type`) or a key written by one client and read by another can silently produce a "no data" rendering bug. The pre-existing `metadata_registry_test.dart` catches this at CI time for Dart writers — this command extends that sweep to web + Edge Functions.
...
4. **Registry.** Read `docs/metadata.md`. Extract the registered key list.
5. **Compute drift:**
