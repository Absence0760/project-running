...
2. **Policy strength.** For each `CREATE POLICY`, classify: owner-only (`auth.uid() = user_id`), public-read (`is_public = true`), club-scoped (joins `club_members`), service-role-only. Flag any that's broader than its table's documented intent in `docs/api_database.md`.
...
- `docs/api_database.md` — the documented surface (compare against reality)
