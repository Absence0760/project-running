...
GDPR fines are calibrated to global revenue, not local impact. The headline numbers (€20M or 4% of global turnover) get the press; the actual launch risk is reviewer rejection by Apple / Play if no in-app privacy policy + account-deletion + age-gate exist. Find every gap.

## What to check

1. **Privacy policy + Terms of Service.** Do `/privacy` and `/terms` routes exist in `apps/web/src/routes/`? Are they linked from the signup form, the marketing footer, and the iOS / Android in-app surface? Without them, both stores reject.
2. **Lawful basis (Art 6) per data category.** For each personal-data column the user listed in `compliance-auditor.md`, what lawful basis would the user claim? Consent / contract / legitimate interest are the three plausible ones. Health data (HR, run intensity, body fields, gender, dob) probably needs Art 9 explicit consent. Flag every column that has no obvious basis.
3. **Consent capture.** No cookie / SDK consent banner today on the web app — confirm. Document what would need to fire only after consent on an EU IP (Sentry, MapTiler, RevenueCat web SDK, Anthropic streaming endpoint).
4. **DSAR endpoints.**
   - **Export (Art 20)**: `apps/job_worker/internal/dataexport/server.go` (primary) and `apps/backend/supabase/functions/export-data/index.ts` (rollback). Are they wired to a UI surface? Is the output machine-readable JSON or CSV (Art 20 requires "structured, commonly used, machine-readable")?
   - **Erasure (Art 17)**: `apps/backend/supabase/functions/delete-account/index.ts`. Does the UI expose it on every platform (web, Android, iOS, watches that have account state)?
...
5. **Retention.** Are there documented retention periods? Stale `live_run_pings`, abandoned `coach_messages`, half-finished signups in `auth.users`? Without auto-deletion this is a "keep forever" posture, which is hard to defend.
6. **Cross-border transfers (Chapter V).** Supabase EU project? AWS region? Anthropic processing region? OSRM (Fly.io) region? Strava is US-hosted. Every flow from an EU data subject to a non-EU processor needs SCCs or an adequacy decision. List the flows + their required mechanism.
...
12. **DPIA (Art 35).** Live-location tracking + heart-rate biometrics are explicitly named in EDPB guidance as "high-risk processing" requiring a DPIA before launch. Is there one?
