...
1. **Recurrence engine at 13-week scale.** Audit `apps/web/src/lib/social/recurrence.ts` + the event-creation flow. Can the persona create a weekly Saturday 9am event with a count or until-date that lands 13 instances? Are all 13 visible in the club's events list? Does each instance have its own RSVP roster + cancellation flag? RFC 5545 EXDATE pattern for one-off cancellations?
