...
1. **High-volume / long-history scale.** What happens with 1000+ runs in the list? Does pagination work / does the dashboard query slow to a crawl? Check `apps/web/src/lib/data.ts` fetchRuns, the runs page, mobile `LocalRunStore` init, calendar heatmap renderer. Hunt for O(n²) walks over runs, missing indexes, missing lazy-load.
