...
6. **Over-sharing / privacy.** Audit what the anon tracker exposes. Can a stranger with the link see **every runner's** precise live coordinates? Does it leak the runner's home / privacy-zone start point (check `fetchClippedTrackForRun` is applied on the spectator path)? An anxious-family feature must not become a stalking surface.
