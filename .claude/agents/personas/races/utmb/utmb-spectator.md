...
8. **Over-sharing / privacy.** Audit what the anon livetrack exposes worldwide. Can a stranger with the link see **every runner's** precise live coordinates? Does it leak a runner's home / privacy-zone start (check `clipTrackForUser` is applied on the spectator path in `apps/web/src/lib/core/data.ts`)? A family feature must not become a global stalking surface.
