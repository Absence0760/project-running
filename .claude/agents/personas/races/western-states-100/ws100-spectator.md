...
6. **Over-sharing / privacy.** Audit what the anon tracker exposes. Can a stranger with the link see **every runner's** precise live coordinates? Does it leak a runner's home / privacy-zone start (check `clipTrackForUser` is applied on the spectator path / per `data.ts`)? An anxious-family feature must not become a stalking surface.
