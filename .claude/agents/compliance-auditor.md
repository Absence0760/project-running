...
3. **Data out → DSAR + third-party hops**.
   - **Export** (Art 20 portability, CCPA right-to-know): handler is `apps/job_worker/internal/dataexport/server.go` (HTTP at `POST /v1/export`). Edge Function `export-data` is the deprecated rollback path. Output is a Storage object with a signed URL.
