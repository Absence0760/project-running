...
- **Privacy zones (`decisions.md §33`).** Any new track / waypoint render site must route non-owner views through the server-side clipping path — `fetchClippedTrackForRun` for a run's track, `fetchClippedRouteForViewer` (web) / `ApiClient.clipRouteForViewer` (mobile) for a route's waypoints. No client holds a `clipTrackForUser`; the client-side clip was removed by migration `20260619_001`. Anon viewers (`viewerId == null`) are non-owners. Each helper fails closed — the route pair returns `[]`, the run fetcher throws on web and returns `[]` on a malformed mobile payload — so never fall back to the unclipped input on error.
- **Schema codegen (`docs/architecture/schema_codegen.md`).** New migration → both `npm run gen:types` AND `dart run scripts/gen_dart_models.dart` regenerated, both committed. New set-shaped CHECK column → registered in the `PAIRS` array of `apps/web/scripts/check_constraint_unions.mjs`, naming **every** client rail that enumerates it (TS union, const array, Svelte option list, Dart enum / const list) — or an empty `clients` list plus a `note` saying none does. A widened CHECK that updates only `types.ts` and leaves the Dart dropdown short is the exact defect decisions § 791 closed; flag it.
