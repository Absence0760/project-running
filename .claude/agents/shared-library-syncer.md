...
| `apps/web/src/lib/social/nearby.ts` (`NEARBY_BUCKET_BOUNDS_M`, `NEARBY_BUCKET_COUNT`, `nearbyDistanceBucket`, `nearbyBucketUpperBoundM`; the coarse distance-bucket contract for opt-in "runners nearby" — boundaries MUST match the SQL CASE in migration `20270426_001` + the `discoverable_runners_near` RPC) | `apps/mobile_android/lib/nearby.dart` (same, `kNearbyBucketBoundsM` / `kNearbyBucketCount`) | `social/nearby.test.ts` ↔ `test/nearby_test.dart` |
