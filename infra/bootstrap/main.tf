...
# Rationale: AES256 + CloudTrail data events + locked-down IAM
# (OIDC deploy role only) is the right trade-off for a bucket that
# stores Terraform state with no PII / no customer data.
