...
These jobs are not individually required for merge: branch protection requires a single status check — the **`CI gate`** aggregator job in `ci.yml`, which `needs:` all of them (with `ci-gate-docs.yml` reporting the same `CI gate` context for docs-only PRs), so a green `CI gate` is the merge gate rather than each job separately.
