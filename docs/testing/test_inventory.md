...
### `apps/watch_wear/android/app/src/test/kotlin/**/*Test.kt` — 816 Wear OS Kotlin/JUnit tests across 80 files
...
### `scripts/check_watch_ios_source.test.mjs` — 88 tests
...
### Exercise catalogue shadow resolution — `apps/web/src/lib/gym/exercise_catalogue.test.ts` (11) · `packages/core_models/test/exercise_catalogue_test.dart` (9)
...
The wrist's two classified faults and the PostRun discard. Counted in the Wear OS glob row above (816 across 80 files); listed here because the three are what moved it.
