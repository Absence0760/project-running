...
| 4 | **Physical buttons, no touchscreen-only** | Touchscreens fail with sweat, gloves, rain, gel-coated fingers. Buttons work in any condition | 5-button layout (Garmin pattern); touchscreen optional for map panning only |
| 5 | **Offline vector maps with course following** | Cell signal is unreliable in the places ultras happen. The course breadcrumb is the safety feature | 16+ GB onboard flash, MapLibre-style vector tile rendering on the MCU (hard, see firmware.md) |
