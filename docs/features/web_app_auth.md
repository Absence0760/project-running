...
On mobile the only password-minting surface is sign-up (`sign_up_screen.dart`) — there is no reset or change-password screen — and it routes through the Dart twin `auth_gates.dart`. **The two helpers are a TS↔Dart parity pair: keep the rule in lockstep.**
