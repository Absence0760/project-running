...
- `race-results-import/index.ts` + `lib.ts` — **mirror `parkrun-import` closely.** Two modes by `provider`:
  ...
  - `paste` / `general` — accept a results URL or bib+race; for the v1 the durable path is **structured paste** (the user pastes their result row from a non-API timing site) rather than a brittle per-site scraper. Build the scraper hooks (URL-pattern dispatch in `lib.ts`) but ship only the RunSignUp + paste paths; per-site scrapers are scoped follow-ups (the integrations.md fragility warning).
