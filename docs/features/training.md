...
## Own training-load ramp (`self_load.ts`, web-only)
...
`selfLoad(runs, nowMs)` closes that. It **composes** `acwr` / `injuryRiskBand` / `loadTrend` rather than re-deriving any of them, so the band edges cannot drift from the ones a coach sees, and it reuses `recentRunVolume` from `plan_ramp` (which grew `acuteM` in the same traversal, so the two halves of the ratio can never be reduced from different run sets). Surfaced as `LoadRampCard.svelte` beside the other dashboard analytics cards, scoped to the same `filteredRuns`.
...
- **It refuses a thin history.** Below `MIN_ACTIVE_WEEKS` of the chronic window the ratio is arithmetic on noise, so the card self-hides rather than showing a zero that would read as "safe". `shouldSurfaceSelfLoad` is a type predicate, which narrows the band to the four the card has copy for — an unlabelled band is a compile error, not a missing-key render.
