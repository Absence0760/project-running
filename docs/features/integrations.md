...
One mobile-only disclosure has no web counterpart: web imports one activity at a time, so a push failure IS a per-item failure, whereas mobile saves locally then batch-pushes. That push swallowed its throw, so an import whose runs never reached the server still reported a clean success. It now says the runs are on the device and the next sync will retry — a note rather than N failure entries, because nothing was lost.
