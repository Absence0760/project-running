...
The backfill walks Strava's activity pages until it reaches the end of the lookback window, and has **five** exits. Four of them leave activities in the window unfetched: Strava throttling us (429/503), an upstream error, a page body that is not an array, and the 20-page / 1000-activity safety cap. Only the throttle case ever carried a field — `rate_limited` — and neither client declared it, so all four rendered as a finished sync ([decisions § 766](../architecture/decisions.md)).

That is worse than an unreported count. `after=` is derived from `Date.now() - lookbackDays`, not from a stored cursor (`integrations.sync_cursor` is written by nothing and `last_sync_at` is read by nothing but the two tiles), so a re-sync re-walks the same window and the un-imported activities are still there — **until they age past `lookbackDays`**, which is 90 and which neither client offers a way to raise. Nothing is lost by the truncation; it is lost by the report, which tells the runner there is nothing left to fetch.
