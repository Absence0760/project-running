...
Both importers add a start-time + distance near-duplicate guard **across all sources** (`isCrossProviderDuplicate`, mirrored in `apps/backend/supabase/functions/_shared/strava.ts` and `apps/web/src/lib/integrations/garmin_dedupe.ts`): before inserting, they compare the candidate's start instant and total distance against every existing run of the user. A candidate is skipped when it starts within **180 s** of AND is within **5 %** distance of any existing run. Both axes must match — two genuinely distinct back-to-back runs can't start within a few minutes of each other (you can't record two GPS tracks at once), so gating on start proximity *and* distance similarity skips the same effort across providers without suppressing a warm-up-then-race. The Strava backfill (`strava-import`) and the Garmin ZIP importer (`garmin-zip.ts`) each pre-fetch every existing run's `started_at` + `distance_m` once per import for this check.

### Embedded best efforts on import

Both importers also compute per-canonical-distance embedded best efforts (`metadata.fastest_{5k,10k,half_marathon,marathon}_s`) from the imported activity's per-point track/stream, matching the live recorder's `fastestWindowOf` algorithm (shared `computeEmbeddedBests` helpers). Without them, a fast 5k/10k *inside* a long imported run misses every whole-run canonical bracket and never reaches `personal_records` / the dashboard PR card / the public profile. A track with no per-point timestamps (indoor / treadmill / a route file with no times) writes nothing — no fake bests. See [`docs/backend/metadata.md` § Personal-records hints](../backend/metadata.md).
