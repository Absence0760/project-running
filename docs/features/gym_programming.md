...
- **`gym_routines`**: `select` / `insert` / `update` / `delete` all gated on `author_id = auth.uid()` in v1. Two additive SELECT branches were added later (Postgres ORs permissive policies, so author-only stays the base): a club-member read for club-owned templates (`20270109_001`) and an `is_public_template` public read for the public library (`20270224_001`). The exercises + sets inherit each read branch transitively.
