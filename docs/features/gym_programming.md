...
> **Status:** **P1 shipped on web** (2026-06-11, migration `20261231_001`): `gym_routines` / `gym_routine_exercises` / `gym_routine_sets` + `gym_workouts.metadata`, the `gym_routine` parity helper (`routineFromWorkout` + `prefillFromRoutine`, web + Dart twin), the `/gym/routines` library / `[id]` detail / `new` builder, "Save as routine" + "Repeat last" on `/gym/[id]`, DSAR export of all three tables, Playwright + pgtap. **Mobile twin (LocalRoutineStore + screens + ARBs) is NOT yet built** — the parity helper is in place on both sides, the Flutter screens/store are the remaining P1 work. P2–P4 remain specced and gated (see [Phasing](#phasing--rollout) and [The validation gate](#the-validation-gate-stated-honestly)).
...
Flutter (byte-identical twin) mirrors the web surfaces — strictly **downstream of web** (don't start the Flutter mirror until the web screens + the three parity pairs are merged and tested). Purely additive and self-hiding.
