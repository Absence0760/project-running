...
- **`exercise_key`** is `normalise_exercise_name(exercise_name)` stamped **at write time by the server** (a `before insert or update` trigger, migration `20270623000001`) and held by `gym_routine_exercises_key_normalised_chk`. It is the stable identity that binds a planned exercise to logged `gym_sets` (which carry only free-text `exercise_name`). Storing it as a column lets us index it and keeps binding deterministic. **The client no longer decides it.** It used to: both clients stamped it from their own `normaliseExerciseName`, and the SQL that groups on the same identity derived it a third way — measured, no two of the three rails agreed, so a name carrying a pasted tab or NBSP split one lifter's history in two and a Turkish `İ` merged or split it depending on which platform wrote the row (decisions.md § 790). The three are now one definition plus two faithful clients, read against each other by `scripts/check_shared_constants.mjs`. **Freeze policy:** if the normaliser changes, the migration that changes it re-stamps the column in the same file — the trigger keeps new writes honest, the scoped backfill fixes the old ones, and `VALIDATE CONSTRAINT` proves it. (`derived_state.md` does not carry a row for this column and does not need one: it is not a trigger-maintained *cache* of an authoritative query, it is the identity itself.)
