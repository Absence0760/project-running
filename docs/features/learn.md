...
2. **Guide prose** — **per-locale lookup with English fallback, shipped** (2026-06-20). `guides.ts` indexes every `<slug>.<locale>.md` variant from the same `import.meta.glob`. The resolution surface:
   ...
   **Localized today:** `road-running-101` in all six locales (the proof + template). **English (resolver falls back):** the other seven guides — `couch-to-5k`, `choosing-running-shoes`, `weekly-running-routine`, `how-to-pace-your-first-race`, `trail-running-basics`, `what-to-eat-before-a-long-run`, `your-first-race`. Authoring those is the remaining work; the mechanism is complete. See [decisions.md § 178](../architecture/decisions.md).
