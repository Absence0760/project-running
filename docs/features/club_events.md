...
- **`event_orders`** — the ledger. `id` PK, `event_id`, `instance_start`, `buyer_user_id`, `host_user_id`, `stripe_checkout_session_id`, `stripe_payment_intent_id`, `amount_cents`, `currency`, `platform_fee_cents`, `status ∈ {pending, paid, refunded, partially_refunded, refund_failed, failed, canceled}`, `created_at`, `paid_at`, `refunded_at`, `reserved_until`. RLS: buyer reads own; host reads orders for their events; **writes are service-role only** (webhook is the sole writer — the `subscription_tier` lock pattern, [paywall.md § Tiers](paywall.md#tiers)).
