...
- [ ] **A partial refund on a donation is not representable.** `donations` has no `partially_refunded` state and no refunded-amount column, so `fundraiser_totals` overstates by whatever came back. Filed in `followups.md`; decide whether the schema change blocks launch of the fundraising surface.
