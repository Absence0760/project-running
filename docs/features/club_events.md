...
- **A refund that FAILS after we announced it strands the buyer, and the ledger now says so.** `charge.refunded` fires when a refund is *created* — including one created `pending` against a delayed-notification method, with `charge.amount_refunded` incremented optimistically — so the webhook has always acted on an announcement, not a settlement. When the bank rejects it days later the money returns to us and the seat is already gone (and possibly promoted off the waitlist). `charge.refund.updated` / `refund.updated` / `refund.failed` are now dispatched (all three, because which one an endpoint receives is set by the API version pinned *on that endpoint*), and a `failed` / `canceled` refund status CASes `refunded -> refund_failed` (`20270623_001`) and clears `refunded_at`. **The buyer is deliberately NOT re-seated** — the mat may already belong to a promoted waitlister, `enforce_event_capacity` would silently demote them to `waitlisted` rather than raising, and `enforce_paid_order_for_priced_event` refuses the insert anyway. `refund_failed` is the operator's reconciliation queue, not a terminal state: a re-issued FULL refund moves it on to `refunded`. **`data.object` on these events is a Refund, not a Charge** — it carries neither `refunded` nor `amount_refunded`, so `readRefund` refuses any payload whose `object` is not `refund` rather than letting `refundScopeOfCharge` answer `full` for all of them (which is the seat-deleting branch). [decisions § 789](../architecture/decisions.md).
- **A failed PARTIAL refund is logged, not corrected.** Nobody is stranded (the seat was never released) and `event_orders` carries no refunded amount, so "the one partial we announced failed" and "one of two failed" are indistinguishable. Writing either would be a guess; giving the table its own `refunded_cents` is the durable fix and is filed.
...
- **A refund issued from the Stripe *dashboard* has the same hazard and no code to protect it.** The dashboard's refund dialog reverses nothing unless the operator ticks it, and `charge.refunded` will be processed by the webhook either way. **Pre-deploy ops item:** any manual refund on an event order or a donation must reverse the transfer, or the platform absorbs it.
...
- [ ] **`on_behalf_of`: is the host the settlement merchant, or the platform?** Today it is the platform (see § Money movement). Reconcile with `/terms` §6, which tells the buyer the host is merchant of record. Required, not optional, for any host outside the platform's region.
- [ ] **Chargeback liability is the platform's** on a destination charge regardless of that choice. Confirm the host-onboarding copy says so rather than the reverse.
...
- [ ] **Manual dashboard refunds must reverse the transfer.** Runbook item for whoever holds the Stripe dashboard.
...
- [ ] **Subscribe the endpoint to the refund-reversal events.** `charge.refund.updated`, `refund.updated` and `refund.failed` — all three, since which one is delivered depends on the API version pinned on the endpoint. An endpoint listing none of them never hears that a refund was rejected, and the buyer stays stranded with the ledger saying they were refunded. Same class of item as § 785's two `checkout.session.async_payment_*` types: no code here can enforce it.
...
- **`event_orders`** — the ledger. `id` PK, `event_id`, `instance_start`, `buyer_user_id`, `host_user_id`, `stripe_checkout_session_id`, `stripe_payment_intent_id`, `amount_cents`, `currency`, `platform_fee_cents`, `status ∈ {pending, paid, refunded, refund_failed, partially_refunded, failed, canceled}`, `created_at`, `paid_at`, `refunded_at`, `reserved_until`. RLS: buyer reads own; host reads orders for their events; **writes are service-role only** (webhook is the sole writer — the `subscription_tier` lock pattern, [paywall.md § Tiers](paywall.md#tiers)).
...
- `OrderStatus = 'pending' | 'paid' | 'refunded' | 'refund_failed' | 'partially_refunded' | 'failed' | 'canceled'` (`refund_failed` added by `20270623_001`; backs no seat and is not buyer-cancelable)
