...
- [ ] **`on_behalf_of`: is the host the settlement merchant, or the platform?** Today it is the platform (see § Money movement). Reconcile with `/terms` §6, which tells the buyer the host is merchant of record. Required, not optional, for any host outside the platform's region. **Sending it is a copy change as well as a money change**: `payouts.merchantNote` now tells the host the platform is merchant of record, in seven locales, and `apps/web/src/routes/settings/payouts/merchant_note_guard.test.ts` fails the moment a checkout call site sends the parameter, so the flip cannot land while the host-facing sentence still says the opposite.
