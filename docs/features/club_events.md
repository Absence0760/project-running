...
| **P1** | Stripe Connect onboarding + destination-charge checkout + one webhook, **in-person only**, **host payouts**, web checkout only, manual refunds via Stripe dashboard. | E shows non-`run` events get created; owner + CISO + counsel sign-off |
