...
| 5 Teach / follow along | **Pure-logic + runner landing (M5, in progress)** | The `GymWorkoutRunner` (`packages/run_recorder`) + the adherence/progression/expand/session parity pairs + the metadata trios are merged; the follow-along **screen** (web player + mobile runner) is still in progress. Gated on the M3/M4 engagement signal. |
