...
> **Status: PLAN, not built.** This is a sequencing document, not a new spec. It does **not** introduce new architecture — it ties three existing specs ([club_events.md](club_events.md), [gym_programming.md](gym_programming.md), [session_planner.md](session_planner.md)) into one ordered path that gets a real instructor from "I scheduled a class" to "I run my class business in this app without leaving it." Read those three first; this doc only sequences and gates them around two personas. Nothing here ships without the per-milestone gate being met — and the payment go-live milestone carries a **hard compliance gate** (owner + CISO + counsel sign-off under SOC 2 / GovRAMP).
...
| 4 Author content | **MISSING** | No gym-routine engine (no migration, no `expandRoutineSteps`); no `session_plans` table/engine. Only the flat `events.gym_template` `{discipline, duration_min}` hint exists. |
| 5 Teach / follow along | **MISSING** | No `SessionRunner`, no class teaching screen. |
| 6 Cross-modal log | **Partial** | One-tap "Log this as a workout" from the flat template (`event_gym_template.ts`↔`.dart`, `workoutDraftFromTemplate`) — shipped web+mobile, inform-tier. Can't carry a superset, a load, a hold time, or per-side. |
| 7 Attendance | **MISSING** | `event_attendees.status` is `going`/`maybe`/`declined`/`waitlisted` only — no `attended`/`checked_in`/`no_show`. Paid ≠ showed-up is unrepresentable. |
...
**Also confirmed:** `gym_sets` has `reps` / `weight_kg` / `rpe` but **no `duration_s`** — so a 90 s plank or a timed hold has no first-class home (the `session_planner.md` open question #1 is real and blocks clean yoga logging).

**Read:** the business is ~3 steps from working. The *rail* exists; what's missing is (a) turning the money on safely, (b) the content engines, (c) a few operational surfaces.
...
### M2 — gym_sets.duration_s — first-class timed work (Track B, foundation)
...
### M3 — Gym routine engine P1 (Track B — gym persona content)
...
### M4 — Session plan engine P1 (Track B — yoga persona content)
...
- [metadata.md](../backend/metadata.md) — `session_plan_id` / `session_step_results` / `session_adherence` at M5.
