...
| `/clubs/[slug]` | Club home. Tabs: **Feed** (admin posts + "next event" card, with threaded replies), **Events** (upcoming + past), **Routes** (club-owned routes — see below), **Templates**, **Photos** (the club photo gallery — see below), **Members**. Join/Leave button in the hero. Admins see "New event", a post composer, a pending-requests panel (per-row Approve / Reject plus an "Approve all" button when more than one request is pending, via `bulkApproveMembers`), and the invite-link panel. **Reject** is a destructive moderation action and goes through a confirm dialog on both web (`ConfirmDialog`) and mobile (`AlertDialog`) — Approve stays one-tap. The clubs-list panel (`SocialClubs` / `clubs_screen`) renders a distinct error+retry state when the fetch fails (web reads via `browseClubsWithError` / `fetchMyClubsWithError` / `searchClubsWithError`), never the empty-list card. |
