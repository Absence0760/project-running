...
> **Status:** v1 shipped on Android. `packages/run_recorder/lib/src/workout_runner.dart` is the state machine + step expander; `apps/mobile_android/lib/widgets/workout_execution_band.dart` is the overlay; entry points are the today's-workout card on the Run tab ("Start workout") and the **Start workout** button on `workout_detail_screen` (which pops back to the run tab with the workout loaded). On `recorder.stop()` the run picks up `plan_workout_id`, `workout_step_results`, and `workout_adherence`; both `apps/mobile_android/lib/screens/run_detail_screen.dart` and the web `/runs/[id]` render the planned-vs-actual review section from those keys. Below is the original spec; what shipped largely follows it. `rewindStep` is wired up — `WorkoutRunner.rewindStep()` undoes the most recent advance (whether completed or skipped) and re-enters the previous step with `stepDistanceMetres` reset to zero, mirroring the spec at § Rewind step. The band exposes it as the leftmost button, disabled while on step 0. The **ghost pacer** (deferred from v1) is wired as well: `apps/mobile_android/lib/widgets/ghost_pacer.dart#ghostPacerPosition` walks the planned route's waypoints by `elapsed × target speed` and returns the interpolated lat/lng; `run_screen.dart` passes it through to `LiveRunMap` via the new `ghostPosition` field, which renders a faint outline-only dot under the live blue dot (no second pulse so it doesn't compete for attention). Returns null when the ghost runs off the end of the path so a finished ghost doesn't bunch onto the last waypoint for the rest of the step.

**Contents:** [Product contract](#product-contract) · [Data flow](#data-flow) · [Step expansion](#step-expansion) · [`WorkoutRunner` state machine](#workoutrunner-state-machine) · [Integration with `RunRecorder`](#integration-with-runrecorder) · [UI](#ui) · [Audio cues](#audio-cues) · [Control semantics](#control-semantics) · [Persistence](#persistence) · [Failure modes](#failure-modes) · [Testing](#testing) · [File list](#file-list) · [Open questions (before building)](#open-questions-before-building) · [Rough sizing](#rough-sizing)
...
- **Process killed mid-run**: the run is recovered on next launch via `LocalRunStore.loadInProgress()` (the same path that's covered ultra-length runs since the May 2026 hardening). As of 7d, the workout review trail survives too — `_saveInProgress` calls `WorkoutRunner.reviewMetadata` on every tick, so the recovered run lands with `plan_workout_id`, `workout_step_results`, and `workout_adherence` already on its metadata bag. The in-progress step shows as `skipped` (the user crashed before completing it); adherence reads `partial`. A separate "resume the live recording session" mode (re-spawn GPS, re-enter the runner mid-step) is still out of scope — what's shipped is preservation of the review, not continuation of the run.
...
5. **Ghost pacer on the map?** *Shipped (v2 follow-up).* Pure-geometry helper `apps/mobile_android/lib/widgets/ghost_pacer.dart#ghostPacerPosition` walks the planned route by `elapsed × target speed` and returns an interpolated lat/lng; `run_screen.dart` feeds it to `LiveRunMap` via the new `ghostPosition` field, which renders a faint outline-only dot UNDER the live blue dot. No animation — only the live position pulses — so the ghost reads as secondary signal. Returns null when the ghost overshoots the path end so it doesn't bunch on `path.last` for the rest of the step.
