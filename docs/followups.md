...
- [x] Full-screen route builder MVP (mobile click-to-place + OSRM snap + save). `apps/mobile_android/lib/routing.dart` ports the web's `routing.ts` (snapToRoad + fetchRouteThrough with foot/car profiles, 9 unit tests on URL building / parsing / error fallback). `apps/mobile_android/lib/screens/route_builder_screen.dart` is the full-screen surface: tap-to-place waypoints, mode toggle (Trail/Road/Straight), undo, clear, save dialog (name + public toggle) → `ApiClient.saveRoute` + `LocalRouteStore.save`. Reached from a new "Build" FAB on `routes_screen`. Test surface: 4 widget + helper tests. Deferred for parity with web: draggable marker reshape, elevation profile while drawing, snap-to-start pulsing marker, MapTiler place search, overlap detection (purple out-and-back), geolocate auto-center. Twin-mirrored to iOS.
