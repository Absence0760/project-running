...
| Key | Shape | Writers | Readers | Required? | Notes |
|---|---|---|---|---|---|
| `imported_from` | `string` — one of `strava`, `health_connect`, `garmin` | `mobile_android/health_connect_importer.dart`; `mobile_android/strava_importer.dart`; `apps/web/src/lib/garmin-zip.ts`; `apps/backend/supabase/functions/_shared/strava.ts` (writes `'strava'` from the live API path used by `strava-import` + `strava-webhook`) | — | Optional, audit-only | No UI consumer today; keep writing it so future audit tooling has a source. |
| `imported_at` | `string` (ISO 8601) | Same as above (including `_shared/strava.ts`) | — | Optional, audit-only | Client wall-clock time of the import, not the original run time. |
