...
| `avg_bpm` | `number` — mean heart rate in BPM across the run | `apps/watch_ios/WatchApp/ContentView.swift` (forwards `FinishedRun.averageBPM` from `HealthKitManager`) | `apps/watch_ios/WatchApp/ContentView.swift` (post-run summary) | Optional; only present for watch-recorded runs where HealthKit delivered samples | Sampled via `HKLiveWorkoutBuilder` inside an active `HKWorkoutSession`. Future web/mobile read sites can surface it alongside pace. |
