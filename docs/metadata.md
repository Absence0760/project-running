...
| Key | Shape | Writers | Readers | Required? | Notes |
|---|---|---|---|---|---|
| `title` | `string` — user-entered display title | `mobile_android/screens/run_detail_screen.dart` (edit dialog); `mobile_android/strava_importer.dart` (default from Strava activity name); `apps/web/src/lib/data.ts` (`saveRun` merges `input.title` into `metadata`); `apps/web/src/routes/runs/[id]/+page.svelte` (in-place edit on the run-detail page); `apps/backend/supabase/functions/_shared/strava.ts` (defaults to `act.name` from the live Strava API path used by both `strava-import` and `strava-webhook`) | `mobile_android/screens/run_detail_screen.dart`; `apps/web/src/routes/runs/[id]/+page.svelte` (header, share-card, social previews); `apps/backend/supabase/functions/export-data/index.ts` (CSV + GPX export) | Optional; falls back to the formatted start date when absent | Strava imports default to the Strava activity name. Editable on both platforms. |
