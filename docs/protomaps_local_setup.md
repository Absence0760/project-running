...
The production migration is a separate decision (see [decisions.md § 68](decisions.md#68-tile-rendering-honours-an-env-override-so-local-dev-can-use-self-hosted-protomaps-without-touching-prod-code-paths)) and isn't covered here. This is the dev path.
...
The web + mobile + Wear OS code in production calls the same URL builders — they just see an empty override and fall through to MapTiler. The only production change is: no production change. Flip the env back in a future migration when self-hosted Protomaps is deployed on S3/CloudFront. See [decisions.md § 68](decisions.md#68-tile-rendering-honours-an-env-override-so-local-dev-can-use-self-hosted-protomaps-without-touching-prod-code-paths).
