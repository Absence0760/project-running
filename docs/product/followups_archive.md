---
name: Gap-closure follow-ups — archive
description: Frozen snapshot of followups.md taken 2026-06-14 when the live file was pruned to open items only. Holds the completed/closed follow-ups (and their context) that were removed from followups.md. The authoritative live list of OPEN work is followups.md — this file is a dated record, not maintained going forward.
---

# Gap-closure follow-ups — archive (snapshot 2026-06-14)
...
- [x] **G2 · Web Home + History redesign** (web-only) — **DONE 2026-06-12.** self-hiding prioritised card stack on web Home; single reverse-chron timeline + client-side filter chips over the `activities` view; lift cards in the social feed (`is_public`, meals not shareable). Owns: web home + history routes + their components under `apps/web/src/routes/` + `apps/web/src/lib/components/`. No mobile, no backend schema.
  - **F14 rename + the structural timeline have since shipped (2026-06-04 → 2026-06-08):** `/history` is the unified single timeline over the `activities` view with All/Runs/Lifts/Meals chips (every chip incl. Runs is the timeline; per-tab `View all`). The F14 `/runs → /history` rename happened, but `/runs` was then **un-redirected into the dedicated run-list / management page** (filters / pagination / bulk-delete / Add run / Heatmap, a `Runs` sidebar item) — it is NOT a redirect any more (§63 amendment, 2026-06-08). Mobile mirrors all of this (`runs_screen.dart`, local-store-backed timeline). The `history.*` i18n keys + the `timeline` nav glyph are in place. **The last G2 piece — lift cards in the social feed — shipped 2026-06-12** (commit `88ade0e2`, web `SocialFeed.svelte` + `fetchFollowingActivityFeed`/`fetchFollowingLifts` in `core/data.ts`; runs via the redacted `public_runs` view, public lifts via `gym_workouts`' owner-or-public RLS projecting only `title`/`set_count`/`volume_kg`; `Lift` filter chip; lift card links to `/share/workout/[id]`; engagement stays run-only). **Meals are NOT feed-shareable:** `food_log.is_public` exists in the schema but the feed reads no food path, pinned by `tests-e2e/social/feed-lift.spec.ts` (a public `food_log` row never surfaces). Run detail/new/heatmap stay at `/runs/[id]` etc. by design.
...
- [x] **G4 · Mobile gym screens** (mostly new files) — `gym_screen.dart` (list), `gym_compose_sheet.dart` (composer), `gym_detail_screen.dart` (detail); wire `LocalGymStore`; byte-identical iOS twin. Owns: new `screens/gym_*.dart` + `widgets/gym_compose_sheet.dart`. **Dep:** needs a nav entry from G5 — build standalone, wire after. **— DONE (verified shipped 2026-06-13; doc was stale): `gym_screen.dart`/`gym_detail_screen.dart`/etc. + `LocalGymStore` + iOS twin, reachable via the Log FAB + FitnessHub Gym tab.**
  ```
...
- [x] **G5 · Mobile app shell: bottom nav + Log sheet + Home + History** — `Home / History / Log / Social / Settings`; Log centre action button → sheet (start run/lift/meal/snack, long-press = repeat last); runner-protection Settings toggle (keep Run as one-tap primary); mobile Home self-hiding card stack; mobile History timeline + chips. **Owns `home_screen.dart`** + `widgets/log_sheet.dart`, `screens/runs_screen.dart` (the History tab — there is no separate `history_screen.dart`; History reused `runs_screen.dart`), `widgets/nutrition_rings_card.dart`, `widgets/gym_summary_card.dart`; iOS twin. **— DONE (verified shipped 2026-06-13): bottom nav Home/Fitness/Social/You + docked centre Log FAB (run/lift/food, MRU float, long-press repeat) + `keepRunPrimary` toggle in Settings→Preferences + Home self-hiding card stack (`gym_summary_card`/`nutrition_rings_card`) + iOS twin.**
...
> - **Mobile nav entry — DONE (verified 2026-06-13).** The gym + nutrition
>   screens are reachable via the shipped centre Log FAB (run/lift/food) +
>   the FitnessHub Gym/Nutrition tabs + the Home today's-modality cards; the
>   G5 shell reshape landed.
> - **Mobile Settings body-metrics entry — DONE (verified 2026-06-13).**
>   `settings_body_metrics_screen.dart` (Settings → Preferences → Body
>   metrics): height/weight gated behind an explicit Art 9 health-data
>   consent toggle (`grantHealthDataConsent`/`withdrawHealthDataConsent`),
>   activity-level + goal as nutrition prefs, feeding `loadNutritionTargets`;
>   iOS twin. The web↔mobile body-metrics parity gap is closed.
...
- [x] **G6 · Nutrition foundation (pure + schema)** — `nutrition_targets` parity pair (Mifflin-St Jeor BMR × activity-level); `body_metrics` migration (height on `user_profiles` + `body_metrics` weight time-series, owner RLS, cascade-delete); Settings height/weight entry; privacy-disclosure updates (iOS label / Play Data Safety / Open Food Facts sub-processor). **Feeds G1** (add `body_metrics` to export). Owns: `nutrition_targets.ts`/`.dart`, the new migration, Settings body-metrics entry. **— DONE (verified 2026-06-13; landed 2026-06-04 per the Wave-3 note above).**
  ...
  ```
- [x] **G7 · Nutrition web** — `/nutrition` (rings + meal-slot daily view + weekly trends), `/nutrition/log` (Open Food Facts search → tap → confirm portion; manual entry fallback only); `food_search` client + web `core/data.ts` food queries; water tracker; Nutrition sidebar item (flag-gated). **Dep:** G6. Owns: `apps/web/src/routes/nutrition/`, web food data-access, nutrition sidebar in `+layout.svelte`. **— DONE (verified 2026-06-13; landed 2026-06-04 per the Wave-3 note above).**
  ```
  ...
  ```
- [x] **G8 · Nutrition mobile** — `nutrition_screen.dart`, `nutrition_log_sheet.dart`, `food_search.dart` (Open Food Facts, pluggable-fetcher seam); wire `LocalFoodStore`; iOS twin. Barcode scan is a v1.1 fast-path on the same lookup. **Deps:** G6 + plugs into G5's shell. Owns: new `screens/nutrition_screen.dart` + `widgets/nutrition_log_sheet.dart` + `food_search.dart`. **— DONE (verified 2026-06-13; landed 2026-06-04 per the Wave-3 note above).**
  ```
...
- [x] **Typed-events (slice E): mobile `EventEditor` create-wiring** — **DONE 2026-06-12** (mobile + iOS twin). `event_form_sheet.dart` now picks `category` (run/cycle/class/social) as its first control, shows a free-text `discipline` for a `class`, and self-hides + clears the athletic distance field off-category via `isAthleticEventCategory` (mirroring the web `pickCategory`); `SocialService.createEvent` + `buildCreateEventBody` persist `category` + `discipline` (class-only, trim→null) + `gym_template` (class-only, via `gymTemplateFromInputs`) and gate `route_id`/`distance_m`/`pace_target_sec` to athletic categories — the same body shape web's `createEvent` writes. New `eventEditorCat*`/`eventEditorCategory`/`eventEditorCategoryHint`/`eventEditorDiscipline*` ARB keys (camelCase twins of web's `eventEditor.cat*`/`discipline*`) added to all 7 catalogues + gen-l10n regenerated + iOS twin mirrored. Widget tests pin the category picker, the class-only discipline field, and the off-category field hiding; `create_event_parity_test.dart` pins the body shape (category/discipline/gym_template + athletic-field gating). The read-only `event_detail_screen.dart` gate already landed. `parity.md` typed-events row flipped to ✓ on Android/iOS. See [club_events.md § Slice E](../features/club_events.md).
...
- [ ] **Route-design preferences** — once the graph-cycle generator exists, expose preference knobs (avoid highways, stick to neighbourhoods, scenic/quiet, elevation-aware, opt-in cul-de-sacs) as edge weights/filters + a multi-objective scoring layer on the graph search. Design in [graph_cycle_loop_generation.md § Extension](../features/graph_cycle_loop_generation.md). **Avoid-highways / prefer-residential half DONE 2026-06-14**: a "Quiet roads (avoid highways)" toggle on `/routes/new` sets `preference:'quiet'` on the generate request; `handler.ts` posts a GraphHopper `custom_model` (round_trip POST with `ch.disable`) that down-weights motorway/trunk/primary and up-weights residential/living_street, skips the graph-cycle sidecar (which doesn't yet honour preferences), and falls back to plain generation if the engine rejects the model or finds no loop — a preference never breaks generation. Web-only (route generation is a web-canonical surface; there's no mobile route builder). Remaining (~2–3 d, needs v3): scenic/park-adjacency, elevation-aware, cul-de-sac mode, and the multi-objective post-hoc ranking — the half that needs the graph search where we own the edges + scoring.

- [x] **AI route assistant** — an LLM *bookend* around the generator: plain-English request → validated generation constraints (tool-use extraction), and generated route → plain-English description. The LLM never routes (graph search owns correctness); it translates intent + writes prose, reusing the existing Anthropic/AI-Coach transport + paywall. **Both halves shipped (web):** the **description** half (`route_describe/`, `/api/coach/route-describe`, templated fallback, `decisions.md § 151`) and the **request** half (2026-06-14 — `route_request/handler.ts` + `route_request/constraints.ts` whose `validateConstraints` is the trust boundary that clamps/whitelists every extracted field, `/api/coach/route-request` + coach Lambda `/route-request`, `route_request_client.ts`, and the "Describe the route you want" box on `/routes/new` that fills the generator form; forced `tool_choice` for typed extraction, Pro-gated fail-closed, manual form unaffected on any failure). Design + shipped-state in [ai_route_assistant.md](../features/ai_route_assistant.md). Remaining is product/optional: the `avoid_highways` constraint only *bites* once the route-design-preferences edge-weighting lands (entry above), and mobile parity (web-canonical, decisions §24).
