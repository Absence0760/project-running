...
- [x] Migration: `runs.kind` column carrying the broader modality (`'run' | 'lift' | 'meal'`) — the existing `activity_type` enum (`run | walk | cycle | hike`) stays as the finer-grained running-family classification. The new column is what the `activities` view unions on. **Shipped** in `20261204_001`.
...
- [~] Home composes all three modalities (self-hiding); History timeline + filter chips over the `activities` view (windowed/paginated). **Web shipped** for runs + lifts (see Home/History rows above); nutrition cards/rows land with the nutrition module.
