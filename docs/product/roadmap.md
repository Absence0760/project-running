...
- [x] **Race calendar + results import** — [race_calendar.md](../features/race_calendar.md) (shipped 2026-06-20, migration `20270214_001`). *Supersedes backlog row #10.* Discover races near me + entry links (web `/races` + mobile `RacesScreen`) + auto-match-on-record (inform-tier prompt on run-detail, both platforms) + manual paste import + crowd-submitted listings. parkrun shipped; **RunSignUp leg built fail-closed behind the missing `RUNSIGNUP_API_KEY`** (returns 503 `provider_not_configured`; the UI shows the unavailable explainer) — provisioning the key is the only remaining go-live step. **ChronoTrack leg built fail-closed** the same way (2026-06-20) behind the missing `CHRONOTRACK_CLIENT_ID` / `CHRONOTRACK_USER_ID` / `CHRONOTRACK_PASSWORD` (503 `provider_not_configured`; Settings card shows the unavailable explainer) — provisioning the three CTLive credentials is the only remaining go-live step. UltraSignup / RaceResult per-site scrapers deferred as scoped follow-ups.
