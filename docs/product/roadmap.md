...
- [ ] Extend to in-progress runs, auth flows, and sync paths over time. **(Stretch — not yet built.)** The runs + routes round-trips shipped first; the in-progress-run / auth / sync round-trips follow the same Dart-write → Node-read fixture pattern.
