...
- Food database (USDA + Open Food Facts). ~~Barcode scan (mobile-only, camera permission).~~ **Shipped 2026-06-20** — see the mobile nutrition row above.
...
- [ ] Extend to auth flows and sync paths over time. **(Stretch — not yet built.)** The runs + routes + in-progress-run round-trips shipped; the auth / sync round-trips follow the same Dart-write → Node-read fixture pattern.
...
- [x] **Challenges & competitions** — [challenges.md](../features/challenges.md). **Shipped web + mobile 2026-06-19** (migrations `20270209_001` + `20270210_001`). Monthly/weekly mileage·time·activity-count·streak challenges (vert deferred — no per-run elevation column); individual / club-vs-club / group-goal; read-time leaderboard via a single GROUP-BY RPC (no N+1); self-hiding strip on dashboard + `/social` tab + mobile Social sub-tab; completion badge + notification via opportunistic RPC + daily cron sweep. No compliance gate. *Flagship engagement bet.*
...
- [x] **Race calendar + results import** — [race_calendar.md](../features/race_calendar.md) (shipped 2026-06-20, migration `20270214_001`). *Supersedes backlog row #10.* Discover races near me + entry links (web `/races` + mobile `RacesScreen`) + auto-match-on-record (inform-tier prompt on run-detail, both platforms) + manual paste import + crowd-submitted listings. parkrun shipped; **RunSignUp leg built fail-closed behind the missing `RUNSIGNUP_API_KEY`** (returns 503 `provider_not_configured`; the UI shows the unavailable explainer) — provisioning the key is the only remaining go-live step. Per-site scrapers (ChronoTrack / UltraSignup / RaceResult) deferred as scoped follow-ups.
