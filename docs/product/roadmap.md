...
- [x] **History unification (mobile + web)** — single timeline of activities (runs + lifts + meals) with filter chips. Run-detail / lift-detail / meal-detail are separate routes. **Web shipped**: `/runs` gains All/Runs/Lifts/Meals chips + a unified timeline over the `activities` view when `multi_modal_nav` is on and a second modality exists (rows link to `/runs/[id]` + `/gym/[id]`; meals read-only pending the nutrition detail route). Pure runner / flag-off page unchanged. **Mobile shipped** (byte-identical twin): `runs_screen.dart` hosts the same chips + a day-grouped timeline (`widgets/activity_timeline_list.dart` over `ApiClient.fetchActivities`), gated on the gym store being wired + a second modality having data; run rows open run-detail (local lookup, else `fetchRunById`), lift rows open `GymDetailScreen`, meals read-only. Remaining (both): a per-meal nutrition detail route.
