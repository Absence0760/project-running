...
- [~] **Phase 4b — native push (FCM / APNs):** The remaining device-delivery leg — native push to iOS (APNs) + Android (FCM) — is now **built, gated on credentials** (migration `20270212_001`, `native_push` kind). A third consumer of the `notifications` rows copies the shipped `web_push` sibling: the FCM HTTP v1 + APNs HTTP/2 sender (`apps/job_worker/internal/nativepush/`) + handler (`handler_native_push.go`, reuses the `push_notifications` pref gate), the device-token-gated enqueue trigger + `clear_device_token` prune RPC, and the mobile `firebase_messaging` token registration (`push_messaging_bridge.dart`, both twins — register on sign-in, re-register on refresh, remove on sign-out, per-device `is_notifications_enabled` filter). `device_tokens` (migration `20260506_001`) finally has its client write path. Going live is blocked only on user-supplied Firebase / APNs credentials on the worker (`FCM_*` / `APNS_*`) + the per-app config files (`google-services.json` / `GoogleService-Info.plist`); unset → jobs finish done, rows stay pending, the mobile bridge no-ops. See `docs/features/native_push.md` + `decisions.md § 161`. (Web push is a separate slice — the VAPID path already ships; see `parity.md`.)
...
- [x] **Achievements / badges** — [achievements.md](../features/achievements.md). **Web shipped 2026-06-19** (migration `20270208_001`): badge catalogue as code (guided-runs pattern), full-rebuild SECURITY DEFINER trigger mirroring `personal_records`, `achievements` table; profile Achievements tab + feed badge strip + public `/share/badge/[id]` page + OG card + `share-badge` Lambda; `'achievement'` notification kind. Mobile twin deferred to follow-up. Pairs with Challenges.
