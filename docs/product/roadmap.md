...
- [x] SEO-indexed public route pages — `/share/route/[id]` is prerendered per-route (build-time `entries()` + meta `load()`), listed in `/sitemap.xml`, and carries a `<link rel="canonical">` + `og:url` + schema.org JSON-LD (`WebPage` + `BreadcrumbList`, via `buildRouteJsonLd` in `share/share_meta.ts`). The in-app `/routes/[id]` canonicals to the public page so the two URLs for one route don't split ranking signals. JSON-LD omits `geo` (track is privacy-clipped server-side). Request-time freshness for routes made public after a build (the share-run Lambda pattern) is deferred — see followups.
...
- [ ] **Duplicate-a-week bulk op** (split out of "Bulk operations" above) — needs an atomic INSERT of a week + its workouts plus a `week_index` re-index of the following weeks and a date shift, so it warrants a single `SECURITY DEFINER` RPC (`duplicate_plan_week(p_week_id)`) rather than client orchestration. ~1 dev-day incl. migration + pgTAP. Shift-by-N-days + mark-week-recovery already ship (`plan_bulk_ops.ts`).
...
- [x] **Mobile bottom nav** — **shipped (G5)**: `home_screen.dart` is `Home / History / Log / Social / Settings`. The `Run` tab leaves the nav; the centre `Log` slot is a raised FAB (`widgets/log_sheet.dart`) presenting Start run / Start lift / Log meal / Log snack, most-recently-used floats to top, long-press repeats the last modality (one-tap "start run" muscle memory preserved), ≥48 dp + a Semantics label. Run stays a keep-alive PageView page so a recording survives navigating away. Shipped **ungated** with a `prefsKeepRunPrimary` "Run as primary action" toggle (protect-the-core-runner) rather than behind `multi_modal_nav`. Twin-mirrored to iOS.
- [~] **Home redesign (mobile + web)** — cards from every modality: today's run, today's lift, daily nutrition rings, plus the existing dashboard cards (mileage chart, training load, fitness, intensity, weekly goal). Cards self-hide when the modality has no data. **Web shipped** (gym slice): a Today's-lift card + a Recent-lifts trend card + a first-run "log a lift" footer on `/dashboard`, all flag- + data-gated. The nutrition macro rings shipped on their own `/nutrition` page; folding a daily-nutrition-rings card into the `/dashboard` Home stack is the remaining piece of this redesign. **Mobile Home card composition shipped (G5):** `dashboard_screen.dart` composes a self-hiding today's-lift card (`widgets/gym_summary_card.dart`) + today's nutrition rings card (`widgets/nutrition_rings_card.dart`), each omitted when that modality has no data today, 2-up on phones ≥360 dp. Remaining either side: a recent-lifts trend card (web-only) and the web Home nutrition card.
- [~] **History unification (mobile + web)** — single timeline of activities (runs + lifts + meals) with filter chips. Run-detail / lift-detail / meal-detail are separate routes. **Web shipped**: `/runs` gains All/Runs/Lifts/Meals chips + a unified timeline over the `activities` view when `multi_modal_nav` is on and a second modality exists (rows link to `/runs/[id]` + `/gym/[id]`; meals read-only pending the nutrition detail route). Pure runner / flag-off page unchanged. Mobile pending.
...
- [~] **Feature flag** — `multi_modal_nav` per user (universal `user_settings.prefs`, default off — `docs/backend/settings.md`). **Read on web** to gate the Gym sidebar item. The **mobile** nav reshape shipped (G5) **ungated** — it ships for all mobile users with a `prefsKeepRunPrimary` toggle for pure runners rather than hiding behind the flag.
...
- [x] Web: **shipped** — `/nutrition` (daily log + rings + weekly trends) + `/nutrition/log` (composer — **search Open Food Facts → tap → confirm portion**; manual entry only as the no-match fallback), flag-gated.
- [x] Mobile: **shipped** — `nutrition_screen.dart`, `nutrition_log_sheet.dart`, `food_search.dart` (Open Food Facts client, pluggable-fetcher seam), twin-mirrored. **Nav entry landed (G5):** reachable via the Log sheet ("Log meal"/"Log snack") + the Home nutrition rings card. Barcode scan (camera) is a v1.1 fast-path on the same lookup (deferred).
...
- [~] Home composes all three modalities (self-hiding); History timeline + filter chips over the `activities` view (windowed/paginated). **Web shipped** for runs + lifts (see Home/History rows above). **Mobile Home shipped (G5):** self-hiding today's-lift + nutrition rings cards on `dashboard_screen.dart`. The unified mobile **History** timeline is still pending (mobile History remains the run-only list).
