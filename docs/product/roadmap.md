...
- [x] **Race calendar + results import** — [race_calendar.md](../features/race_calendar.md) (shipped 2026-06-20, migration `20270214_001`). *Supersedes backlog row #10.* Discover races near me + entry links (web `/races` + mobile `RacesScreen`) + auto-match-on-record (inform-tier prompt on run-detail, both platforms) + manual paste import + crowd-submitted listings. parkrun shipped; **RunSignUp + UltraSignup legs built fail-closed behind their own missing credential pairs** (`RUNSIGNUP_API_KEY` / `ULTRASIGNUP_API_KEY` etc.; each returns 503 `provider_not_configured` per provider; the Settings integrations card shows the unavailable explainer) — provisioning each key is the only remaining go-live step (decisions §185). ChronoTrack / RaceResult per-site scrapers deferred as scoped follow-ups.
