...
- [x] **Public "Learn" / guides pages** — [learn.md](../features/learn.md) (specced in [learn_pages.md](../features/learn_pages.md)). **Shipped web** (2026-06-19): `/learn` hub + `/learn/<slug>` articles + `/learn/category/<id>`, eight evergreen new-runner guides as prerendered MDsvex content, full SEO (canonical / Open Graph / `Article` JSON-LD) mirroring `/share/*`, sitemap inclusion, and a contextual in-app CTA per guide. Web-only (no twin), shell-less + anon. The "your first race" guide's CTA uses the interim `/social?tab=clubs` pending a race calendar. **Prose localization complete** (2026-06-20): all eight guides authored in all six locales (`en` source + `de/fr/es/ja/pt-BR`), resolved with field-by-field English fallback (decisions §179).
