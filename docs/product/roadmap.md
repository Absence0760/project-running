...
| 8 | **Photos on runs, routes, and clubs** ([x] **runs shipped** — `run_photos` + `run-photos` bucket + server-side EXIF strip; [x] **route photos shipped** — `route_photos` + private `route-photos` bucket, web + android (+ iOS twin), client-side EXIF strip, backlog C1; [x] **club photos shipped** — `club_photos` + private `club-photos` bucket + `club_photo_process` EXIF/thumbnail worker, web + android (+ iOS twin), member-upload / owner-or-admin-delete gallery, migration `20270301_001`, decisions §190) | done | Strava, AllTrails | `run_photos` / `route_photos` / `club_photos` (buckets `run-photos` / `route-photos` / `club-photos`) shipped — migrations `20260525_001` / `20270114_001` / `20270301_001` | Deferred: server-side thumbnails + EXIF worker for route photos (run + club have the worker; route photos still client-strip only) |
...
- [x] **Public "Learn" / guides pages** — [learn.md](../features/learn.md) (specced in [learn_pages.md](../features/learn_pages.md)). **Shipped web** (2026-06-19): `/learn` hub + `/learn/<slug>` articles + `/learn/category/<id>`, eight evergreen new-runner guides as prerendered MDsvex content, full SEO (canonical / Open Graph / `Article` JSON-LD) mirroring `/share/*`, sitemap inclusion, and a contextual in-app CTA per guide. Web-only (no twin), shell-less + anon. The "your first race" guide's CTA uses the interim `/social?tab=clubs` pending a race calendar. Localization scope (prose translation) still owed — chrome is six-locale, prose English-only with a fallback resolver wired.
