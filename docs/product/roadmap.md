...
- [ ] Plan editor (web-first, mobile read-only in v1) — *create + inline per-day edit ship; the import + bulk-ops items below don't*
  ...
  - [~] Bulk operations: shift the plan forward/back by N days + mark a week as recovery ship (`lib/training/plan_bulk_ops.ts` — pure date-shift + recovery-scaling helpers; `/plans/[id]` owner controls orchestrate the per-row updates). *Still pending: duplicate-a-week — it needs an insert + week-index re-index, best done atomically via an RPC, so it's deferred to its own migration-backed change*
...
- [ ] **Adjustment based on missed sessions and recovery patterns** — *not built: the generator emits a static plan and doesn't re-plan around missed sessions. This is the last real training-engine gap and Runna's headline feature.* Scoped task:
  - **Engine (pure, web-first + Dart twin):** new `lib/training/plan_replan.ts` — `replanRemaining({ weeks, workouts, completedRuns, today, goalDate })` regenerates only the *future* portion of the plan. Rule-based (deterministic, mirrors the generator's phase model — no ML): redistribute a missed week's deficit forward without spiking any single week >10%, honour `missedWorkoutAdvice` (insert a make-up long run when it says `make_up`, drop it when `skip`), ease the next week when the runner is cumulatively over-running (reuse `weeklyDrift`), and **never move the goal race date or compress the taper**. Past weeks are frozen.
  - **Surface:** an owner-only "Re-plan remaining weeks" action on `/plans/[id]` that shows a preview diff (which workouts move / change) and applies on confirm. Reuses the bulk-op orchestration pattern (per-row `updatePlanWorkout` / `updatePlanWeek`).
