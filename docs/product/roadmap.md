...
| 5 | **Trail / offline navigation** (turn-by-turn nav on a loaded route, offline tile packs, condition reports). [x] shipped — community **condition reports** (web canonical + mobile, migration `20270212_001`, privacy-redacting `route_conditions_for_viewer` RPC); **offline tile packs** (mobile, sibling on-disk cache hooked onto the offline pin, read-through live map, `decisions.md § 167`); **turn-by-turn voice cues** (mobile, geometric generator over the route polyline, `decisions.md § 166`); plus the earlier "Save for offline" per-route pin + phone → Wear OS DataLayer push of starred routes (`decisions.md § 64`). Routing-engine question resolved: self-contained geometric cue generator (no external engine), `decisions.md § 166`. **Prod gates:** condition reports are a privacy-boundary surface (run `/audit/privacy-zones`); offline tile packs need MapTiler-ToS sign-off for bulk tile persistence before prod (else use self-hosted Protomaps), `decisions.md § 167` | 3–4 wk | AllTrails, Komoot | `route_conditions` (user reports), tile-pack store on disk | Resolved: geometric generator (§166) |
...
- [x] **Turn-by-turn offline nav + condition reports** — [trail_navigation.md](../features/trail_navigation.md). *Advances backlog row #5.* Shipped: `route_conditions` reports (web + mobile, migration `20270212_001`), offline tile-pack store (mobile, `decisions.md § 167`), turn-cue voice cues (mobile, `turn_cues` parity helper, `decisions.md § 166`). Routing engine resolved: self-contained geometric generator (no external engine). Prod gates: `/audit/privacy-zones` on the conditions surface + MapTiler-ToS sign-off for offline tile persistence.
