...
| 4 | **Heatmap / popular-route discovery** ([x] v1 shipped — public-routes-only heatmap via PostGIS `heatmap_points_in_bbox` RPC on top of densified `routes.geom`; web overlay on `/routes?tab=heatmap` with a MapLibre `heatmap` paint layer; refreshes on map moveend with 350 ms debounce, 5k point cap. Privacy: route opt-in already gates inclusion via `routes.is_public`. **[x] v2 shipped (web) — turned the blob into a route browser:** filter chips (`popular` / `friends` / `featured` / `hidden_gems`) on `discoverable_routes_in_bbox`'s new `p_filter` arg, MapLibre clustering on the route pins, a viewport-synced collapsible list panel, and the heat layer dimming as you zoom in so it stops competing with the pins. `friends` = public routes *created by* users you follow (there is no retained run↔route link to power "run by friends"); `hidden_gems` = un-run public routes past a 1 km sanity floor. **Mobile discovery read-path (filters + clustering + list) still deferred — web-only today, per [decisions § 24](../architecture/decisions.md#24-web-is-the-canonical-feature-surface-mobile-and-watches-are-platform-additive).**) | 2 wk | Strava, Komoot | Migrations `20260828_001_heatmap_points.sql`, `20261113_001_discoverable_routes_filter.sql` | Resolved: opt-in (via existing `routes.is_public`) — no separate user-level toggle |
