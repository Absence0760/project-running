...
- [ ] **`apps/job_worker` Go tests don't gate CI** — the entire worker Go suite (worker, dataexport, premium, livehub, stravahook, and `internal/personal_data_export_guard_test.go`) is run only by local `go test ./...`; CI just `go build`s the worker (`ci.yml` builds the livehub e2e binary, `security.yml` builds for CodeQL — no `go test`/`go vet` step). Consequence: the GDPR Art 20 export-completeness guard (which fails when a new `user_id`-bearing table isn't wired into `exportPersonalDataSpecs` or the reasoned exclusion list) only fails locally, not the build. **Fix:** add a `go test ./...` step (working-directory `apps/job_worker`) to the existing job_worker CI job — it already has `setup-go` wired. Left out of the SERVER-GO change because `.github/workflows/ci.yml` is outside that task's `apps/job_worker/**` + `functions/**` ownership scope. Pairs with the export guard added 2026-06.
