...
- [ ] **`playwright install-deps` hard-fails a test shard when the runner's apt mirrors do not answer.** Five shard failures across two pushes on 2026-08-18 (round 4: shards 1, 7, 13; round 5: shards 6, 7), every one identical: three 120 s attempts, all timing out, then `playwright install-deps failed three times — the runner's apt mirrors are not answering`. No test ever runs, and a re-run passes clean every time. The step installs Chromium's **system libraries**; the browser binaries themselves are cached separately. A third-party mirror outage should not fail a test shard — the durable fix is to verify the libraries are present and install only what is actually missing, failing only when a genuinely required library cannot be resolved. Do **not** just add more retries: three already run, and the failure mode is an outage rather than slowness.

- [ ] **The `pages` sim scenario wedges the firmware executor on the CI runner but not on arm64 macOS, and issue #754 — which the harness blames — is closed and describes a different wedge point.** Failed 3 consecutive CI runs on 2026-08-18 (PR #787, jobs `95791544620`, `95812234823`, `95819446096`), always `scenario pages`. Reproduction attempted and **failed to reproduce locally: 4/4 pass** on the merged branch, three of them at CI's exact `--budget 300`; `pages` is the FIRST scenario in its job, so no predecessor contaminates it, and CI and this machine pin the SAME Renode (1.16.1) — the difference is the host (CI's `linux-portable-dotnet` x86_64 vs arm64 macOS). Round 3's firmware changes are **cleared** as the cause: the baro task's 1 Hz DEBUG line, which is the invariant the wedge detector keys off, still ticks at a 0.996 s max gap, so neither the new `continue` on an implausible altitude nor the `if let Some(corrected)` guard silenced it, and the `continue` cannot busy-loop because the `select(ticker.next(), ...)` await sits at the loop head. Two things the harness message gets wrong: it names #754 as the known occurrence, but #754 is **CLOSED**, is scoped to the `storm` scenario at ~1-in-30, and wedged at t=19.0 s **mid-run** — whereas this wedges right after the boot supply/UICR diagnostics, before the first baro sample (those UICR lines are normal and appear in the passing local runs too). Reopen #754 or file a successor, and point the harness at whichever is live; a message citing a closed issue trains readers to dismiss it.
...
- [ ] **The export builds the whole archive in memory**, so it carries a 5000-run cap and a 50,000-row-per-section ceiling. Both are now honestly reported (`complete: false` + `incomplete`), but the durable fix is a chunked / resumable Storage upload that removes them. Art 20 relevant for the deepest histories.
...
- [ ] **The mobile LOCAL backup writer is silently capped at 1000 runs.** `ApiClient.fetchRunRowsRaw` issues an unranged select, so PostgREST clamps it to `db-max-rows` — tighter than the server path's 5000 ceiling, and with no `complete` flag to disclose it. Same class as the Art 20 paging fix, on the fallback path. Needs `fetchAllPages`-equivalent paging plus a completeness verdict the settings screen can render through the shortfall row that now exists. This is also why falling back to the local writer when the server archive is short would be the wrong repair.
...
- [ ] **A partially-failed import push is still silent.** `saveRunsBatch` can return a non-empty `failed` id set **without throwing** — those runs are on the device and not on the server, which is exactly the claim `importStatusCloudPushDeferred` makes. Both `_saveImportedRuns` and `_backfillHealthConnectTracks` set the deferred flag only from the catch, so a half-landed batch reports a clean import. Set it on `failed.isNotEmpty` in both together, so the two paths cannot diverge.
- [ ] **`ImportScreen` has no seam for the Health Connect route path.** `requestHealthRoutePermission` and `HealthConnectImporter.fetchRoutes` both refuse off Android, and the push branch needs a signed-in `ApiClient`, which has no fake — so the route-backfill deferral is guarded by source assertions rather than behaviour. Either inject the two platform calls or give `ApiClient` a fakeable seam for `userId` + `saveRunsBatch`; the same gap blocks a real test of the disclosure above.
- [ ] **`NutritionScreen` never inits its template / recipe stores.** `initState` calls `loadAll()` without `init()`, so `OfflineSyncStore.dir` is null and `persist()`'s `dir!` throws on every "Save as meal" / "Save as recipe" (`nutrition template save failed: Null check operator used on a null value`, visible in the shipped widget-test output). The row lands in memory so the UI looks right, but nothing is written to disk and nothing ever drains to the server. Pre-existing; found while mirroring the diary-day work.
- [ ] **Wear OS activity words drift from web and mobile.** The wrist says `Lauf`/`Correr`/`Caminar`/`Rando` where the phone says `Laufen`/`Carrera`/`Caminata`/`Trail`, and still says `Hike`/`Wandern`/`Senderismo` where both other platforms took the `hike` -> "Trail run" product rename. Unclear whether the shorter wrist words are deliberate for the 56 dp `maxLines = 1` chip — `Radfahren` and `Senderismo` already exceed it today, which argues they are not. **Owner call**: the rename half is a product decision, not a translation fix, so it was deliberately not steamrollered.
- [ ] **`deleteNotifications` is now N statements, not one.** Chunking it means a partial dismiss is possible where a >100 list previously deleted nothing at all. Partial was judged better than a silent no-op, but it is not transactional; an RPC taking the id array would be.
...
- [ ] **`fetchGymRoutineSessions` under-reports past ~500 sessions, now on two platforms.** Both web `data.ts#fetchGymRoutineSessions` and the new `ApiClient.fetchGymRoutineSessions` window at 500 rows because an unbounded PostgREST SELECT truncates silently at `db.max-rows` with a 200. Neither panel words its count as all-time, so neither lies outright, but a lifter running one routine weekly for a decade sees a capped figure. The durable fix is a SQL aggregate RPC mirroring `gym_exercise_records` (count + max(started_at) + completed/graded tallies server-side), which also stops reading every row just to reduce it; it needs a migration, so it stayed out of scope. **Do NOT fix mobile alone** with `packages/api_client/lib/src/paged_read.dart#readAllPages` — a complete mobile count beside a capped web one is exactly the cross-platform drift the parity pair exists to prevent. Web first per § 24, then mirror.
- [ ] **The displayed "Elapsed" stat on `/live/[id]` freezes at the last ping while the page internally knows the true race clock.** `raceElapsedS` (`liveElapsedS`, § 601) advances the clock by the ping age for cut-off maths, but the stat strip renders the raw frozen `elapsed`. A spectator on a 240-mile race doing mental cut-off arithmetic during an 18-hour dead zone is off by 18 hours. Deliberately left: `elapsed` excludes auto-paused time, so advancing the *displayed* figure by wall clock overstates it for a runner who paused — a trade `liveElapsedS` documents accepting for a deadline, but which needs a product decision before it reaches a headline stat.
- [ ] **Local Playwright is unusable when several agents share the checkout, and the failure is worse than the documented one.** `playwright.config.ts` sets `reuseExistingServer: !CI`, so a dev server started from another worktree serves *its* bundle — and when that server dies mid-suite the result is `ERR_CONNECTION_REFUSED`, so unrelated specs fail in ways indistinguishable from real breaks. Three rounds in this batch lost cycles to it. The harness guard should assert the served build matches the working tree, and the symptom list in `docs/testing/testing.md` should name the connection-refused case, not just the stale bundle.
