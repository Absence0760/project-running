...
- [ ] **Mobile mirror: per-exercise records + progression surfaces** — shipped on web 2026-06-08 as two linked web-only surfaces over `LocalGymStore`'s set history: `/gym/records` (`apps/web/src/lib/gym/exercise_records.ts`) rolls up each weighted exercise's current bests (est. 1RM / heaviest set / top volume) + last-performed date + distinct-session count; each card links to `/gym/exercise?name=` (`apps/web/src/lib/gym/exercise_history.ts`) — that exercise's progression over time: headline est.-1RM delta vs the first session + a session-by-session list with a new-e1RM PR badge per session. Both reuse `computeExercisePrs` / the other `gym_prs` primitives so the numbers can't drift from the per-workout PR badges. Mobile has the PR engine (`gym_prs.dart`) and the gym list/detail screens but neither surface. To reach parity: port `exercise_records.ts` → `exercise_records.dart` **and** `exercise_history.ts` → `exercise_history.dart` (both would become TS↔Dart parity pairs — register them in `CLAUDE.md` + `shared-library-syncer.md` when ported), drive them from `LocalGymStore`, and add a records screen (reachable from `gym_screen.dart`) drilling into a per-exercise progression screen. Web-first per §24; no shared setting → no cross-device drift.
