...
- [ ] **`fetchGymWorkouts` has no date window, so a deep diary day under-reads its lift calories** — `fetchRuns` takes `startedAtFrom` / `startedAtBefore` and the nutrition day view uses them to fetch exactly the viewed day's runs, but its gym sibling only takes a `limit`, so the page pulls the newest 50 sessions and filters. A diary day older than the caller's 50 most recent lifts therefore contributes no gym calories to that day's `exerciseKcal` — an under-read, identical in effect to "the lift wasn't logged yet", not a wrong kind of number, but avoidable. Fix is the symmetric one: give `fetchGymWorkouts` / `fetchGymWorkoutsWithError` the same two optional bounds `fetchRuns` has and window the call in `routes/nutrition/+page.svelte` (dropping its `GYM_FETCH_LIMIT`). Touches `lib/core/data.ts`, which is why it was not folded into the diary round.
- [ ] **A long-lived `/nutrition` tab does not roll over at local midnight** — `todayIso` / `yesterdayIso` (`routes/nutrition/+page.svelte`) are recomputed only inside the `$effect`, which fires on a `$page.url` or `authReady` change and never on a timer. A tab left open past local midnight therefore keeps labelling the previous day "Today" and keeps the Next-day button wrongly disabled until the user navigates. **This is a stale-label bug, not a data bug — do not over-escalate it:** every write path resolves `entryTimestampFor(viewDate, new Date())` at save time with a fresh clock ([decisions § 591](../architecture/decisions.md)), so nothing is ever filed onto the wrong day; only the header, the Next-day affordance and the `dash.today` section labels go stale. The fix is a rollover check when the tab **regains visibility** (a `visibilitychange` listener re-running the same `isoDateOf(new Date())` the effect does, and re-resolving the day when it has moved), not a polling timer — a per-minute interval would burn a wakeup on every open tab to catch an edge that only matters once a day.
