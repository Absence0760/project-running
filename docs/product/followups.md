...
- [ ] **Mobile mirror: per-exercise personal-records surface** — shipped on web 2026-06-08 as `/gym/records` (`apps/web/src/lib/gym/exercise_records.ts`, pure, web-only): a roll-up of each weighted exercise's current bests (est. 1RM / heaviest set / top volume) plus last-performed date + distinct-session count, reusing `computeExercisePrs` so the numbers can't drift from the per-workout PR badges. Mobile has the PR engine (`gym_prs.dart`) and the gym list/detail screens but no records screen. To reach parity: port `exercise_records.ts` → `exercise_records.dart` (it would become a TS↔Dart parity pair — register it in `CLAUDE.md` + `shared-library-syncer.md` when ported), drive it from `LocalGymStore`'s set history, and add a records screen reachable from `gym_screen.dart`. Web-first per §24; no shared setting → no cross-device drift.
