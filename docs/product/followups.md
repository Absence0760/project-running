...
- [ ] **Mobile/watch a11y + price polish (2026-05-30 Medium audit)** — a batch of byte-identical-twin Flutter + Wear Kotlin changes left after the signup-ToS-links fix shipped: (1) mobile delete-account re-entry challenge (mirror the web ConfirmDialog requireText to `settings_account_screen.dart` so the twin matches the web hardening); (2) coach + onboarding dot animations should honour `MediaQuery.disableAnimations` (WCAG 2.3.3); (3) `SemanticsService.announce` on dashboard + route-builder state changes (only the run screen has it today); (4) `settings_pro_screen.dart` hard-coded `$9.99/month` → `getOfferings()` store-reported price (blocked on a configured RevenueCat project — see the RevenueCat item below; until then mirror the web "billed in USD" honesty note); (5) Wear OS pre-run chips (activity/route/pace/sign-in) need explicit `contentDescription` (`RunWatchApp.kt`). Each needs the Flutter/Android toolchain + `dart analyze` + widget tests + the iOS-twin mirror.
...
- [ ] **Move club/event membership oracles to the `private` schema** — `is_club_member`, `is_club_admin`, `is_event_organiser`, `is_race_director` are SECURITY DEFINER + anon-EXECUTE, so anon can probe membership/role for any (user, club/event) via `/rest/v1/rpc/...` (audit-findings 2026-05-30 Medium). The fix is the `is_run_visible_to` pattern (`20260812_001`): recreate them in `private` (PostgREST doesn't expose that schema), grant EXECUTE so RLS still evaluates, and rewrite EVERY dependent policy to the `private.`-qualified name. They're referenced across ~30 policies (`is_club_member` alone in 28 migration files), each needing its *latest* body re-emitted — a focused, security-critical migration of its own, not a batch edit. The DROP of the public function fails loudly if any policy still references it, which is the safety net to verify completeness. Validate against the full pgtap RLS suite.
