...
- [ ] **All-time routine session count needs a server-side aggregate.** `fetchGymRoutineSessions` windows at 500 rows because an unbounded PostgREST SELECT truncates silently at 1000. Past roughly a decade of running one routine weekly, the panel's session count and completion rate under-report. The durable fix is a SQL aggregate RPC mirroring `gym_exercise_records`; it needs a migration, so it was out of scope for the round.
