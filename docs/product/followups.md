...
- [ ] **Flaky: `apps/web/tests-e2e/challenges/list-load-error.spec.ts:43`** ("Retry re-fetches and clears the error on a healthy response") fails ~50 % of runs — its `page.route` handler arms `failNext` once, but `/challenges` issues more than one `GET /rest/v1/challenges` (My challenges + Browse), so which request receives the simulated 500 is a race. Scope the interceptor to the My-challenges request rather than adding a retry.
- [ ] **`tests-e2e/dashboard/load-ramp.spec.ts:111` passes vacuously.** Its "no card" assertion would also pass on a page that never loaded. The `dash-total-runs` testid added on `/dashboard` in this batch exists to anchor it; the comeback spec already uses that pattern.
- [ ] **`RouteMarkerEditor.detailLine` drops a clock-only target.** It renders the target chip under `if (target?.elapsedS != null)`, so a marker saved with `target_clock` and no `target_elapsed_s` shows no chip in the marker list — even though the editor can author that form and the roadbook now resolves it correctly ([decisions § 614](../architecture/decisions.md)). A one-line fix plus a test.
- [ ] **Playwright reuses a stale dev server.** `playwright.config.ts` sets `reuseExistingServer: !CI`, so a long-lived `vite` process on `:7777` from an older checkout serves a bundle that predates a new component — a new-surface spec then fails with no console error while every pre-existing spec passes. Two rounds in this batch lost cycles to it. Worth a harness guard that asserts the served build matches the working tree, and a line in `docs/testing/testing.md`.
...
- [ ] **Mobile: the club event listings still advertise a cancelled occurrence.** `SocialService._enrichEvents` and `SocialService.fetchNextRsvpedEvent` both resolve an event's next instance with a bare `nextInstanceAfter` and never consult `event_exceptions`, so the mobile club Events tab and the Run-tab `widgets/upcoming_event_card.dart` keep pointing at an occurrence the organiser called off — the leak web closed in [§ 620](../architecture/decisions.md). Mobile already has the primitive (`fetchCancelledInstances`, used today only by `event_detail_screen.dart`), so the fix is to feed it into both fetchers. If the Dart side takes the shaping helpers too, `social/event_occurrence.ts` becomes a registered parity pair.
