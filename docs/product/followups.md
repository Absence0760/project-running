...
- [ ] **`weight_unit` (kg/lbs) display/entry converter + toggle** (small, cross-cutting; web-first then mobile twin) — the `weight_unit` user-pref key is **registered but not consumed** (settings.md, F19; placement followed the F4 rule). Storage is canonical kg (`gym_sets.weight_kg`, and the future `body_metrics`), so this is display + input-parse only: a unit-aware formatter/parser (mirror the `preferred_unit` km/mi split in `format/units.svelte.ts` ↔ mobile `Preferences`), the Settings toggle (web Preferences + mobile twin), and wiring it through every weight render/entry — today the gym surfaces (`/gym`, `GymEditor`, `gym_compose_sheet.dart`) hardcode kg. Best landed with G6/G7 (nutrition body-weight entry) so body + lift weights pick up one converter. Tests in the same commit (round-trip kg↔lbs both sides).
...
- [ ] **Apple-Watch WCSession ingest must forward `last_modified_at` (+ future `steps`/`laps`)** — the watch now stamps `last_modified_at` on the run metadata it hands off (R3 WATCHOS H1, `apps/watch_ios/`), but the phone-side ingest drops it: `apps/mobile_ios/ios/Runner/WatchIngestBridge.swift` forwards only a fixed key list, and `WatchIngest._runFromArgs` in `apps/mobile_android/lib/main.dart` rebuilds `Run.metadata` from `avg_bpm` + `activity_type` only. Until both carry the key through, a WCSession (paired-phone) watch run is invisible to the phone's delta-fetch (`metadata->>'last_modified_at' > since`) after the first full pull — it never resurfaces on another device. Fix: add `last_modified_at` to the bridge's forwarded keys and to the `_runFromArgs` metadata map (byte-identical iOS twin via the mobile-twin-mirror agent). The DEBUG-direct watch upload (`SupabaseService.swift`) already writes the key straight to the row, so this only affects the Release phone-proxy path. See [docs/backend/metadata.md § Known issues](../backend/metadata.md#known-issues). Small, well-scoped — left out of the R3 watch session because those files are outside `apps/watch_ios/`.
- [ ] **`OfflineSyncStore<S,R>` base over gear / gym / food (pure refactor — no findings outstanding)** — the three per-row offline stores (`local_gear_store.dart`, `local_gym_store.dart`, `local_food_store.dart`) share a near-identical per-row sync-state machine (`pendingCreate/Update/Delete`, one JSON file per row, client-minted UUID, `_persist` / `_dropRow` / `_markSynced` / `_rewriteAll` / `_loadAll` / `_migrateRecord` / `_newUuid`, `replaceFromServer` newer-wins, `syncWithServer` drain). Extracting a generic base would de-duplicate ~150 lines × 3. **This is structural only — every divergence finding it was meant to close is already fixed + tested + committed** (gym/food `replaceFromServer` newer-wins = L-FILE H4; gear `createLocal` clock = M2; gear `_markSynced` clock = L3; the `_v` schema stamp = L-FILE H1; the `GymWorkout`/`FoodEntry` DTOs). Scope when picked up: a base in `apps/mobile_android/lib/offline_sync_store.dart` (mirrored to iOS) generic over the stored entry `S` (a `SyncEntry` interface: `id`/`syncState`/`lastModifiedAt`/`isTombstone`/`toJson`) and the server-row type `R`; a single shared `SyncState` enum with `typedef GearSyncState = SyncState;` (etc.) aliases so the ~5 store-test files + gym screens don't churn; abstract surface `decode` / `serverId` / `fromServer` / `serverLastModified` (null ⇒ no newer-wins, the gear case) / `withState` / `push` (returns drop-vs-mark-synced). Run/route stay separate by design — they use a sidecar sync-state model, not a per-row clock (see [decisions.md § 122](../architecture/decisions.md) amendment). Behaviour-preserving: the ~57 store tests are the guard. Best done as its own focused commit, not bolted onto another change.
