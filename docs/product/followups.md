...
- [ ] **Cross-modality mirror: dynamic TDEE + gym-aware readiness** — shipped on web 2026-06-08 (decisions §134). The pure `exercise_calories` math has a Dart twin already (`apps/mobile_android/lib/exercise_calories.dart` + iOS), but three mobile *surfaces* still need wiring to reach parity: (1) `nutrition_screen.dart` should feed today's runs + gym sessions through `exerciseCaloriesForDay` into `loadNutritionTargets` and show the `base + exercise` breakdown; (2) the mobile dashboard should show the gym-in-fatigue note; (3) a mobile Settings → Preferences toggle for `exclude_gym_from_readiness` (read it in the mobile training-load wiring to drop lifts from the curve). All web-first per §24.
...
- [ ] **Mobile mirror: per-exercise records + progression surfaces** — shipped on web 2026-06-08 as two linked web-only surfaces over `LocalGymStore`'s set history: `/gym/records` (`apps/web/src/lib/gym/exercise_records.ts`) rolls up each weighted exercise's current bests (est. 1RM / heaviest set / top volume) + last-performed date + distinct-session count; each card links to `/gym/exercise?name=` (`apps/web/src/lib/gym/exercise_history.ts`) — that exercise's progression over time: headline est.-1RM delta vs the first session + a session-by-session list with a new-e1RM PR badge per session. The web gym **detail** screen (`/gym/[id]`) also shows a per-exercise **"vs last time"** hint via `exercise_history.ts#previousExerciseSession` (previous weighted session's top set + a +/- delta on the heaviest set). All reuse `computeExercisePrs` / the other `gym_prs` primitives so the numbers can't drift from the per-workout PR badges. Mobile has the PR engine (`gym_prs.dart`) and the gym list/detail screens but none of these surfaces. To reach parity: port `exercise_records.ts` → `exercise_records.dart` **and** `exercise_history.ts` → `exercise_history.dart` (both would become TS↔Dart parity pairs — register them in `CLAUDE.md` + `shared-library-syncer.md` when ported, and `exercise_history.dart` carries `previousExerciseSession` for the detail-screen hint too), drive them from `LocalGymStore`, add a records screen (reachable from `gym_screen.dart`) drilling into a per-exercise progression screen, and add the "vs last time" hint to `gym_detail_screen.dart`. Web-first per §24; no shared setting → no cross-device drift.
...
- [ ] **Web i18n has no plural support** (surfaced 2026-06-04) — the web runtime's `m(key, params)` does plain `interpolate` substitution, **no ICU plurals**, so count strings render the bare template: `history.setCount` = `"{n} sets"` → "1 sets", `gym.exercisesShort` = `"{count} exercises"` → "1 exercises", etc. Mobile (gen-l10n ARB) *does* pluralize these (`{n, plural, one{…} other{…}}`), so it's a web↔mobile inconsistency, not just a web nit. The fix is app-wide — teach `interpolate` (or `m`) an ICU-plural form and add `one`/`other` variants to the affected keys across all six locales — not a per-call-site special case (don't add a one-off `setCountOne` key; that just trades one inconsistency for another). Low priority (grammar only; counts of 1 are uncommon for sets/exercises in real data).
