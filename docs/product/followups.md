...
- [ ] **Gear wear status on the mobile gear screen** — web shipped 2026-06-08: `gear_wear.ts` classifies rolled-up distance vs the replacement target (untracked/ok/due/worn) and the gear card recolours the bar + shows a Replace-soon / Past-replacement badge. Mobile `gear_screen.dart` has its own inline `_progress` calc and no wear signal. Mirror: port the classification to a `gear_wear.dart` twin (register the parity pair), colour the `LinearProgressIndicator`, and add the badge via two new ARB keys. Pure derivation (no shared setting), so no cross-device drift — purely additive on mobile.
