...
- [ ] **Neither web nor mobile surfaces a truncated export.** The endpoint returns `complete` and the manifest names the short sections, but the client shows only "your export is ready". A runner past 5000 runs has no way to know their archive is short without opening `manifest.json`.
