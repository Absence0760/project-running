...
| Star routes for the watch picker (`is_starred`) | ✓ | ✓ | ✓ | ✓ (read-only) | ✗ | Owner-curated flag drives the Wear OS route picker. Toggle from web (`/routes` cards + `/routes/[id]` header), mobile (`routes_screen.dart` trailing star, `route_detail_screen.dart` AppBar star). Watch fetches `is_starred=eq.true&order=updated_at.desc&limit=30`; when that returns empty (first-launch / un-curated) it falls back to the 10 most-recently-updated owned routes so the picker isn't empty. No toggle on the wrist. Schema: `routes.is_starred boolean` + partial index `idx_routes_user_starred (user_id, updated_at desc) WHERE is_starred`. See `decisions.md § 44`. Apple Watch picker is still scaffolded only. |
...
| Auto-link unmatched run to a saved route | ✗ | ✗ | ✓ | N/A | N/A | Web: when a run isn't linked to a route but its track overlaps one of the user's saved routes (closest-endpoints + length-similarity check against the `routes_intersecting_track` RPC), `/runs/[id]` surfaces a one-tap "Link to *X*?" banner. Backed by the `routes.geom` GIST index from migration `20260607_001`. Mobile read path not yet wired; watches don't browse run history. |
...
| Pro "Get Pro" checkout UI | Partial | ✗ | ✓ | N/A | N/A | Shipped on web: `/settings/upgrade` "Get Pro" button now calls the RevenueCat web SDK when `PUBLIC_REVENUECAT_WEB_API_KEY` is set; unconfigured builds fall back to the placeholder toast. Mobile purchase flow still pending. Android: covered by the "Manage subscription" tile in Settings → Account (opens the same `/settings/upgrade` URL via `url_launcher` in the system browser). Native RevenueCat purchase sheet is still TBD — the plumbing already exists in the project but the in-app checkout UI is deferred. |
...
| BillingIssueBanner — payment-failed grace-period warning | ✗ | ✗ | ✓ | N/A | N/A | When RevenueCat fires `BILLING_ISSUE` (a renewal payment failed but the entitlement is still active during the store's grace period — ~16 days App Store, up to 30 days Play), the `revenuecat-webhook` EF stamps `user_profiles.billing_issue_at = now()` (decoupled from `subscription_tier` so we don't strip Pro mid-grace). Web mounts a persistent banner on the root layout — "Your Pro renewal payment failed N days ago. Update your card before your grace period ends or you'll be downgraded to Free" — with a CTA to `/settings/upgrade`. Cleared automatically when RC fires `RENEWAL` / `UNCANCELLATION` (recovered) or `EXPIRATION` / `CANCELLATION` (access ended). 🔸 **Web-only by design for v1**: App Store + Play Store both surface their own grace-period notifications via OS-level alerts + email + the system Subscriptions UI, so a mobile-only Pro user is not silent — they're surfaced through the platform layer instead of the app layer. The column data is already on the row + Dart-typed (`billingIssueAt: DateTime?` in `db_rows.dart`), so a future mobile mirror is a one-widget addition (~half-day) if telemetry shows OS notifications aren't enough. Re-evaluate once we have data on how often a mobile-only user actually sees the OS notification before grace exhaustion; today the OS layer is treated as sufficient. Pinned by `cross-cutting/billing-issue-banner.spec.ts` (4 e2e tests covering visibility, copy, and the `lock_subscription_columns` defence-in-depth that prevents client-side suppression of the banner). See [paywall.md § Pro perks](paywall.md#pro-perks-and-where-theyre-enforced). |
