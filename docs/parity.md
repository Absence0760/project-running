...
| Daily usage cap (10 / day) | ✓ | ✗ | ✓ | N/A | N/A | Server-side cap; client mirrors the count via `get_coach_usage` RPC + 429 fallback. Android shows a "$N messages left today" banner when remaining ≤ 3 and disables the composer when reached. |
...
| Unlimited AI Coach for Pro users | ✓ | N/A | ✓ | N/A | N/A | `/api/coach/+server.ts` skips the 10/day cap when `is_pro()` returns true. The same endpoint serves the Android Coach screen, so the cap-removal is automatic on both. |
