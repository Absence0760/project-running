...
| Find other runners (search + suggested) | ✗ | ✗ | ✓ | N/A | N/A | Web-only today (web is the canonical surface per decisions §24). The **People** tab under `/social` provides a name-search (ILIKE on `user_profiles.display_name`, self-excluded, signed-in only) plus a Suggested-for-you list (members of viewer's clubs they don't already follow, ranked by shared-club count). Inline Follow toggle with optimistic flip. See `decisions.md § 61` and `data.ts#searchPeople` / `fetchSuggestedPeople`. Mobile twins not yet ported. |
