...
| Notifications inbox (kudos / comments / replies / follows) | ✗ | ✗ | ✓ | N/A | N/A | `notifications` table populated by SECURITY DEFINER triggers on `run_kudos`, `run_comments`, and `user_follows`. Sidebar bell with unread badge + popover; `/notifications` standalone full-list view with all / unread tabs. Refresh on auth-ready and on window focus (no polling). See `decisions.md § 38`. |
