...
| Lap splits table | ✓ | ✗ | ✓ | ✗ | ✗ | |
| Heart-rate zone breakdown | ✗ | ✗ | Partial | ✗ | ✗ | Web's `/runs/[id]` now computes a real zone distribution from per-point `track[].bpm` samples when present, with user-defined `hr_zones` cutoffs pulled from the settings bag (falls back to 60/70/80/90/100 % of HR-max when unset). When no per-point samples are present, the panel renders an honest "No HR samples on this run" fallback (with scalar `metadata.avg_bpm` shown when available). Still `Partial` because **no writer populates `bpm` yet** — Android BLE recorder, watchOS, and Wear OS need to start emitting per-point BPM into `track.json.gz` alongside `lat`/`lng`/`ele`/`ts`. 🔸 gap vs web-canonical — mobile/watch recorders to follow in separate sessions. |
