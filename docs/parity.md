...
| Star routes for the watch picker (`is_starred`) | ✓ | ✓ | ✓ | ✓ (read-only) | ✗ | Owner-curated flag drives the Wear OS route picker. Toggle from web (`/routes` cards + `/routes/[id]` header), mobile (`routes_screen.dart` trailing star, `route_detail_screen.dart` AppBar star). Watch fetches `is_starred=eq.true&order=updated_at.desc&limit=30` only — no toggle on the wrist. Schema: `routes.is_starred boolean` + partial index `idx_routes_user_starred (user_id, updated_at desc) WHERE is_starred`. See `decisions.md § 44`. Apple Watch picker is still scaffolded only. |
