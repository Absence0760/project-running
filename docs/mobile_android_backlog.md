...
[`docs/parity.md`](parity.md) is the single source of truth for *which* features are missing — every cell flips there when work lands. This doc is the **execution order**: what to build, in what sequence, and why structural primitives need to land before the screens that consume them.
...
2. **`api_client` typed methods** — `packages/api_client/lib/src/api_client.dart` currently exposes 26 methods covering auth + runs + routes + route reviews. Every other gap feature needs methods added here first. This is mechanical porting from `apps/web/src/lib/data.ts`.
...
- **Order**: this doc.
- **Why**: [`decisions.md § 24`](decisions.md#24-web-is-the-canonical-feature-surface-mobile-and-watches-are-platform-additive).
