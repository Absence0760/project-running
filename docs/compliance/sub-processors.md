...
| **Fly.io** (job_worker + OSRM + GraphHopper) | Map-match jobs (run tracks transit through), strava-webhook payloads, data-export job state, route-loop generation requests (start lat/lng + target distance — see the self-hosted-engines note below) | `lhr` (London) — per `apps/job_worker/fly.toml` + `osrm/fly.toml` `primary_region` | Contract | <https://fly.io/legal/dpa/> | None practical (server-side only) |
...
**Self-hosted routing engines (OSRM + GraphHopper) are NOT new third-party sub-processors.** Both run on **our own Fly.io infrastructure** — the only sub-processor in the chain is Fly.io (the hosting provider, already a row above), not the engine projects themselves. OSRM serves map-matching (`apps/job_worker/`) + manual-waypoint snapping; **GraphHopper** (added 2026-06-09, decisions §137) serves distance-targeted loop generation (`/api/routes/generate`). The user's start coordinates reach GraphHopper only *inside* our infra — `GRAPHHOPPER_URL` is a server-only env (never `PUBLIC_`), so the browser never calls the engine directly and no coordinate egresses to a third party (same posture as the self-hosted OSRM map-matcher). No GraphHopper / OSRM DPA exists or is required — there is no third party processing the data; we are. Logged here for completeness, not because a new processor relationship was created.
