...
| **Art 20 export artifacts** (Storage `exports/{user_id}/exports/...`; legacy `runs/{user_id}/exports/...`) | Storage | **Reachability**: 7 days from creation. **Bytes**: account deletion only | `cleanup-stale-export-blobs` cron removes the `storage.objects` row (widened to the new bucket by `20270602_001_exports_storage_bucket.sql`); `delete-account` walks both buckets through the Storage API and is what actually erases | **These are two different guarantees and this row used to state only the stronger one.** Measured ([decisions § 1049](../architecture/decisions.md), re-measured 2026-09-03): a probe uploaded through the Storage API, aged past the window and swept by the shipped `cleanup_stale_export_blobs()`, loses its row and stops being listable — and its bytes stay on the backend with a matching `sha256`. Deleting the same object through the **Storage API** does remove the backend file (measured on the same bucket in the same session: 1 file → 0), which is the path `delete-account` uses, so the deletion leg is a real erasure and the nightly leg is not. The signed URL is 10 minutes, so the sweep does bound who can reach the archive; it is not an Art 17 erasure and nothing may call it one. Durable fix — move the sweep onto the Storage API as a Go-worker job kind — is **half built and inert** ([decisions § 1112](../architecture/decisions.md)): `export_blob_reap` lists the bucket and deletes through the API, but `jobs_kind_chk` forbids the kind so nothing enqueues it and `Worker.dispatch` carries no case. Until the CHECK, the enqueue function, the `cron.schedule` and the dispatch case land together, the **Bytes** column above is unchanged. Separately, the bytes ALREADY orphaned by past sweeps are invisible to that reaper by construction — it lists `storage.objects`, and those rows are gone — so they need an S3 lifecycle rule or an operator pass, filed |
