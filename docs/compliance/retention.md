...
| **Art 20 export artifacts** (Storage `exports/{user_id}/exports/...`; legacy `runs/{user_id}/exports/...`) | Storage | 7 days from creation, OR account deletion | `cleanup-stale-export-blobs` cron (widened to the new bucket by `20270602_001_exports_storage_bucket.sql`) + `delete-account` walks both buckets | The signed URL is 10 minutes, so nothing legitimately needs the blob past that — a subject re-runs the export. The blob holds a COMPLETE copy of the subject's history, so the sweep is the load-bearing control: leaving it scoped to the old bucket would have retained one forever |
...
| `cleanup-stale-export-blobs` | 04:23 UTC daily | data-export blobs older than 7 days, in the `exports` bucket **and** the legacy `runs/{user_id}/exports/` prefix | `20260720_001_cleanup_stale_exports.sql`, widened by `20270602_001_exports_storage_bucket.sql` |
