...
| **Live spectator pings** (`live_run_pings`) | Postgres | 24 hours | `cleanup_stale_live_run_pings()` cron (`20260602_001_pg_cron_schedules.sql`) | The Go live-hub path uses Redis TTL of the same window |
...
| **Coach usage counter** (`user_coach_usage`) | Postgres | Rolling daily window; rows can stay since they're already per-day counters | `delete-account` cascade | Pseudonymous per-user daily counter; small enough that a longer tail doesn't hurt |
...
| **Direct messages** (`direct_messages`) | Postgres | Until either party deletes their account | ON DELETE CASCADE on `sender_id` + `recipient_id` | User correspondence; both sides' FK cascade so the row vanishes when EITHER participant is deleted. No separate time-window today — a fixed conversation-retention window is a possible future tightening |
...
| `cleanup_stale_live_run_pings()` | hourly | `live_run_pings` older than 24 h | `20260602_001_pg_cron_schedules.sql` |
...
All six jobs are live. Window tightening is a single-file edit to the
function body. The `gdpr_dsar_closeouts_test.sql` pgtap suite pins the
existence of `purge-stale-jobs`; the matching pins for the others
ride alongside the 20260922_001 migration.
