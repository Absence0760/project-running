...
|---|---|---|---|---|---|
| 1 | **Training plan runner** — [x] web: schema + generator + editor + dashboard card + auto-match; [x] Android: engine port + plans list + create wizard + plan/workout detail + today's-workout card on Run tab idle; [ ] live structured-workout execution loop (interval state machine in run recorder) | 6–8 wk (web + Android shipped, execution loop left) | Runna, Garmin | `training_plans`, `plan_weeks`, `plan_workouts` (shipped) | Live execution wiring — reuse existing audio-cue layer or add an intervals overlay? |
