...
- [ ] **Phase 2 — recurrence + invites:** Enum recurrence (`weekly` / `biweekly` / `monthly` + `byday[]` + `until_date`), invite-only and request-to-join clubs, shareable invite links, threaded replies on posts.
- [ ] **Phase 3 — Android mirror:** Clubs list/detail + event detail + RSVP + admin post UI in `apps/mobile_android`. Upcoming-event card replaces the Last-Run card on the Run tab when an event is within 48h.
- [ ] **Phase 4 — realtime + push:** Supabase Realtime subscriptions on the club feed so admin updates appear without a refresh; FCM push on admin updates and event-day reminders for RSVP'd members.
