...
- [ ] Wire up sync path:
  ...
  - [ ] Return the matched geometry to the client for display — Go worker (`apps/job_worker/`) computes + uploads a `.matched.json.gz` blob and PATCHes `run_matched_tracks.status='matched'`. Default matcher is `PassthroughMatcher` (visually identical to raw); set `OSRM_URL` to switch the worker over to `OSRMMatcher` against the local OSRM stack at `apps/job_worker/osrm/`. **Web read path is wired** (`apps/web/src/lib/data.ts:fetchRunMatchedTrack` + `/runs/[id]` swaps `RunMap.track` to the matched line when present, surfaces `pending` / `failed` / `skipped` via a corner pill); mobile read path still pending.
- [ ] Client display:
