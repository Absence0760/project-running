...
- [ ] **Server-side backup via `/v1/export?format=backup`.** The Go data-export service (`apps/job_worker/internal/dataexport/server.go`) supports `csv` + `gpx` today. Adding a `format=backup` mode (manifest + runs.json + routes.json + profile.json + tracks/{id}.json.gz, same shape `BackupService.restore` reads) would let mobile offload bandwidth + battery for typical users. **Blocker:** the current `MaxRunsPerExport = 5000` cap is below what the new local streaming path handles, so the server path is currently a *downgrade* for power users. Need to lift the cap (via paginated runs select + streaming Storage response) before this becomes a net win. See `decisions.md § 66`.
- [ ] **Web backup streams to disk.** `apps/web/src/lib/backup.ts` still buffers the entire archive in memory (`JSZip` + `generateAsync({ type: 'blob' })`); same shape the mobile path had before the May 2026 refactor. Switch to `BlobWriter` + per-track stream-add so the browser tab survives a 5 000-run backup without an OOM crash.
...
| 4 | **Heatmap / popular-route discovery** ([x] v1 shipped — public-routes-only heatmap via PostGIS `heatmap_points_in_bbox` RPC on top of densified `routes.geom`; web overlay on `/routes?tab=heatmap` with a MapLibre `heatmap` paint layer; refreshes on map moveend with 350 ms debounce, 5k point cap. Privacy: route opt-in already gates inclusion via `routes.is_public`. Mobile read-path deferred to v2.) | 2 wk | Strava, Komoot | Migration `20260828_001_heatmap_points.sql` | Resolved: opt-in (via existing `routes.is_public`) — no separate user-level toggle |
