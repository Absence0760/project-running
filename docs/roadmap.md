...
- [x] **Notifications inbox** — `notifications (user_id, actor_id, kind, run_id, comment_id, read_at, …)` populated by SECURITY DEFINER triggers on `run_kudos`, `run_comments`, `user_follows` so kudos / comments / replies / follows are written in the same transaction as the source. Sidebar bell with unread badge + popover; `/notifications` standalone page with all / unread tabs. Refresh on auth-ready and on window focus (no polling). v2: batch / collapse ("Alice and 4 others gave kudos…"), realtime push, mobile fan-out worker. Spec: `decisions.md § 38`.
