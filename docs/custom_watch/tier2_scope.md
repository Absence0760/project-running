...
| **Course climb-profile graph** | **S** | — | The gap between the two elevation surfaces that shipped: the run's own sparkline (`Page::ElevationProfile`) and the pushed course's numbers (`Page::RouteElev`). `MiniProfile` + `route_elevation` both exist, so this is wiring plus a position marker on the profile. The cheapest genuinely-missing nav feature in the list. |
...
4. **The cheap wins that were waiting on P1/P2** — pace/distance/time alerts, the course climb-profile graph, HR broadcast, compass, metronome, rolling load, the `guided_runs` page. Roughly one **M** in total, and it is the batch that makes the device feel finished.
