...
| **Structured / interval workout execution** — **LANDED 2026-07-28** | **M** | — | Shipped as [decisions § 354](../architecture/decisions.md): `watch_core::workout` ports the shipped mobile runner's advance math; the `WKT1` push frame this row predicted landed on a seventh characteristic, chunked like `CRS1` and checksummed from v1. The cue *events* (halfway / last-50 m / pace drift) stay with the tier-2 haptic channel. |
...
4. **The cheap wins that were waiting on P1/P2** — HR broadcast, compass, metronome. Seven items on this list turned out to need *neither* prerequisite and were pulled forward ahead of tier 2: pace/distance/time alerts ([§ 332](../architecture/decisions.md)), the `guided_runs` page ([§ 333](../architecture/decisions.md)), the course climb-profile graph ([§ 334](../architecture/decisions.md)), #607's required-pace port (all 2026-07-26), the rolling training load + TRIMP upgrade ([§ 352](../architecture/decisions.md)), activity profiles ([§ 353](../architecture/decisions.md)), and structured workout execution ([§ 354](../architecture/decisions.md), all three 2026-07-28). What is left in this phase genuinely waits on the silicon or the BOM. It remains the batch that makes the device feel finished.
