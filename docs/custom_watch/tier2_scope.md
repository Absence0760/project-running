...
| **Daylight remaining / sunset countdown** | **S** | — | Pure computation off state already on-device: latitude from the fix, day-of-year, the `SET1` v2 timezone offset; the seasonal model is a port of the web `safety_nudge.ts` seasonal half (`isSunDown`). One glance page. Pull-forward candidate like the 2026-07-26 batch. |
...
| **Waypoint marking / save location** | **S** | — | The geometry is `trackback`'s distance-and-bearing against a stored point; the real work is interaction (no spare key in the § 350 grammar — a § 351 settings-menu action is the candidate) plus persisting marked points in flash so a reboot keeps them. |
...
| **ICE / medical-ID idle screen** | **S** | — | A `SET1` extension carrying a few phone-synced strings (contact, blood type, allergies) plus one idle-face surface a medic can find. Fail-closed like every settings field: absent until synced, never defaulted. |
