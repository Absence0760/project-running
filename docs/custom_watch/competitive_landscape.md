...
**Vector 1b: Build a Zepp OS app for existing Amazfit owners.** Same play as Vector 1, different ecosystem. Amazfit watches run Zepp OS, which has its own third-party SDK (the `Zeus CLI`, "Mini Programs" + watch faces, a Zepp App Store, sideload via dev mode) and supports current SKUs like the T-Rex 3. Cost is weeks, risk is zero, distribution is their store. **The hard limit:** the SDK is sandboxed — you get data screens, metrics, coaching nudges, watch faces, but *not* the GPS pipeline, power management, or the maps / navigation engine. So the one thing Amazfit is weakest on (offline routing / map UX) is the one thing a Zepp OS app can't fix — you'd be polishing around their core, not replacing it. Verify the SDK's live-data access against `docs.zepp.com` before betting on it.
...
**Vector 3: White-label or firmware-partner with a mid-tier ODM.** Mobvoi, Amazfit/Zepp, Polar's lower SKUs. Risk is medium, cost is significant but no tooling capital. You ship your software on their hardware, brand it as yours. **Zepp Health (formerly Huami) is the standout candidate because it already *is* an ODM** — it built Xiaomi's Mi Band line and put Timex-branded smartwatches on its own hardware + app-design stack (the Huami–Timex deal), so "they manufacture, you brand + bring the software" is a business they already run. Polar is the other option: decent hardware, openly-criticised software — the COROS-of-2018 playbook. The trades: you're tied to their hardware roadmap; you negotiate firmware-level access (a skinned Zepp OS is the easy deal, owning maps / GPS / power is the hard ask); MOQs run to thousands of units with upfront NRE (the Timex precedent is a big brand, not a hobby-volume solo dev); and — critically — **riding an AMOLED SKU inherits its ~38hr dual-band battery ceiling, trading away the multi-day-MIP advantage that is our only structural edge in the ultra niche.** If multi-day battery is the wedge, the ODM ask narrows to "build us an MIP SKU"; if the wedge is software for the sub-day runner, any of their AMOLED watches will do.
