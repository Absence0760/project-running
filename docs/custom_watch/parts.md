...
0. **Moving SW9 to Li-Po drops the whole board to 1.8 V, and that is the configuration the DoD run needs.** This one is not a subtlety about one pin — it takes every breakout below its minimum supply at once, and Nordic's own hardware user guide is explicit about why. SW9 selects the SoC's supply between **VDD** (the default), **Li-Po** and **USB**. In the VDD position the SoC runs from the DK's onboard 3 V buck in normal mode and the board rail is 3.0 V, which is why everything works on the bench. In the **Li-Po** position — and in the USB position too — the SoC's *high-voltage* regulator is used, so `VDD_nRF` is whatever `UICR.REGOUT0` says, and that is **1.8 V on an erased UICR**. The DK then follows: "to make sure the rest of the DK has the same voltage level, the VDD of the DK is sourced by a regulator following the `VDD_nRF` voltage when the high voltage regulator is used" (nRF52840 DK HW user guide § 4.3.2 / § 4.4). So the header rail every breakout is powered from tracks the SoC down to 1.8 V, against a Sharp breakout wanting 3-5 V, a BMP581 breakout wanting 3-5 V, a MAX-M10S wanting 2.7 V minimum and a MAX30101 board built for 3.3 V. Nothing comes up, and nothing is damaged either — it is a clean brownout of the entire rig, which at least fails in one direction rather than mixing rails. Program `REGOUT0` to **3.0 V** once, before wiring anything: 3.0 V rather than 3.3 V because a nearly-flat 1S cell cannot hold 3.3 V through the regulator's dropout, and because it matches what the board supplies in the VDD position, so behaviour does not change with the power source. The firmware reports the state at boot (`supply: ...`, `app/src/supply.rs`) and deliberately does **not** write it: the write needs a reset to take effect, and a write that does not stick is a reset loop ([§ 626](../architecture/decisions.md)).
