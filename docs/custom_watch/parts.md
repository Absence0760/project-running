...
| **MAX30101** optical-HR breakout | SparkFun `SEN-16474` **or** Pimoroni `PIM438` **or** any generic MAX30101 module | ~$20–34 | Optical HR, decided at [§ 623](../architecture/decisions.md). **MAX30101, not MAX30102** — the 30102 is red + IR only (a fingertip SpO2 part) and the **green** LED the 30101 adds is the one every wrist sensor reads a moving capillary bed with. MAX30105 is an acceptable sibling. I²C `0x57`, 400 kHz, breakout regulates its own 1.8 V rail. Register map is **public**, so the board vendor does not matter — both brand-name boards were thin at the big distributors on 2026-08-17 (SparkFun backorder, Pimoroni 0 at Digi-Key though *Active*, stocked at Pimoroni direct) and a generic module answers the same registers. Owes a driver — see below |
...
**What the choice owes, stated honestly.** An earlier draft of this section claimed the part-agnostic work "carries over unchanged". That is true of the logic and **false of the packaging**: `peak_detect` (374 lines) and the LED AGC live inside the **`max86177` crate**, and `app/src/tasks/hr.rs` imports them from there while calling a concrete `Max86177` type, not a trait. So the order of work matters — **lift the part-agnostic half out first** (into `watch_core` or a shared `ppg` crate), then write a thin `max30101` register driver under it, and point `hr.rs` at the abstraction. The other order forks the peak detector, and two detectors is two things to tune against one wrist. § 623 also names the three concrete deltas — an 18-bit ADC ceiling instead of 19, ambient sampling through multi-LED slots instead of the MEAS1/MEAS2 tag pair, and a register-retention-across-shutdown question that is *new* against a new part rather than inherited.

**Order the rest of the list without waiting for that driver.** The firmware tolerates an absent HR sensor by design — the `hr` task's timeout-bounded presence probe parks cleanly rather than stalling the executor, which is exactly why the Renode runs work with no sensor modelled. Steps 3, 4, 6 and most of 7 bench-verify with no HR part on the bench; only the DoD run itself waits. `drivers/max86177` stays in the tree: it is the head start on the production part, not dead code.
