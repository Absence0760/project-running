...
| Optical-HR AFE | **Decision needed — see below before ordering** | $20–$191 | The `MAX86177EVSYS#` this list used to name is a two-board evaluation *system*, is discontinued at Digi-Key, **and its register map is NDA-gated**, so the part cannot be driven from public documents at all. This is the only unresolved line on the list and the only one on § 82's DoD path |
...
4. **The Maxim AFEs want a 1.8 V main supply** (3.1–5.5 V for the LED driver) against the DK's 3.0 V default VDD, so their I²C lines are a level-shifting question, not a jumper-wire one. Applies to options 2 and 3 below; a MAX30101 breakout is 3.3 V-friendly and sidesteps it.

### The optical-HR line item needs a decision, not a supplier

The one thing on this list that could stall the bench phase, and it is on the critical path: [§ 82](../architecture/decisions.md#82-tier-1-firmware-is-done-when-one-outdoor-run-syncs-end-to-end-to-supabase-from-the-bench-prototype)'s DoD requires a run recording **GPS + HR**. Three separate problems stack on the part this list has named since the beginning, and the third is the one that matters:
...
**The framing that resolves it:** the MAX86177 is a *production* pick ([`bom.md`](bom.md)) that got imported into tier 1. § 82 asks for a run that records HR — it explicitly accepts "raw photodiode reads + naive peak-detect" and puts the licensed Maxim algorithm after tier 1. It does not ask for the launch-pick AFE. Choosing a tier-1 part on its own merits is therefore in scope, not a compromise.

Three ways forward:

1. **A MAX30101 / MAX30102 breakout (~$20).** *Recommended for tier 1.* Fully public register map, I²C, stocked everywhere (SparkFun Qwiic, ProtoCentral, DFRobot, Pimoroni). Costs a new `drivers/max30101`, but that is the smaller half of the work: `peak_detect`, the LED AGC, the off-wrist and saturation contact classes, the duty-cycling schedule and the whole `hr` task seam are part-agnostic and carry over unchanged. Gets a real pulse onto a real wrist for the price of a lunch, which is what unblocks the DoD.
2. **`MAXREFDES280#` ($191, in stock).** Digi-Key's own suggested replacement, and a wrist-worn PPG band carrying the **MAX86171** — four photodiodes, eight LEDs in four packages, an accelerometer. Stays in the Maxim wrist-AFE family with a **full public datasheet including the register map**, and the band form factor suits a wearable prototype. Ten times the price, still a new driver (MAX86171 ≠ MAX86177 register map), and worth it only if closeness to the production family is worth paying for now. Note the bare `MAX86171EVSYS#` is *obsolete and no longer manufactured*, so this reference design is the live route to that chip.
3. **Pull the ADI NDA forward and source the MAX86177EVSYS direct.** The only path that keeps `drivers/max86177` *and* validates it. This is the right tier-2 move — it is how the production part eventually gets a correct driver — and the wrong way to unblock a tier-1 bench run, because it makes a working prototype wait on a vendor relationship.

**Order the rest of the list now regardless.** The firmware tolerates an absent HR sensor by design — the `hr` task's timeout-bounded presence probe parks cleanly rather than stalling the executor, which is exactly why the Renode runs work with no sensor modelled. Steps 3, 4, 6 and most of 7 bench-verify with no HR part on the bench; only the DoD run itself waits. Record whichever option wins as a decision, because it determines whether a written driver survives.
