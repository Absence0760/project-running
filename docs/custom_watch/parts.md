...
| Sharp Memory LCD 1.3" 168×144 breakout (LS013B7DH05) | Adafruit 3502 ("SHARP Memory Display Breakout — 1.3" 168x144 Monochrome") | ~$35 | The MIP display family Garmin Fenix uses. SPI interface, write-only, 3 KB framebuffer the MCU must hold. Static draw ~10 µA. **Resolution is load-bearing:** `drivers/sharp_mip` hard-codes a 168×144 framebuffer (`framebuffer.rs` `WIDTH`/`HEIGHT`), so a different panel silently corrupts every line-update address. Do **not** order LS013B4DN04 — that is the 96×96 panel (Adafruit 1393) and is obsolete/EOL |
