...
1. **Custom GATT service** — this design-era note said two characteristics: `run_manifest` (list of unsynced run IDs + sizes) and `run_chunk` (binary chunk transfer with offset). What shipped is nine on one service (`apps/custom_watch/app/src/tasks/ble.rs`): the per-second status `frame`, `run_manifest` + `run_chunk` as designed, five write-only push characteristics — `settings` (`SET1`), `course` (`CRS1`, chunked), `workout` (`WKT1`, chunked — the structured-workout step list, decisions §354), `screens` (`SCR1`, **unchunked** — the runner's composed data screens, decisions §364; the whole set is 28 bytes, so it fits one ATT write and needs no assembler) and `roadbook` (`RBK1`, chunked — the checkpoint + cut-off schedule that makes the Roadbook / CutoffEta / Fuel / SleepStation pages read live data on hardware) — plus the read-only `push_status` (`PSH1`), the one row that answers rather than accepts: an ATT write-with-response is a transport fact, so without it five characteristics could refuse a push while the phone reported success. Every one is encryption-gated (§285), and `scripts/check_watch_ble_uuids.mjs` reads this list against the firmware's own table on every PR.
