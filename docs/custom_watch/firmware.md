...
1. **Custom GATT service** — this design-era note said two characteristics: `run_manifest` (list of unsynced run IDs + sizes) and `run_chunk` (binary chunk transfer with offset). What shipped is six on one service (`apps/custom_watch/app/src/tasks/ble.rs`): the per-second status `frame`, `run_manifest` + `run_chunk` as designed, and three write-only push characteristics — `settings` (`SET1`), `course` (`CRS1`, chunked), and `workout` (`WKT1`, chunked — the structured-workout step list, decisions §354) — every one encryption-gated (§285).
