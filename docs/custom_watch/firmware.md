...
1. **Custom GATT service** — this design-era note said two characteristics: `run_manifest` (list of unsynced run IDs + sizes) and `run_chunk` (binary chunk transfer with offset). What shipped is seven on one service (`apps/custom_watch/app/src/tasks/ble.rs`): the per-second status `frame`, `run_manifest` + `run_chunk` as designed, and four write-only push characteristics — `settings` (`SET1`), `course` (`CRS1`, chunked), `workout` (`WKT1`, chunked — the structured-workout step list, decisions §354), and `screens` (`SCR1`, **unchunked** — the runner's composed data screens, decisions §364; the whole set is 28 bytes, so it fits one ATT write and needs no assembler) — every one encryption-gated (§285).
