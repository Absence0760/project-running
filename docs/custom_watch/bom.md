...
| *Alt: Apollo4 Blue Plus (AMA4B2KP-KXR)* | Previous-gen Ambiq pick. ~4 µA/MHz active, BLE 5.1. ~$8 / $4 | ~2× higher MCU energy than Apollo510B on the same workload; locks in last-gen silicon for a 2027+ product. Superseded per [§ 90](../architecture/decisions.md#90-bom-refresh-2026-05-28--apollo510b--bmp581-swap-ins-supply-alternates-qualified) |
...
| *Alt: Airoha AG3335M* | 12 nm L1+L5 dual-band, wearable-optimised power envelope. Powers some Garmin / COROS-tier watches; Quectel LC29H module is built on it (easier sourcing via Quectel). Documented alt per [§ 90](../architecture/decisions.md#90-bom-refresh-2026-05-28--apollo510b--bmp581-swap-ins-supply-alternates-qualified) | Performance close to Sony CXD5610 but with a more accessible vendor relationship if Sony NDA engagement stalls |
...
**16 GB SPI NAND flash** — Macronix MX25R series, Winbond W25Q series, or equivalent. ~$3–5 BOM at tier-3 volumes (TSSOP-8 / WSON-8 package). Per [decisions.md § 85](../architecture/decisions.md#85-map-renderer-full-pmtiles-vector-rendering-on-the-mcu--16-gb-external-nand-flash): required for the on-watch PMTiles map archives; LittleFS-formatted, partitioned for one global low-zoom tileset plus per-region high-zoom tilesets the user has explicitly downloaded.
...
| *Alt / qualified backup: Sharp LS027B7DH01* | Larger (2.7", 400×240), same MIP family. Higher current than LS013 series; useful as a qualified backup if LS013 line goes NRND post-Mobara closure (March 2026 — see [§ 90](../architecture/decisions.md#90-bom-refresh-2026-05-28--apollo510b--bmp581-swap-ins-supply-alternates-qualified)) or if we want a bigger watch face | Watch case has to grow with the larger display |
