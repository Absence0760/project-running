...
1. **Pull the ADI NDA forward for this one part.** § 88 Layer 2 already schedules ADI contact (they're "friendlier to small inquiries" per § 88); an NDA/documentation request is lighter than the HR-algorithm-licensing conversation and could be made at the same time or slightly earlier. The EV system plus NDA docs is the straight route to the launch-pick part.
...
**Procurement update, 2026-08-17.** Both routes above moved, and the sentence this section used to carry — that the MAX86177EVSYS is "orderable from DigiKey/Mouser" — is no longer true. `MAX86177EVSYS#` is *Discontinued at Digi-Key*, 0 stock, backorders refused, no lead time; `MAX86171EVSYS#` is *obsolete and no longer manufactured*. The live route to a MAX86171 is now **`MAXREFDES280#`** (~$191, stocked), a wrist-worn PPG band built around that chip. Separately, the driver written in the meantime (`drivers/max86177`) inherited exactly the gap this section predicted: its register map is family-modelled, not datasheet-derived, and the driver now says so in its own doc comment. The full three-option decision, including the cheap MAX30101-class route that § 82's "raw photodiode reads + naive peak-detect" bar actually permits, lives in [`parts.md`](parts.md) — that is the file to read before ordering.

The follow-through this section asked for — an NDA note on `roadmap.md` step 5 and the `bom.md` HR row — was **not** done at the time and is done as of the same date.
