...
- [ ] **The settings menu, one-handed (§351).** Idle BTN5 opens it; BTN3/BTN4 step the cursor the directions they page; BTN1 cycles GNSS MODE in place with the projected hours readable first; HIDE EMPTY toggles and **survives a power cycle** (CFG1); RE-ZERO fires and closes with the idle banner answering; BTN2 exits; 30 s of no presses closes it back to the clock; and no press inside it can start, pause, or lap a run.
