...
- [ ] **Settings push applies over the radio.** A v2 `SET1` frame sets max HR / pacer goal / fuel intervals / QNH / timezone / page mask; a bad-flags frame is **rejected** (fail-closed) rather than partially applied. Confirm the timezone push flips the home clock hero `UTC` → `LOCAL`.
...
- [ ] **The filtered page cycle is honest.** An unsynced watch cycles only data-present pages (roughly 8–12, not 32 empty-state stops); every synced-only page reads `NOT SYNCED` rather than zeros; the page-dot indicator counts the filtered set.
...
- [ ] **Exercise a pipelined BLE chunk pull.** The chunk-request path is now a depth-8 queue rather than a single-value signal, so two requests in flight can no longer silently drop the first. Pass criterion: a phone that issues several `run_chunk` requests without waiting receives an answer to every one, and a deliberate overflow past the queue depth produces the `ble: chunk queue full` warning rather than a silent stall. Cannot be sim-verified (no SoftDevice under Renode) and has no host coverage today, so the bench is its first real test.
...
- [ ] **Verify the settings frame's integrity fix once it lands.** `WatchSettings::decode` currently has no checksum, and a single-byte corruption can re-frame a push (a QNH pressure applied as a page mask). Pass criterion: after the planned `SET1` v3 CRC32 bump, a deliberately corrupted frame is rejected on-device and the `#[ignore]`d property in `core/tests/prop_settings.rs` passes.
