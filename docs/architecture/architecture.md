...
| Web icons | Self-hosted Material Symbols webfont, rendered as ligatures | Subset to the 347 icons the tree names by `scripts/gen_web_icon_font.mjs` — see [decisions.md § 779](decisions.md). `unplugin-icons` + `@iconify-json/material-symbols` are configured in `vite.config.ts` but no `~icons/` import exists; no rendered icon comes from them. |
