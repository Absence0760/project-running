...
New product work starts on web. Drift in the mobile → web direction (Android has X that web doesn't) is closed by building the web version; drift in the web → mobile direction (web has X that mobile doesn't) is closed by porting down. The [parity matrix](../product/parity.md) is the living checklist for both directions.
...
See [run_recording.md](../features/run_recording.md) for the full subsystem reference — state machine, filter chain, auto-pause gates, crash-safe persistence, background-recording requirements, and tunable constants.
...
High-level sequence on the phone. The full detail — filter chain, auto-pause gates, hardening timers, crash recovery — lives in [run_recording.md](../features/run_recording.md#data-flow-recording-a-run).
...
`.github/workflows/ci.yml` runs **every job it defines** on every PR + push to `main` (no PR-only vs. push-only split). The total is deliberately not asserted in prose here — the two that were (“seventeen” here, “nineteen” in [testing.md](../testing/testing.md)) were both stale, and disagreed with each other as well as with the file. Recompute it with `awk '/^jobs:/{j=1;next} j&&/^[^ ]/{j=0} j&&/^  [A-Za-z0-9_-]+:$/{n++} END{print n}' .github/workflows/ci.yml` (35 on 2026-09-07, 34 of them in the gate's `needs:`). The principal ones, and the only list here — `ci.yml` is the source of truth and this enumeration has gone stale twice: `test-packages`, `test-worker`, `test-graph-cycle`, `parity-types`, `build-web`, `parity-matrix`, `build-watch-wear`, `build-firmware`, `build-mobile-android`, `twin-parity`, `schema-codegen-drift`, `api-client-integration`, `edge-functions`, `pgtap-rls`, `e2e-web` (Playwright sharded 14-way), `e2e-web-livehub`, and `e2e-web-sso`. The per-job breakdown lives in [monorepo.md § CI/CD](monorepo.md#cicd); `ci.yml` itself is the source of truth. Branch protection requires a single status check — the **`CI gate`** aggregator job, which `needs:` every other job in the file. That is not a convention a new job has to remember: `scripts/check_ci_diagnostics.mjs` (in `workflow-lint`) fails the PR when a job is in no `needs:` entry of the gate, because a job the gate never waited on counts as no result rather than as a red; `ci.yml` always runs, so it is the only emitter of that context; on a docs-only diff its `changes` filter job skips the heavy jobs and the aggregator still reports green.
...
Web app deploys to AWS via `.github/workflows/release-web.yml`, which uses GitHub OIDC to assume an IAM role and runs `aws s3 sync` + `aws lambda update-function-code` + `aws cloudfront create-invalidation` against the `prod` environment when a `web@*` GitHub Release is published (a bare tag push does not deploy — the published Release is the gate, consistent with every other `release-*.yml`). Pushes to `main` deploy to the `preview` environment at `preview.threkir.com`. See [`apps/web/deployment.md`](../../apps/web/deployment.md).
