...
## 277. Changing a password needs proof of the current one; an account that has none proves possession of the mailbox instead
...
## 278. A signup confirmation that lands off `/auth/callback` fails closed in code — a dashboard setting must not be able to drop the Art 8 consent stamp
