...
## 178. Logged-set `set_type` is a real CHECK-constrained column reusing the routine vocabulary, defaulting to `working` — not a metadata key

**Decided (2026-06-20, migration `20270224_001`).** The "RPE / set-type metadata" Gym-mid item is modelled as a first-class column on `gym_sets`, not a `metadata.md` jsonb key:
