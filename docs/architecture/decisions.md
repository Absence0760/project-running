...
## 283. The watch's BTN3 cycle is filtered — data-present ∩ curated, Dashboard forced — superseding §225's fixed 32-page walk
...
## 284. Run-store v2: byte 15 becomes a record tag (laps interleave with points) and a full slot decimates instead of truncating; the Threkir GATT service is encrypted-only with one persisted bond
...
## 285. The watch page cycle is frequency-ordered in themed clusters, with Back-to-start anchoring the wrap

**Decided (2026-07-21, issue #596 follow-up).** §283's filter fixed how MANY pages the BTN3 cycle walks; this fixes their ORDER, which until now was accretion order (each page sat wherever its batch appended it), leaving mid-race pages like Roadbook / Fuel / Nav 10–14 presses out while rarely-mid-run synced summaries sat close. Because the cycle is bidirectional (short press forward, long press back), a page's cost is its distance from the Dashboard in EITHER direction, so the declaration/cycle order is now: the live-run cluster (Dashboard → Distance → Pace → Lap → Zones → Splits → Pacer), the course/race-ops cluster (Nav → TurnCue → CutoffEta → Roadbook → Fuel), effort analysis (ElevationProfile → RacePredictor → TrainingLoad → DistanceBand), synced training (GearWear → TrainingPaces → Fitness → Readiness → Goals → RaceDay → PlanReplan → PlanAdaptive), synced summaries (Recap → Streaks → RunStats → PrRecency → RouteSimplify → RouteElev → AutoEffort), and **Back-to-start LAST** — the safety page is exactly one long-press from home, and the whole back half is the long-press walk's territory. Combined with the §283 filter the practical effect is strong: an unsynced no-course run cycles ~8 live pages; a race with a loaded course puts every course page within ~5 presses forward or back. The enum declaration order IS the cycle order (statusbar rank math + a test pin that equivalence), which means the settings `FLAG_PAGES` bitmask bits renumbered with the reorder — free while the deployed fleet is zero watches and no phone-side picker UI exists yet; the golden vectors carry raw masks and were already version-stamped by §283's frame change.
