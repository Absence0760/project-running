...
## 166. Turn-by-turn voice cues use a self-contained geometric generator over the route polyline, not an external routing engine
...
## 167. Offline tile packs are a sibling on-disk cache (eviction-resistant), not LRU-evicted and not in Supabase
