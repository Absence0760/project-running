...
## 190. Club photos are a `club_photos` gallery re-keying the route-photo shape to club membership — member-upload, owner-or-admin delete, a club-visibility SELECT gate, and a `club_photo_process` worker
...
- **ADR number**: §190 chosen deliberately high to avoid collision with sibling round-2 workers claiming §178–§184 on the shared trunk; renumber on merge if it lands lower.
