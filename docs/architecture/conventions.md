...
`font_size_floor_guard.test.ts` reads the floor **out of `app_theme.dart`** so the two platforms cannot drift, then fails any `rem`/`px` `font-size` under it. Prefer `var(--font-size-section-label)` (0.7rem = 11.2 px — headroom over the floor, not an exact meet, so § 522's no-later-multiplier rule does not bite it).

**Reach for the token, not its value.** 81 declarations spell the literal `0.7rem` and 21 used the token; both land on 11.2 px today, but only the token moves if the floor does, and per § 522 a hand-spelled value sitting exactly *on* a floor is the fragile kind. The floor guard cannot see those 81 by construction — they are at the floor, not under it.
