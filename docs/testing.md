...
### `apps/web/tests-e2e/` — Playwright suite (37 tests across 7 files)
...
**Stack:** `@playwright/test` 1.59.1, Chromium-only, run via `pnpm test:e2e` from `apps/web`. Webkit + Firefox would 3× the runtime; cross-browser bug yield on a SvelteKit static site is low. Single worker (`fullyParallel: false`, `workers: 1`) — concurrent tests against shared Supabase data race. The dev server (vite, port 7777) is auto-started by Playwright's `webServer` block; spec files attach pre-saved auth state so the only place the `/login` form runs is `globalSetup`. Total wall-clock for the full suite: ~70-80 s.

**One thing to know about sign-out:** the `globalSetup` fixture signs each user in once and saves cookies + the refresh token to `.auth/<user>.json`. Every test that uses `test.use({ storageState: USER_X.storageStatePath })` loads that file fresh. **Any test that triggers `auth.logout()` (which calls `supabase.auth.signOut()`) will revoke the current session's refresh token server-side — and that's the same token sitting in the saved file** (yes, even with `scope: 'local'` — supabase-js still hits the server logout endpoint). Subsequent tests for that user load the file, get the now-revoked token, and fail to authenticate. Practical rule: only one test in the suite can sign-out a given user, and it must run last alphabetically. Today that's `smoke.spec.ts` (USER_A sign-out, only). If you need to add another sign-out test, either pick a different user with no later-running tests, or make the test the new last-alphabetical one.
...
**Pinned UUIDs (`fixtures/seeded-data.ts`)** — `RUNNER_PUBLIC_RUN_ID` and `ALEX_PRIVATE_RUN_ID` are deterministic UUIDs hardcoded into `seed.sql` so cross-user, share-page, and privacy-clipping tests can address known rows without first listing-and-picking. The pinned public run is also exempted from the cross-user kudos / comment seeds (`id != '...'` filter on the engagement inserts) so the data-flow toggle test starts at zero kudos.

#### `tests-e2e/smoke.spec.ts` — 7 tests

Sign-in / sign-out + the headline authenticated routes. Pins that the four most-trafficked URLs (`/dashboard`, `/runs`, `/runs/[id]`, `/share/run/[id]`) actually mount and render seeded data — the regression risk is a route-level fetch that 500s, an `auth-loading-vs-fetch` race, or a `BASE_PATH` regression that breaks every page. `/runs` test exercises the `switchRunsToAllTime` helper because the default filter is "today" and the seed runs span ~6 weeks; without it the list looks empty and the test fails for the wrong reason. Anonymous `/share/run/[public-id]` stubs the `clip-public-track` Edge Function — its happy path is covered by `rls_privacy_clipping_test.sql` (17 pgtap tests on the underlying RPC) so we don't need to keep the EF host running for every spec.

#### `tests-e2e/security.spec.ts` — 5 tests

RLS / authorisation regressions that reach the UI. The pgtap suite (181 tests across 23 files) covers SQL-layer correctness; this spec covers the next failure mode — an RLS policy that's correct at the database level but reaches the UI via a fetch path that bypasses or misuses it. Three describe blocks: cross-user run isolation (User A ↛ User B's data and vice-versa, both directions because a typo on an `or` branch could be one-sided), anonymous walls (anon ↛ private content via `/share/run/[id]` + redirects from `/dashboard` and `/runs` to `/login`), storage-state walls (unauthenticated ↛ authenticated routes). The `User B's /runs list excludes runner's runs` test asserts every visible href belongs to the seed-pinned user-B set rather than just the negative — a silent empty would still let a future fetch-path bug pass.

#### `tests-e2e/data-flow.spec.ts` — 3 tests

CRUD round-trips through Supabase. Smoke covers "the page loads"; security covers "you can't see what you shouldn't"; this spec covers "writes actually persist." Catches: a form field that doesn't bind correctly (form looks right but never round-trips), an optimistic-update path that updates local state but doesn't hit Supabase (rolls back silently on reload), a delete that removes the row from the list but leaves the server-side record (or vice-versa). Three tests: manual-run create-then-delete via `/runs` modal, edit run title with a reload to confirm persistence, and cross-user kudos toggle (alex on runner's pinned public run via `/share/run/[id]` — `/runs/[id]` is owner-only). The kudos test reload step is the load-bearing assertion: optimistic UI alone could make the in-memory toggle pass; the reload proves the row hit the table.

#### `tests-e2e/happy-paths.spec.ts` — 4 tests

Multi-step user journeys that exercise the most-trafficked flows end-to-end. Smoke covers "the page loads"; security covers "you can't see what you shouldn't"; data-flow covers "writes round-trip"; this spec covers "the user actually accomplishes a thing." Four describe blocks, each picked to exercise a different surface that a single-page check would miss:

- **Filter runs by activity type** — User A's `/runs`, switch to "All time", click the Walk button in the activity-type group, assert the list collapses to exactly 1 row (the seed has one walk, one hike, 26 runs). Proves both the filter UI exists and that it queries the right metadata key (`metadata->>activity_type`, not a column).
- **Cross-user comment + delete** — User B (alex) on runner's pinned public run via `/share/run/`. Posts a comment via the composer, asserts the comment count flips to "1 comment", reloads to confirm the write hit Supabase (not optimistic state alone), deletes via the per-comment delete button, asserts the count goes back to "0 comments". Complements the kudos test in `data-flow.spec.ts` — together they pin the two main engagement-chain writes.
- **Period summary modal** — User A's `/dashboard`. Click the "This Week" stat card, assert the `Period summary` modal opens with the week/month toggle inside (so we know `<PeriodSummary>` mounted, not just the modal shell). Catches a regression where the stat card stops being a button or the modal wiring breaks.
- **Display name update + restore** — User A's `/settings/account`. Save a unique display name, reload the page, assert the input still holds the new value, then restore to the original. Reload-then-assert is the load-bearing step — the regression risk is a save returning 200 but RLS blocking the update silently, or optimistic local state masking a failed write.

#### `tests-e2e/interactions.spec.ts` — 5 tests

UI plumbing that must survive a navigation, reload, or cross-user write. Smoke / security / data-flow / happy-paths cover sign-in, RLS, CRUD, and multi-step flows; this spec covers the next layer down — the bits of UI plumbing that silently break when a `$state` declaration loses sync with its persistence layer (localStorage, the auth store, the user_profiles row). Five describe blocks:

- **Run filter state persists across reload** — User A's `/runs`, switch to "All time" + Walk activity, reload, assert both selections survived. Pins the `runs_filters_v1` localStorage round-trip — a regression where the writer fires before hydration silently overwrites the user's choice every page-load.
- **Routes search narrows the My-routes list** — User A's `/routes`, type "Richmond" into the search box, assert the list collapses to the seeded `Richmond Park Loop` row, click the clear button, assert the count restores. Pins both the substring-match `$derived` and the clear-button affordance.
- **Theme toggle persists across reload** — User A's `/settings/preferences`, switch to Dark, assert `<html data-theme="dark">`, reload, assert the attribute resurrects from localStorage. The combination is the load-bearing assertion — `applyTheme` writes both the DOM attribute AND localStorage, and `initTheme` runs on every layout mount; a regression in either half breaks "user picks dark, comes back tomorrow, sees light."
- **Follow → unfollow toggles button + counter** — User C (morgan, no outgoing follows in seed) on `/u/<runner.id>`. Click follow, assert the button label flips from `Follow` to `Following` and the follower counter increments by 1. Click again, assert both restore. Pins `followUser` / `unfollowUser` round-trips against `user_follows` plus the optimistic UI on top.
- **Anonymous `/share/route/[id]`** — Anon visitor on the pinned public-route share URL. Assert the route name + surface tag + distance render. The route's anon path goes through the `public_routes` view (which strips `geom` / `start_point`) — this test catches a regression where the view definition drops a field the share page renders.

#### `tests-e2e/flows.spec.ts` — 5 tests

Cross-cutting flows whose regressions span multiple components or sessions. Five describe blocks:

- **Sidebar collapse persists across reload** — Click the collapse-toggle on /dashboard, assert the `.sidebar.collapsed` class is applied, reload, assert the class survived (read from `localStorage.sidebar_collapsed`), expand again to restore. The same shape as the `/runs` filter persistence test but at the layout level — a regression in the layout's `onMount` reader would silently force every authenticated session back to expanded on every page-load.
- **Cross-user kudos triggers a notification on the recipient** — Two simultaneous browser contexts. USER_B (alex) on `/share/run/<runner public>` clicks kudos, USER_A (runner) on `/dashboard` reloads, the bell badge increments by 1 and the popover entry reads `"Alex Chen gave kudos to your 9.0 km"` (from `verbFor()` in `NotificationBell`). "Mark all read" clears the badge; alex rescinds the kudos to leave seed state intact. Pins the SECURITY DEFINER `notify_run_kudos` trigger (migration `20260528_001`), the layout's `notificationStore.refresh()` on auth-ready, AND the popover entry rendering with the actor's display name.
- **Comment reply (nested write)** — USER_B on `/share/run/`. Posts a top-level parent, clicks the per-comment "Reply" button, fills the inline form, submits, asserts the reply appears under `.replies` inside the same article. Cleanup deletes the parent — `parent_comment_id ON DELETE CASCADE` (migration `20260522_001`) takes the reply with it. The regression risk: a UI form binding that drops the parent id and orphans the reply at the top level.
- **Star a route persists across reload + filter** — USER_A clicks the `.star-btn` on `/routes/<pinned-public>`, asserts the `.starred` class, reloads (server-side state must agree), navigates to `/routes`, flips the "Show starred only" filter, asserts the route is in the narrowed list. Cleanup unstars + clears the filter. Catches regressions in either the `setRouteStar` write (RLS dropping the update) or the list re-fetch (filter reading a stale cache).
- **Inline edit on `/runs/[id]` — Cancel reverts unsaved changes** — Companion to the data-flow Save test. Click Edit, type a draft title, click Cancel, assert the editor closes and the heading reads the original title (no save fired); reload to confirm nothing landed server-side. Catches a regression where Cancel accidentally writes (e.g. a refactor wiring Save and Cancel to the same handler).

#### `tests-e2e/areas.spec.ts` — 8 tests

Broad coverage sweep — one thin "this area mounts and renders something seed-specific" test for every route the deeper specs don't already touch. Catches the "did this entire area break?" failure mode (a 500, a missing fetch, an empty render). Future rounds will deepen each area into its own spec — for now this is the breadth pass. Eight describe blocks:

- **Landing (`/`)** — anon visitor sees the hero h1 (`/Plan routes/`) + the "Get Started" CTA. Pins the public marketing surface that lives outside the auth shell.
- **Feed (`/feed`)** — User A sees a non-empty feed. Runner follows alex + morgan; both have ~12-13 recent public runs all within the `FEED_WINDOW_DAYS=14` window. Empty would mean either the follow graph dropped, the `public_runs` view broke, or the date window regressed. Asserts at least one entry shows author "Alex Chen" or "Morgan Lee".
- **Plans (`/plans`)** — Seeded "Sydney Half 2026" plan card visible. Drills into `/plans/[id]` to confirm the detail page also mounts. Pins the plan-list fetch + the plan-detail layout.
- **Clubs (`/clubs`)** — Seeded "Sydney Run Club" card visible on the My-clubs tab (default), drills into `/clubs/sydney-run-club`. Pins the `enroll_club_owner_trigger` (owners are auto-added to `club_members`), `fetchMyClubs`, and the slug routing.
- **Coach (`/coach`)** — Chat surface mounts (no LLM call — that would need to stub the SSE endpoint). Asserts the composer textarea (placeholder "Ask about today, …") is visible past the "Loading…" state. Pins `CoachChat` mount + `auth.user`-gated init.
- **Settings / Integrations** — Strava / parkrun / Garmin Connect rows render regardless of connection state. Pins the hardcoded provider list + the connection-state query.
- **Settings / Upgrade** — Pro pricing card renders for a free user. Pins `PRO_PRICE_MONTHLY` import + `auth.isPro` derivation.
- **Live spectator (`/live/[id]`) — anon** — Anon visit to a public run's live page mounts the shell: brand label, status badge (Connecting / Demo / Live), three stat tiles (Distance / Elapsed / Pace). The status transitions from connecting → demo without active broadcast pings; the test only asserts the shell, not the status, since real-time depends on background timers.
...
**Nine production bugs were fixed during the suite's authoring passes.** Each was a real defect, not test scaffolding:
