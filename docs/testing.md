...
- **`SocialService` and `TrainingService` ChangeNotifier glue.** Pure helpers + the role-derivation booleans on `ClubView` are covered by `social_service_test.dart` (13 tests). The Supabase-touching methods on both classes now have a DI seam — `SocialService.withClient(SupabaseClient)` and `TrainingService.withClient(SupabaseClient)` — mirroring `ApiClient.withClient`. Wire-level integration coverage of the highest-leverage entry points (`browseClubs`, `fetchMyClubs`, `fetchMyPlans`, `fetchActiveOverview`) lives in `apps/mobile_android/test/services_integration_test.dart` (5 tests), gated on `SUPABASE_TEST_URL` and run by the same `api-client-integration` CI job that boots local Supabase. The other ~25 methods on the two services are still uncovered — the seam + the test scaffold lower the cost of adding more from "design a fake-client framework" to "add another test case." Service-level errors here are bounded — failure modes are stale lists or missed-RSVPs, not data leaks (RLS gates the actual rows; see the pgtap suite).
...
If you want to expand coverage, the best remaining targets in priority order: (1) extend `run_screen_recording_flow_test.dart` past the Finish tap by mocking the storage-upload boundary; (2) stand up a true `integration_test` harness for the device-led paths; (3) widen the `SocialService` / `TrainingService` wire-level suite (the DI seam + 5-test scaffold from `services_integration_test.dart` is in place — adding new tests is one block per method, not a refactor).
