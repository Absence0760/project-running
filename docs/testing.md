...
### `apps/backend/supabase/tests/rls_*.sql` — pgtap RLS suite (255 tests across 35 files)
...
- **`rls_club_posts_test.sql`** — 9 tests pinning the threaded-feed policies. Active member SELECT works; stranger SELECT on a private club's feed returns zero; any active member can INSERT a **top-level OR reply** (the 20260428_001 "members can post" catch-all supersedes the narrower 20260417_001 "admins can post top-level" policy that's still on the table — current effective behaviour is "any active member can post anything"); stranger and **pending requester** are both rejected (the `is_club_member` exclusion of pending status is the load-bearing write guard); forged author_id rejected; author can DELETE their own post; **club owner cannot DELETE a member post** (no admin-DELETE policy — moderation is via member removal, asymmetric with `club_members` where admins can DELETE other rows).
...
- **`rls_segments_test.sql`** — 8 tests pinning the final policy stack on `segments` after migrations `20260526_001` + `20260703_001` (route through `is_route_visible_to`). Author + stranger can SELECT a segment on a public route; stranger cannot SELECT a segment on a private route; forged-`created_by` INSERT rejected; INSERT against an invisible private route rejected (closes the same shape as `route_reviews`); author can UPDATE their own segment but a stranger UPDATE is a silent no-op; the `length_m >= 100` CHECK (decisions §37) fires on a tiny segment. The author-OR-route-owner write paths share a policy, so route-owner-driven edits also flow through the same `routes.user_id = auth.uid()` subquery these tests exercise.
- **`rls_route_reviews_test.sql`** — 8 tests pinning the final policy stack on `route_reviews` after migrations `20260627_001` (INSERT visibility gate) + `20260703_001` (route through `is_route_visible_to`). Reviewer can read their own row + a non-author can read on a public route; forged-`user_id` INSERT is rejected; INSERT against an invisible private route is rejected (closes the cross-user pollution shape); cross-user UPDATE / DELETE are silent no-ops; the rating 1..5 CHECK and the `(route_id, user_id)` UNIQUE both fire. Anon paths are deliberately not covered — PostgreSQL 17.6 SIGSEGVs in this file when an anon role evaluates the `is_route_visible_to` policy chain inside the pgtap transaction; the anon read path is exercised at the wire level by Playwright instead.
...
These twenty-three files cover the tables + RPCs + triggers where a single-row leak / single-trigger bypass would be a privacy, impersonation, or revenue incident. They do NOT exhaustively cover every table (37 in total) — `events` (full), `event_attendees`, and others are still uncovered. Add a file when you touch a sensitive policy, or when an audit lands.

### `apps/backend/supabase/functions/_shared/webhook_security.test.ts` + `revenuecat-webhook/lib.test.ts` — 33 deno tests
...
### `apps/web/tests-e2e/` — Playwright suite (308 tests across 82 files)
