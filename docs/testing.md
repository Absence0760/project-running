...
### `apps/backend/supabase/tests/rls_*.sql` — pgtap RLS suite (236 tests across 33 files)
...
These twenty-three files cover the tables + RPCs + triggers where a single-row leak / single-trigger bypass would be a privacy, impersonation, or revenue incident. They do NOT exhaustively cover every table (37 in total) — `club_members`, `club_posts`, `events` (full), `event_attendees`, and others are still uncovered. Add a file when you touch a sensitive policy, or when an audit lands.
