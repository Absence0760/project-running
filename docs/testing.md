...
### `apps/web/tests-e2e/` — Playwright suite (24 tests across 5 files)
...
**Seven production bugs were fixed during the suite's authoring passes.** Each was a real defect, not test scaffolding:
...
These would have shipped silently — they don't show up in unit tests because the failures are in the integration between auth state, network calls, Svelte 5's hydration timing, and accessible-name computation. This is the bug-yield the suite was put in for.
