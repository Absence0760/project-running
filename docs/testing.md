...
**Tests pass locally but fail in CI** — most commonly from wall-clock assumptions. The recorder used to have exactly this bug in its speed clamp (wall-clock dt went near zero under load). If you see flaky timing tests, use GPS-reported timestamps from the `Position` rather than `DateTime.now()`, and be suspicious of any direct `DateTime.now()` subtraction in production code. The same lesson hit `_calculatePace`: `_currentWaypoint.timestamp` was set from `DateTime.now()`, so a tight inject burst collapsed every track-waypoint timestamp to the same millisecond and pace silently returned null. The fix was to use `pos.timestamp` for the waypoint — `calculate_pace_test.dart#synchronous inject loop still produces a non-null pace` is the regression guard.
