...
Total: **~662 unique Dart mobile tests across 83 test files, executed by both mobile targets** (mobile_android and mobile_ios share a byte-for-byte identical Dart codebase — see the iOS / android `CLAUDE.md` files), plus 44 tests in run_recorder (across 4 files), 2 in core_models, and **223 TypeScript unit tests across 18 files** in the web app. Both mobile apps run the same 83 test files; `flutter test` compiles them once per target, so end-to-end CI exercises ~1,324 mobile test runs. No integration tests, no golden tests yet. Counts here are point-in-time — they drift fast. Run `grep -cE '^\s*(test|testWidgets)\(' apps/mobile_android/test/*.dart` for the live per-target count and `diff -rq apps/mobile_android/test apps/mobile_ios/test` to confirm the trees stay in lockstep.
...
- **Coach handler pre-stream paths.** `apps/web/src/lib/coach/handler.ts` mixes pure validation logic (auth-header parsing, runs-limit clamping, tier/quota gating) with a `createClient()` Supabase call at module scope, which trips `tsx --test`'s loader. Refactoring those into a `coach/limits.ts` pure helper would lift the constraint; not done yet. The existing `coach/types.test.ts` pins `TIER_LIMITS` + `emptyUsage` only.
- **`settings.ts#effective<T>`.** The pure overlay-lookup helper sits in a module that imports the SvelteKit `$env/static/public` virtual import transitively, so `tsx --test` can't load it. Splitting `effective<T>` into `settings_effective.ts` would unblock testing.
