...
### `apps/web/tests-e2e/` — Playwright suite (19 tests across 4 files)
...
**Idempotency** — every test that mutates data either rolls back at the end (rename / kudos rescind / comment delete / display-name restore) or creates-and-deletes its own row (manual run CRUD). The suite is safe to re-run without `supabase db reset` between runs. If a test fails partway and leaves orphaned state, the next reset cleans it up.

**Four production bugs were fixed during the suite's first authoring pass.** Each was a real defect, not test scaffolding:
...
These would have shipped silently — they don't show up in unit tests because the failures are in the integration between auth state, network calls, and Svelte 5's hydration timing. This is the bug-yield the suite was put in for.
