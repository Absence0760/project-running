...
- **Test coverage.** The recording subsystem is hardened but not covered by a dedicated test suite. `apps/mobile_android/test/widget_test.dart` is still the stale `flutter create` stub. A follow-up testing pass would be a good next move.
