...
| `discoverable_nearby` | `bool` | U | `false` | Opt-in to "runners nearby" coarse-location discovery (issue #466, migration `20270426_001`). When the string `'true'`, other opted-in runners near the area the user set (`user_settings.discoverable_area`, a geocoded city/area centroid rounded to ~1 km, NOT live GPS, written via the `set_discoverable_area` RPC) can find them through the `discoverable_runners_near` SECURITY DEFINER reader. Default false / fail-closed: no one appears unless they flipped this AND set an area. The reader also honours minor-exclusion, `shadow_hidden`, `discoverable_in_search` (a search opt-out also removes you from nearby), and blocks; it returns only a coarse distance bucket, never a coordinate. The whole surface is additionally gated behind the default-off `PUBLIC_ENABLE_NEARBY_RUNNERS` web feature flag pending owner + CISO/counsel sign-off (decisions §260). |
