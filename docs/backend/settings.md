...
|---|---|---|---|---|
| `preferred_unit` | `'km' \| 'mi'` | UD | `km` | Distance unit for all displays. Mirrors `profiles.preferred_unit` today; migrate away from the column in a follow-up. **As a UD key, a per-device override (set on `/settings/devices`) wins over the universal bag, which wins over the profile column.** The web app-wide `setUnit` signal is seeded from the profile column by the auth store, then **re-resolved through the full device→universal→column overlay** in the root `+layout.svelte` (`effectivePreferredUnit` in `settings_overlay.ts`) — otherwise a per-device `mi` override would be silently dead (column-only) and the browser would render km. **Onboarding seeds the units toggle from the visitor/device locale** (`defaultUnitForLocale` in `format/locale_defaults.ts` ↔ `locale_defaults.dart` — mi for US/GB/LR/MM, km elsewhere) on both web `/onboarding` and the mobile setup wizard; an explicitly chosen unit (the bag key) still wins over the locale seed. |
...
| `discoverable_nearby` | `bool` | U | `false` | Opt-in to "runners nearby" coarse-location discovery (issue #466, migration `20270423_001`). When the string `'true'`, other opted-in runners near the area the user set (`user_settings.discoverable_area`, a geocoded city/area centroid rounded to ~1 km, NOT live GPS, written via the `set_discoverable_area` RPC) can find them through the `discoverable_runners_near` SECURITY DEFINER reader. Default false / fail-closed: no one appears unless they flipped this AND set an area. The reader also honours minor-exclusion, `shadow_hidden`, `discoverable_in_search` (a search opt-out also removes you from nearby), and blocks; it returns only a coarse distance bucket, never a coordinate. The whole surface is additionally gated behind the default-off `PUBLIC_ENABLE_NEARBY_RUNNERS` web feature flag pending owner + CISO/counsel sign-off (decisions §258). |
