...
| `email_weekly_digest` | `'on' \| 'off'` | U | `off` | **Opt-IN** consent for the weekly engagement digest — bulk/promotional mail, deliberately a **separate key** from the transactional `email_notifications` (marketing consent is never inferred from a transactional setting). **Default `off`**: the user must explicitly opt in. Read server-side by the Go worker's `weekly_digest` handler; the digest **send itself is gated on CISO + counsel sign-off** and is not yet scheduled (foundation migration `20270108_001`). The **consent toggle** ships on both clients: web Settings → Preferences ("Notifications" card, checkbox) and mobile Settings → Preferences ("Notifications" section, `SwitchListTile`), both i18n'd across all six locales, written through the universal-bag `updateUniversal` path (web `autoSave` / mobile `SettingsSyncService`). |
