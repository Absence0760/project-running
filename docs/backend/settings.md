...
| `push_notifications` | `'all' \| 'important' \| 'off'` | U | `important` | Which notification kinds are delivered by **web push** (system notification to a subscribed browser). Same three-mode shape + `important`-by-default classification as `email_notifications`, but an **independent** key — muting email must not mute push, and vice-versa. Read server-side by the Go worker's `web_push` handler (migration `20261219_001`), which fires only for users who have a `push_subscription` registered on a device (see below) and sends an encrypted Web Push message per browser. Unknown/non-string values fall back to `important`. No per-control UI toggle ships yet (gating works with the default); the browser subscribe toggle lives in web Settings; a category toggle mirroring the email one is a follow-up (`docs/product/followups.md`). |
...
| `multi_modal_nav` | `bool` | U | `false` | Phase 4 multi-modal feature flag (decisions §63, [multi_modal.md](../features/multi_modal.md)). When `true`, the web surfaces light up their gym slices (gym routes stay reachable by URL regardless — the flag gates the affordances, not the data, so a pure runner's app is unchanged): the **Gym** sidebar item (`+layout.svelte`); the self-hiding Today's-lift + Recent-lifts cards AND the lift→load contribution to the fitness/fatigue/form curve on `/dashboard`; and the All/Runs/Lifts/Meals kind chips + unified `activities` timeline on `/history` (the run-history list; `/runs` redirects to it). Each is *also* gated on data presence, so a flagged-but-empty user still sees no clutter. Read in each surface via `effective<boolean>(…, 'multi_modal_nav', false)`. **This flag is web-only.** Mobile shipped the same gym/nutrition surfaces **ungated** (the G5 Run→Log nav reshape + data-gated Home cards; the "protect the pure runner" job is done by `keep_run_primary`, not by this flag), so a web runner sees nothing while a mobile runner gets the Log sheet. There is **no web UI toggle** (set it directly in the prefs bag) — the resulting web↔mobile gating inconsistency is tracked in [followups.md](../product/followups.md) (the durable fix is to ungate web to match mobile, not to add a toggle). |
...
| `nutrition_activity_level` | `'sedentary' \| 'light' \| 'moderate' \| 'active' \| 'very_active'` | U | `moderate` | Activity multiplier for the Mifflin-St Jeor nutrition target (`nutrition_targets.ts`/`.dart`). Not health-special-category (an effort label, not a body measurement), so it lives in the prefs bag and auto-saves like other prefs — unlike height/weight which are consent-gated on `health_data_consent_at`. Phase 4 nutrition (`multi_modal.md`). **Consumed on mobile (G5):** set in Settings → Body metrics (`settings_body_metrics_screen.dart`), read by `loadNutritionTargets`. |
| `nutrition_goal` | `'lose' \| 'maintain' \| 'gain'` | U | `maintain` | Weight-goal calorie delta applied after TDEE for the nutrition target (lose −500 / maintain 0 / gain +300 kcal). Same placement rationale as `nutrition_activity_level`. Phase 4 nutrition. **Consumed on mobile (G5)** alongside `nutrition_activity_level`. |
