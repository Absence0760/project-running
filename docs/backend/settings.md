...
| `show_calories` | `bool` | U | `true` | When `false`, the run-detail calorie-estimate cell is hidden (the estimate silently assumes a 70 kg default when no body weight is set — a weight-conscious runner can opt out entirely; hunt-newrunner 2026-07-02). Read by web `/runs/[id]`; editable in web Settings → Preferences (Units & Display). Mobile does not read it yet — the run-detail calorie tile mirror is tracked in followups.md. |
