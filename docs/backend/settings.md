...
| `primary_goal` | `'general_fitness' \| 'weight_loss' \| '5k' \| '10k' \| 'half_marathon' \| 'marathon'` | U | — | Set by the post-signup `/onboarding` wizard (step 3). Read back by the finish-step "create my training plan" CTA (`planPresetForGoal` in the `onboarding` twin) to deep-link into the plan wizard preselected — the three beginner-leaning goals seed the walk-run 5K, the distances map 1:1 to `training.ts#GoalEvent`. See `docs/architecture/decisions.md § 78`. |
...
| `show_calories` | `bool` | U | `true` | When `false`, the run-detail calorie-estimate cell is hidden (the estimate silently assumes a 70 kg default when no body weight is set — a weight-conscious runner can opt out entirely; hunt-newrunner 2026-07-02). Read by web `/runs/[id]` and mobile `run_detail_screen` (calorie tile hidden when `false`); editable in web Settings → Preferences (Units & Display) and mobile Settings → Preferences → Units & Display ("Show calorie estimates"). |
...
| `auto_live_share` | `bool` | D | `false` | Start a live broadcast automatically when a run recording starts on THIS device ([safety.md](../features/safety.md)). Device-scoped like the other recording prefs (`voice_feedback_enabled`): it's a property of the phone that records. **Default off and deliberately opt-in** — a live share makes the in-progress run publicly viewable by link (`beginLiveBroadcast` stub, `is_public=true`). The mobile run screen reads it at `_begin()`; failure to start the share is L4 (never blocks the recording). No web control (web doesn't record). |
...
- **Dart clients** (`mobile_android`, `mobile_ios`):
  ...
  `voice_cue_types` is read from **both** bags (universal first, device
  second) since it became `UD` — that is what lets a choice made on the
  web settings page reach the phone that actually speaks the cues.
