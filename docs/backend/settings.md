...
| `voice_cue_types` | `object` | D | `{}` | Per-cue voice toggles as a map of cue id → bool (`splits`, `start_finish`, `off_route`, `pace_alerts`, `workout_steps`, `cutoff_catch_up`, `marker_targets`, `phase_transitions`; issue #607). An **absent id is ON**, so a new cue type defaults audible without a migration, and the pull path merges entry-by-entry so a bag written by an older build can't erase toggles it didn't know about. Turn-by-turn cues keep their own older local-only pref and are not in this map. Device-scoped like the other recording prefs; recording is mobile-only, so web has no control. |
