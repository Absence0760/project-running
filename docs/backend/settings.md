...
| `exclude_gym_from_readiness` | `bool` | U | `false` | When `true`, gym sessions are dropped from the dashboard fitness/fatigue/form (CTL/ATL/TSB) curve so run readiness + recovery advice stay run-only. Default `false` = lifts add to fatigue like a run (the `lift_load.ts`/`.dart` → `training_load` contribution). Read by `/dashboard` (web); the run-only curve is always recoverable by construction (`training_load.ts` separable provenance), so this is a display-side opt-out, not a data change. Editable in web Settings → Preferences (Heart-rate-zones card). Mobile honouring is a follow-up. |
