...
| `email_weekly_digest` | `'on' \| 'off'` | U | `off` | **Opt-IN** consent for the weekly engagement digest — bulk/promotional mail, deliberately a **separate key** from the transactional `email_notifications` (marketing consent is never inferred from a transactional setting). **Default `off`**: the user must explicitly opt in. Read server-side by the Go worker's `weekly_digest` handler, which additionally hard-blocks any address in `email_suppressions` (bounce/complaint/unsubscribe). The RFC 8058 one-click unsubscribe (a stateless keyed-HMAC token, no auth) flips this to `off` + adds a suppression row. Migration `20270108_001` ships the foundation; the digest **send itself is gated on CISO + counsel sign-off** and is not yet scheduled. Preference-center UI is a follow-up. |
