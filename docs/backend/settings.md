...
| `nutrition_activity_level` | `'sedentary' \| 'light' \| 'moderate' \| 'active' \| 'very_active'` | U | `moderate` | Activity multiplier for the Mifflin-St Jeor nutrition target (`nutrition_targets.ts`/`.dart`). Not health-special-category (an effort label, not a body measurement), so it lives in the prefs bag and auto-saves like other prefs — unlike height/weight which are consent-gated on `health_data_consent_at`. Phase 4 nutrition (`multi_modal.md`). |
| `nutrition_goal` | `'lose' \| 'maintain' \| 'gain'` | U | `maintain` | Weight-goal calorie delta applied after TDEE for the nutrition target (lose −500 / maintain 0 / gain +300 kcal). Same placement rationale as `nutrition_activity_level`. Phase 4 nutrition. |
