...
| `gym_sets` | `20261204_001` | One row per logged set, child of `gym_workouts`. `id, workout_id, set_index, exercise_name, reps, weight_kg, rpe` (`duration_s` added later for timed holds — instructor_business.md M2; `exercise_id` nullable catalogue FK — `20270222_001`; `set_type` NOT NULL default `working`, CHECK over the routine vocabulary warmup/working/dropset/amrap/failure/backoff — `20270228_001`, decisions §189). Weights canonical kg. **Non-owner reads of a PUBLIC workout's sets go through the redacted `public_gym_sets` view** (`20270327_001`): the base "visible via parent workout" policy became owner-only-effective when `20270313_001` dropped the parent's public branch (which broke the /share/workout page — CI run 28707481878), and the view projects only `id, workout_id, set_index, exercise_name, reps, weight_kg, duration_s` for sets of `is_public` workouts. `rpe` is **owner-only**. |
...
### `normalise_exercise_name(p_name text)` / `collapse_exercise_whitespace(p_name text)`

The single definition of the exercise grouping key (migration `20270623000001`, decisions §790). `collapse_exercise_whitespace` folds every whitespace code point to one space and trims; `normalise_exercise_name` composes it with a five-entry `translate` and `lower()`. Both are `immutable` + `parallel safe`, and immutable **in fact** rather than by declaration — neither consults the database ctype — which is what lets a CHECK constraint reference one.

Three things are spelled out rather than delegated, and each closes a measured divergence with `gym_prs.ts` / `gym_prs.dart`:

* **The class, by code point** (`chr(9) chr(10) chr(11) chr(12) chr(13) chr(32) chr(133) chr(160) chr(5760) chr(8192)–chr(8202) chr(8232) chr(8233) chr(8239) chr(8287) chr(12288) chr(65279)`) rather than `\s`. Postgres `\s` is `[[:space:]]`, whose membership above U+007F is decided by the ctype — it matches U+00A0 and U+2000–U+200A under `en_US.UTF-8` but never U+0085 or U+FEFF, both of which the clients fold. A key whose value depends on the server's locale is not a key.
* **`btrim(x, ' ')` with the character named.** `btrim(text)` with no second argument strips only U+0020, so the old `regexp_replace(lower(btrim(name)), '\s+', ' ', 'g')` left every other whitespace character at either end of a name in place and the collapse then turned it into a leading/trailing **space** — `"<TAB>Bench Press"` keyed as `" bench press"` server-side and `"bench press"` on both clients. Plain ASCII, reachable from a paste.
* **Five case mappings applied by hand** before `lower()`: U+0130 → `i` and the four titlecase digraphs. Sweeping the BMP, JS and Dart's `toLowerCase()` differ on 199 code points and each differs from libc's `towlower`; U+0130 is the one an exercise name reaches, where JS yields `i` + U+0307 and the other two a bare `i`. Everything else is left to each rail's own `lower()`.

`scripts/check_shared_constants.mjs` reads the class and the fold table off all three rails as code points and fails the PR on drift — including on a new SQL function that inlines its own `regexp_replace` over `exercise_name`, which reads as no code points and so reports the guard going blind rather than agreeing. Both keep postgres's default `PUBLIC` EXECUTE, as the other pure text helpers do (`privacy_coarsen_coord`, `privacy_distance_m`): they read no data, and a CHECK constraint's expression is ACL-checked against the **writing** role, so revoking would have to be undone by a grant to every role that can insert a routine exercise. The two trigger functions around them are revoked from `public` and `authenticated`, following the `auto_tag_default_gear` precedent — a grant on a trigger-internal function is vestigial. pgTAP `exercise_name_normalisation_test.sql` (14 tests).

**The stored key is stamped by the server.** `gym_routine_exercises.exercise_key` and `exercises.name_key` are written by a `before insert or update` trigger (`stamp_exercise_key` / `stamp_exercise_name_key`) over `normalise_exercise_name`, so an offline client on an older build can no longer write a key the server disagrees with — that is how the three rails came to disagree about a persisted value in the first place. `gym_routine_exercises` additionally carries `gym_routine_exercises_key_normalised_chk`, added `NOT VALID` and validated separately after a predicate-scoped backfill. `exercises` deliberately has **no** such constraint and no backfill: its two partial UNIQUE indexes on `name_key` mean a re-key can collide with a sibling row, and merging two catalogue entries is a data decision a migration must not take on a user's behalf.

### `gym_exercise_records()`

Returns one row per exercise — `(exercise_name, heaviest_weight_kg, heaviest_weight_reps, best_volume_kg, best_est_1rm_kg, last_performed_at, session_count)` — for the `/gym/records` surface (migration `20261224_001`, perf-hunt follow-up). All-time per-exercise bests can't be served by a windowed client read, so the aggregation lives in SQL (mirroring how run PRs are SQL-maintained); the client-side `exercise_records.ts` stopgap was retired. The SQL is the mirror of `gym_prs.ts#computeExercisePrs` + `exercise_records.ts` (normalised name key, Epley e1rm with the rep clamp, bodyweight-only excluded); the key comes from `normalise_exercise_name` since `20270623000001`, so the display name it returns is whitespace-collapsed too. SECURITY INVOKER (owner-scoped via `gym_workouts`/`gym_sets` RLS + explicit `auth.uid()`). The `gym_prs.ts` badge engine stays client-side for the per-workout temporal badges. pgTAP `gym_exercise_records_test.sql` pins the metrics against the `gym_prs.test.ts` fixture shape.
...
Returns one exercise's sets — `(workout_id, started_at, exercise_name, reps, weight_kg, rpe, duration_s, set_type)` — matched on the **normalised** name — `normalise_exercise_name` since `20270623000001`, which is the SQL twin of `gym_prs.ts#normaliseExerciseName` rather than a second transcription of it, for the `/gym/exercise` progression view (migration `20261225_001`, perf-hunt follow-up; `duration_s` added in `20261231_001` for timed work — planks/holds — instructor_business.md M2; `set_type` in `20270525_001`). Bounds the read to one exercise instead of pulling the whole history; the normalised match picks up sessions logged under a different capitalisation (an exact `=` would drop them). SECURITY INVOKER, owner-scoped. pgTAP `gym_exercise_set_history_test.sql` (6 tests).
...
Both RPCs derive their match through `normalise_exercise_name` since `20270623000001` (decisions §790) — before that a lift logged once cleanly and once with a pasted trailing tab was two different exercises to the server and one to both clients.
