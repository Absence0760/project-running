...
trigger calls). 55 migrations write a function-level `from public, anon` revoke
today (43 as `revoke execute`, 12 as `revoke all`); it is the house form for
exactly this reason, and `check_migration_function_revoke_noop.mjs` is what
keeps it — it replays all 471 migrations in version order and fails the PR on
