...
- `gym_routine_exercises` — `exercise_key` = `normaliseExerciseName(exercise_name)` stamped at write time (binds the plan to logged `gym_sets`), and held to it by the validated CHECK `gym_routine_exercises_exercise_key_canonical` against `normalise_exercise_name()` since `20270623000001` (decisions § 790); `position` orders the groups; `superset_group`/`superset_order` exist for P2 but P1 leaves them null (paired-null CHECK). RLS via `EXISTS` against the parent routine's `author_id`.
