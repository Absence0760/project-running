...
| `cleanup-stale-export-blobs` | `23 4 * * *` | Calls `cleanup_stale_export_blobs()` to remove expired data-export artifacts, then `expire_stale_export_jobs()` for the `data_export_jobs` rows that pointed at them — a `ready` row outliving its own object would keep offering a download that 404s (`20270603_001`). Sets `storage.allow_delete_query` transaction-locally past storage-api's `protect_delete()` trigger, and RAISES if any stale object survives its own DELETE, so a sweep that was blocked cannot read as a night with nothing to sweep ([§ 857](../architecture/decisions.md), `20270703000001`). A failed run lands in `cron.job_run_details`. | `20260720_001` |
