...
The gym-programming **P1** reusable-plan tier (migration `20261231_001`, [gym_programming.md](../features/gym_programming.md)). A routine is a named, ordered list of planned exercises, each with planned target sets — **relational, not jsonb** (the deliberate divergence from `plan_workouts.structure jsonb`, justified by per-exercise querying + row-by-row progression). It is **not** a dated activity, so it does **not** feed the `activities` view.
