...
- `safety_escalated_at`, `expected_return_at` — overdue-escalation internals (the once-only stamp + the per-run "not back by X" override); `expected_return_at` is the runner's private plan and must never surface on the anon live page (added to the strip list in `20270410_001`)
...
When you add a new key, classify it explicitly — and update the strip list in the most recent `public_runs` projection migration (current head: `20270325_001_promote_embedded_best_columns.sql` — the view has been redefined several times since `20260714_001`: `20260724_001`, `20260807_001`, `20260924_001`, `20261105_001`, `20261207_001`, `20270214_001` (added the `race_listing_id` pass-through column + the three race age-group / gun-time keys to the denylist), `20270302_001` (added the `elevation_gain_m` column pass-through), then `20270325_001`, which added the four `fastest_*` embedded-best column pass-throughs and carried the denylist forward verbatim) if it lands on the owner-only side. The seed assertions in `apps/backend/supabase/seed.sql` for `public_runs` exercise the projection; an unclassified key that's accidentally dropped (or accidentally exposed) will fail the seed.
