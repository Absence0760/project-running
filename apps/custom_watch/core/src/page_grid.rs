...
/// Cells one screenful of grid shows. The enabled set can exceed it (34 pages
...
mod tests {
    ...
    fn full_mask_seats_a_screenful_and_scrolls_to_the_rest() {
        ...
        // The cycle is wider than the body: the tail is off-window until the
        // cursor reaches its row, and then the window scrolls to it rather
        // than the cell being silently dropped.
        assert_eq!(
