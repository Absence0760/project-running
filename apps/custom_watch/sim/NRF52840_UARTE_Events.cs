//
// NRF52840_UARTE_Events — the stock Renode NRF52840_UART model (v1.16.1,
// renode-infrastructure @ add012af) that also publishes its EVENTS_* to the
// PPI matrix.
//
// Why this exists: `NRF52840_PPI` resolves an event endpoint by asking the
// system bus which peripheral owns the address written to EVENTP[n] and casting
// it to `INRFEventProvider`. The v1.16.1 UART model does not implement that
// interface, so EVERY PPI channel sourced from a UARTE event is refused at
// configuration time with
//
//     [ERROR] ppi: Failed to register PPI from 0x40002108 for channel 2
//
// and the chain silently never fires. Two firmware paths depend on exactly
// that: UARTE1's `split_with_idle` byte counter (EVENTS_RXDRDY -> TIMER1
// TASKS_COUNT, decisions.md § 407 / § 420) and UARTE0's `BufferedUarte` RX ring,
// which needs both EVENTS_RXDRDY -> TASKS_COUNT (its ring write index IS that
// counter) and EVENTS_ENDRX -> TASKS_STARTRX (the hardware pre-arm that makes a
// CPU halt survivable, § 419 / § 698). Without the counter the ring's derived
// write position never leaves 0 and not one NMEA byte is ever delivered, so
// § 421's SHORTS model was necessary and not sufficient.
//
// The first delta is **upstream's own**, not invented here: master added
// `INRFEventProvider`, the `EventTriggered` event, and thirteen
// `EventTriggered?.Invoke(...)` calls beside the existing
// `interruptManager.SetInterrupt(...)` calls. Retiring that half is a one-line
// `watch.resc` change the day the packaged Renode carries the interface itself.
//
// The second delta is a fix, marked "BOUNDED RX DMA", and it is here because
// this investigation tripped over it: v1.16.1 raises ENDRX when the byte count
// reaches RXD.MAXCNT but never disarms the receiver, so once the guest clears
// EVENTS_ENDRX the counter passes MAXCNT, the equality test never matches
// again, and `currentRxPointer` walks forward WITHOUT LIMIT — writing guest
// memory past the buffer the driver handed over, for as long as bytes keep
// arriving. Observed as `RefCell already borrowed` raised from inside
// `embassy_sync::watch`, i.e. a corrupted borrow flag several kilobytes away
// from any UART buffer. Silicon cannot do this: the receiver stops at MAXCNT
// and loses bytes until STARTRX. So `rxStarted` is cleared where ENDRX is
// raised, which routes those bytes into the model's own existing
// receiver-disarmed path (dropped, with a warning) instead of out of bounds.
// Nothing else in the file differs from v1.16.1 — the register map, the EasyDMA
// counters, the baud table and the IRQ wiring are verbatim, so with no PPI
// channel pointing at a UARTE event and no overrun, this model behaves as the
// stock one.
//
// Upstream: renode-infrastructure
// src/Emulator/Peripherals/Peripherals/UART/NRF52840_UART.cs
// (Copyright (c) 2010-2026 Antmicro, MIT).
//
//
...
{
    public class NRF52840_UARTE_Events : UARTBase, IDoubleWordPeripheral, IKnownSize, INRFEventProvider
    {
        public NRF52840_UARTE_Events(IMachine machine, bool easyDMA = false) : base(machine)
        {
        ...
        // EVENT PROVIDER: the whole reason this model exists.
        ...
        protected override void CharWritten()
        {
            ...
            lock(interruptManager)
            {
                ...
                if(easyDMA)
                {
                    ...
                    {
                        // BOUNDED RX DMA: the transfer is over, so the receiver
                        // is disarmed until something starts the next one. Set
                        // BEFORE the event is raised, so a PPI ENDRX -> STARTRX
                        // chain re-arms on top of it rather than under it.
                        rxStarted = false;
