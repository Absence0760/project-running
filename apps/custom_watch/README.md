...
7. **Integration** — wire steps 3–6 into a single recording state machine that ports the existing Dart `run_recorder` algorithm to async Rust. **RECORDER WIRED, SIM-VERIFIED (2026-07-08); button control + BLE sync pending.** `watch_core::record` holds the host-tested `Recorder` (Idle → Recording → Paused → Finished) — haversine distance, elapsed/moving time, pace, `run_recorder`'s point-acceptance filter (min-move 3 m, max-jump 100 m, max-speed 10 m/s) and its derived moving-time auto-pause (0.5 m/s gate); 7 host tests. The `record` task drives it from `state::FIX` and publishes a snapshot to `state::RECORD`, which the face renders as a recording line; the sim run shows distance accruing and auto-pause holding moving time. Two seams remain: the run **auto-starts on the first fix** as a tier-1 stand-in for a real button (needs a button-handler task feeding start/pause/stop), and the finished run isn't yet handed to the BLE sync path (step 6). The barometer is wired too — the `baro` task drives the **BMP581** (`drivers/bmp581/`, 5 host tests) over TWISPI1 through the same async probe guard, feeding `watch_core::elevation` (barometric altitude + cumulative-vert accumulator) and logging the totals; surfacing altitude/vert on the face/phone is the remaining step.
