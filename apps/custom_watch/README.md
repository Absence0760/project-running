...
5. **Optical HR bring-up** — **DRIVER + PEAK-DETECT CODE DONE, HOST-TESTED (2026-07-08); task wiring + bench verification pending parts.** `max86177` register driver (soft-reset → single-LED PPG channel → FIFO drain over blocking I²C) + a float-free integer PPG→BPM peak detector (DC-baseline removal, envelope-threshold beat detection with a refractory gate, inter-beat-interval smoothing, validity flag; 10 host tests). The `hr` task still needs the TWIM port wired in `main.rs` + a `HeartRate` publisher; the licensed Maxim HR algorithm comes later via `bindgen` against Maxim's C library, post-tier-1.
...
7. **Integration** — wire steps 3–6 into a single recording state machine that ports the existing Dart `run_recorder` algorithm to async Rust. **STATE-MACHINE CORE CODE DONE, HOST-TESTED (2026-07-08); task wiring pending.** The pure-logic half lives in `watch_core::record`: a `Recorder` (Idle → Recording → Paused → Finished) consuming control commands + `Fix` samples, accumulating haversine distance, elapsed/moving time and pace, with `run_recorder`'s point-acceptance filter (min-move 3 m, max-jump 100 m, max-speed 10 m/s) and its derived moving-time auto-pause (0.5 m/s gate) carried over; 7 host tests. The barometric-vert foundation (`watch_core::elevation`) is done alongside. The `record` task still needs to be driven from the `state::FIX` watch + a button/command source, and the completed run handed to the BLE sync path.
