...
python3 …/ci_smoke.py --scenario settings --out-dir "$RUNNER_TEMP/watch-sim-ci/settings" --budget 300
```
...
| `settings` | That the **idle** settings menu (§ 351) works as a whole: BTN5 opens it and it reaches the panel, BTN3 walks all five rows in their documented order and wraps, BTN2 walks back the other way, an edit on the GNSS row moves the ladder **and stays ordered** (parked at its left end, then walked back up — projected hours may not fall as you edit right), and BTN4 closes it. The only scenario that boots `--no-autostart`, so the only one that reaches an idle face at all. | That an edit **survives a reboot** — `CFG1` persistence is a flash claim and this never power-cycles the emulator; the menu's writes are best-effort / L4 by design. Nor legibility of five rows on a 1-bit panel, which is bench-only. |
