...
1. **Click it in the `--gui` window.** The watch-screen window draws BTN1–BTN4 in bezel strips flanking the LCD, each at the physical position its function stands in for on the §81 Garmin-Fenix layout — BTN1 upper-right (START/STOP), BTN4 lower-right (BACK/LAP), BTN2 mid-left (UP slot), BTN3 lower-left (DOWN slot); the left-top LIGHT slot is empty at tier 1. The display model implements Renode's `IAbsolutePositionPointerInput`, so the analyzer forwards mouse presses on those boxes to the same gpio0 pins the macros drive (mouse down = press, mouse up = release; the box inverts while held).
