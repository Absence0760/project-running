...
|---|---|---|
| `smoke` | The recording pipeline end to end: NMEA → parser → fix accumulator → recorder → distance, one run face on glass, and the two-press stop committing a CRC'd blob to a flash slot. Since § 362 it also asserts the optical-HR rail (a BPM off the MAX86177 model) and the phone link (v1 NDJSON frames off the TCP socket, schema checked key by key). | Nothing about the *other* built-in pages — the ring is 40 built-ins plus up to four composed — and nothing about an alert firing. |
