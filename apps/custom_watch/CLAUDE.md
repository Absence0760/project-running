...
| **Buttons / navigation / modals — read before touching the press grammar.** Every state, page, and button edge; §350 spatial paging (BTN3/BTN4 tap left/right, BTN5 lap — doubling as workout-step skip when a workout is armed, §354, and its HOLD marking a waypoint since §357; one-tier 0.5 s hold); §351 idle settings menu (BTN2/BTN3 cursor up/down, BTN5/BTN1 directional edits, BTN4 exit, six rows since §372's BACKYARD, whose far seat costs 5 while every mid-race row stays ≤ 4); §375 idle timer modal on BTN2 (countdown + stopwatch, one clamped ladder whose zero rung IS the stopwatch — and no alarm, because nothing on this device can wake anyone); the three idle faces BTN4 walks (home / diagnostics / ICE, §291 + §358); the computed press-cost model + the "every interaction, priced" floor table; the 40-page cycle since §372's `YARD` | [../../docs/custom_watch/navigation.md](../../docs/custom_watch/navigation.md) |
