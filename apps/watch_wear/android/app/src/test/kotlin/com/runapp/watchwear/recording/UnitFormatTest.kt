...
class UnitFormatTest {
    ...
    @Test
    fun `formatDistance in km uses a period in en-US`() {
        assertEquals("5.12", formatDistance(5_123.0, DistanceUnit.KM, 2, Locale.US))
        assertEquals("0.00", formatDistance(0.0, DistanceUnit.KM, 2, Locale.US))
        assertEquals("10.50", formatDistance(10_500.0, DistanceUnit.KM, 2, Locale.US))
    }

    @Test
    fun `formatDistance in km uses a comma in de-DE`() {
        assertEquals("5,12", formatDistance(5_123.0, DistanceUnit.KM, 2, Locale.GERMANY))
        assertEquals("0,00", formatDistance(0.0, DistanceUnit.KM, 2, Locale.GERMANY))
    }

    @Test
    fun `formatDistance in km always emits exactly two fraction digits`() {
        // 5000 m → "5.00", not "5". The two-decimal read keeps the metric
        // line a stable width on the watch face.
        assertEquals("5.00", formatDistance(5_000.0, DistanceUnit.KM, 2, Locale.US))
        assertEquals("5,00", formatDistance(5_000.0, DistanceUnit.KM, 2, Locale.GERMANY))
    }

    @Test
    fun `formatDistance in km never groups thousands`() {
        ...
        assertEquals("12.35", formatDistance(12_345.0, DistanceUnit.KM, 2, Locale.US))
    }

    @Test
    fun `formatDistance in km honours an explicit decimal count`() {
        // The tile drops to one decimal past 10 km.
        assertEquals("21.1", formatDistance(21_100.0, DistanceUnit.KM, 1, Locale.US))
        assertEquals("21,1", formatDistance(21_100.0, DistanceUnit.KM, 1, Locale.GERMANY))
        assertEquals("5.123", formatDistance(5_123.0, DistanceUnit.KM, 3, Locale.US))
    }
    ...
    fun `default locale path does not crash and round-trips a known value`() {
        ...
        try {
            ...
            assertEquals("5.12", formatDistance(5_123.0, DistanceUnit.KM, 2))
            assertEquals("21.1", formatDistance(21_100.0, DistanceUnit.KM, 1))
        } finally {
    ...
    @Test
    fun `formatDistance in km at the default decimal count`() {
