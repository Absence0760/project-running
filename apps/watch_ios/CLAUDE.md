...
- [x] Heart rate via HealthKit sensor — a failed workout session degrades honestly rather than freezing on the last reading: "Heart rate unavailable" during the run, "Heart rate unavailable — run saved without it" on the summary, and no `avg_bpm` on the synced row
...
Still uncovered (Android-bound, not unit-testable without a simulator/host harness): `WorkoutManager`'s `start()` (it spins timers + `CLLocationManager` + `HKWorkoutSession`; `pause()` / `resume()` ARE driven, see `WorkoutManagerPaceTests`), `HealthKitManager` HR averaging (driven by `HKLiveWorkoutBuilder` delegate callbacks — the failure path is covered via `handleSessionFailure()`, which is all `didFailWithError` does), `WatchConnectivityManager`'s real `WCSession.transferFile`, and `SupabaseService.swift`'s network path. Follow Wear's extract-then-test pattern if those grow non-trivial branches.
