...
- `geocoding_test.dart` — 7 tests for `lib/geocoding.dart` (MapTiler geocoding): empty list on <2-char query / empty apiKey, parses MapTiler features into PlaceResult, URL-encodes the query + includes limit/key params, empty list on fetcher error, skips malformed center coords, falls back to `text` field when `place_name` absent.
...
- `revenuecat_test.dart` — 7 tests for `lib/revenuecat.dart` (mobile Pro purchase flow wrapper): `isRevenueCatConfigured` returns false when no platform key is in `dotenv.env`, true when `keyOverride` non-empty, false when `keyOverride` empty; `configureRevenueCat` returns false when no API key is provided; `keyOverride` is honoured (configure attempts but headless test surfaces MissingPluginException internally → wrapper catches and returns false — no crashes on host-test runner); `startProCheckout` returns `PurchaseResult.notConfigured` when SDK has no API key; `managementUrl` returns null when SDK has no API key. The wrapper is the integration glue used by `settings_screen`'s "Subscribe to Pro" tile.
