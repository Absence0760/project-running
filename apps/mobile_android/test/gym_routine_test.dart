...
void main() {
  ...
  test('routineFromWorkout: two spellings of one lift merge into one block', () {
    ...
    // refuses (decisions section 1286 keeps the column non-unique on purpose).
    final draft = routineFromWorkout(null, [
      lset('Bench Press', 5, 80),
    ...
    expect(draft.exercises[0].exerciseName, 'Bench Press');
    expect(draft.exercises[0].exerciseKey, 'bench press');
    expect(draft.exercises[0].sets.length, 3);
    expect(draft.exercises[0].sets.map((x) => x.setIndex).toList(), [0, 1, 2]);
    expect(draft.exercises[1].exerciseKey, 'incline press');
    expect(draft.exercises[1].sets.length, 2);
  });
  ...
  test('routineFromWorkout: an interleaved lift still splits the blocks', () {
    ...
    expect(draft.exercises.length, 3);
    expect(
      draft.exercises.map((e) => e.exerciseKey).toList(),
  ...
  test('routineFromWorkout: a name whose canonical key is empty is dropped', () {
    ...
    // whitespace class folds it away, so the web rail's name-blank guard let it
    // through and minted exerciseKey '' -- which the column's own
    // length(exercise_key) between 1 and 120 CHECK refuses. Dart's trim()
    // strips U+0085, so the two rails answered differently until both keyed
    // the guard (decisions section 1322).
    final draft = routineFromWorkout(null, [
      lset('\u0085', 5, 50),
      lset('\u0085\u0085', 5, 50),
      lset('Row', 8, 60),
