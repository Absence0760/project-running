...
class _RunDetailScreenState extends State<RunDetailScreen>
  ...
  /// Render the planned-vs-actual table when the recorder linked this
  /// run to a planned workout. Reads `runs.metadata.workout_step_results`
  /// + `workout_adherence` directly so the surface stays in sync with
  /// the writer in `run_screen._stop` without a second query. Web has
  /// the equivalent on `/runs/[id]`.
  List<Widget> _buildWorkoutReview(ThemeData theme) {
    final raw = run.metadata?['workout_step_results'];
    if (raw is! List || raw.isEmpty) return const [];
    final adherence = run.metadata?['workout_adherence'] as String?;

    final steps = <_WorkoutStepReview>[];
    for (final entry in raw) {
      if (entry is! Map) continue;
      steps.add(_WorkoutStepReview.fromMap(entry));
    }
      ...
      Padding(
        ...
        child: Row(
          children: [
            ...
            const Spacer(),
            if (adherence != null) _AdherencePill(adherence: adherence),
          ],
        ),
      ),
      Padding(
        ...
        child: Container(
          ...
          child: Column(
            children: [
              _workoutHeaderRow(theme),
              for (var i = 0; i < steps.length; i++)
                _workoutStepRow(theme, steps[i],
  ...
  Widget _workoutHeaderRow(ThemeData theme) {
    final outline = theme.colorScheme.outline;
    Widget cell(String label, {int flex = 1, TextAlign align = TextAlign.start}) {
      return Expanded(
        flex: flex,
        child: Text(
          label.toUpperCase(),
  ...
  Widget _workoutStepRow(
    ThemeData theme,
    _WorkoutStepReview s, {
    required bool last,
  }) {
      ...
      child: Row(
        children: [
          ...
          cell('${(s.actualDistanceM / 1000).toStringAsFixed(2)} km',
              flex: 2, align: TextAlign.right),
          cell(_fmtPace(s.actualPaceSecPerKm),
              flex: 2, align: TextAlign.right),
          Expanded(
            flex: 2,
            child: Text(
              skipped ? 'skip' : paceDelta.label,
              textAlign: TextAlign.right,
              style: theme.textTheme.bodySmall?.copyWith(
                fontWeight: FontWeight.w700,
                color: paceDelta.color(theme),
              ),
            ),
          ),
        ],
  ...
  _PaceDelta _paceDelta(_WorkoutStepReview s) {
    if (s.actualPaceSecPerKm == null) return const _PaceDelta('—', _PaceTone.neutral);
    final diff = s.actualPaceSecPerKm! - s.targetPaceSecPerKm;
    if (diff.abs() < 1) return const _PaceDelta('on', _PaceTone.on);
    final sign = diff > 0 ? '+' : '−';
    final tol = 10; // matches recorder default
    final mag = diff.abs();
    final tone = mag <= tol
        ? _PaceTone.on
        : (mag <= tol * 2 ? _PaceTone.amber : _PaceTone.off);
    return _PaceDelta('$sign${mag}s', tone);
  }
...
// ──────────────────── Workout review row types ────────────────────

class _WorkoutStepReview {
  final String label;
  final String kind;
  ...
  factory _WorkoutStepReview.fromMap(Map raw) {
    ...
    return _WorkoutStepReview(
      ...
      actualPaceSecPerKm:
          (raw['actual_pace_sec_per_km'] as num?)?.toInt(),
      status: raw['status']?.toString() ?? 'completed',
    );
  }
}

enum _PaceTone { neutral, on, amber, off }

class _PaceDelta {
  final String label;
  final _PaceTone tone;
  const _PaceDelta(this.label, this.tone);

  Color color(ThemeData theme) {
    switch (tone) {
      case _PaceTone.on:
        return Colors.green;
      case _PaceTone.amber:
        return Colors.amber;
      case _PaceTone.off:
        return theme.colorScheme.error;
      case _PaceTone.neutral:
...
class _AdherencePill extends StatelessWidget {
  ...
  const _AdherencePill({required this.adherence});
