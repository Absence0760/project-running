...
/// session roll-up. A planned set is `hit` when the actual reps reach the rep
/// floor AND (no weight target, or the actual weight reaches it); a duration
/// target is hit when the actual duration reaches it. A planned set with reps
/// below the floor (but logged) is `partial`; a planned set whose weight fell
/// short, or that was never logged, is `missed`. A logged set with no matching
/// plan entry is `extra` and is excluded from `plannedCount`.
  ...
  for (final p in planned) {
    ...
    if (a == null) {
      status = SetAdherenceStatus.missed;
        ...
        (a.weightKg == null || a.weightKg! < p.targetWeightKg!)) {
      status = SetAdherenceStatus.missed;
    } else if (p.targetRepsMin != null) {
      if (a.reps != null && a.reps! >= p.targetRepsMin!) {
        status = SetAdherenceStatus.hit;
      } else {
        status = SetAdherenceStatus.partial;
      }
    } else if (p.targetDurationS != null) {
      status = a.durationS != null && a.durationS! >= p.targetDurationS!
