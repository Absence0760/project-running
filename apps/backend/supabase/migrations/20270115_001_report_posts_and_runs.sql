-- E2 — report buttons on more surfaces. Extend the polymorphic
-- `reports` target set so a user can flag an individual club post or a
-- run, not just a profile / club / route / comment.
--
-- Prior state:
...
--
-- This adds 'club_post' (id in club_posts) and 'run' (id in runs).
-- Additive: widen the CHECK + the submit_report validation, and add a
-- target-exists branch per new kind that also rejects reporting your
-- own post/run (a misclick) — mirroring the existing self-user and
-- self-comment guards. Bare-body create-or-replace, so the function is
-- the COMPLETE 20261117_001 body plus the two new arms.

alter table reports drop constraint reports_target_kind_check;
alter table reports
  add constraint reports_target_kind_check
  check (target_kind in ('user', 'club', 'route', 'comment', 'club_post', 'run'));
...
begin
  ...
  if p_target_kind not in ('user', 'club', 'route', 'comment', 'club_post', 'run') then
