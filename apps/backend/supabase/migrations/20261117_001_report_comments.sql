-- Allow reporting individual comments (persona round-5 runner-woman, High).
-- The reports table + submit_report() accepted target_kind in
-- ('user','club','route') only, so a victim of an abusive run comment had no
-- escalation path — they could report the author's whole profile but not the
-- specific comment. Add 'comment' as a fourth polymorphic target whose id
-- lives in run_comments.
--
-- Additive: widen the CHECK + the submit_report validation, and add the
-- target-exists branch for comments. Bare-body create-or-replace, so the
-- function is the COMPLETE 20260908_001 body plus the comment arm.

alter table reports drop constraint reports_target_kind_check;
alter table reports
  add constraint reports_target_kind_check
  check (target_kind in ('user', 'club', 'route', 'comment'));
...
begin
  ...
  if p_target_kind not in ('user', 'club', 'route', 'comment') then
