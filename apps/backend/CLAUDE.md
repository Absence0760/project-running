...
| `strava-webhook` | **Working** — verification + per-activity ingest via shared logic in `_shared/strava.ts`. Replay-protected (7-day freshness on `event_time` + dedupe via `webhook_events` keyed on `owner:object:aspect:event_time`). | GET verification from Strava + POST activity events | Shared `?secret=` in the callback URL guards both methods (Strava doesn't sign POSTs); GET also checks `hub.verify_token`. Service role for DB writes. `verify_jwt = false` in `config.toml`. | `STRAVA_VERIFY_TOKEN`, `STRAVA_WEBHOOK_SECRET`, `SUPABASE_SERVICE_ROLE_KEY` |
