...
- **An UNCONDITIONAL assignment** — at the top level of the trigger body, so it always fires — makes the supplied value unreachable, always. Such a `lives_ok` fails the guard outright. Stop supplying the column, or register it in `STAMPED_VALUE_ASSERTIONS` with the reason its claim is unaffected.
