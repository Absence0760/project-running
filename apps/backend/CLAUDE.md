...
- `RUNSIGNUP_API_KEY`, `RUNSIGNUP_API_SECRET` — RunSignUp REST credentials for the `race-results-import` / `race-listings-sync` race-results legs. Both required: unset → `503 provider_not_configured` (fail-closed; parkrun + manual paste still work).
...
- `RACE_IMPORT_USER_AGENT` — polite identifier for the RunSignUp / ChronoTrack results fetch (defaults to `RunApp/1.0`).
