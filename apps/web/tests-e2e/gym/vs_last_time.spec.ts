...
test.describe('/gym/[id] — vs last time hint', () => {
	...
	test('previous session + positive delta + link to progression', async ({ page }) => {
		...
		try {
			// Open the heavier (later) workout's detail.
			await page.goto(`/gym/${workoutIds[`${exercise} 110`]}`);
			const hint = page.locator('.last-time');
			await expect(hint).toBeVisible({ timeout: 10_000 });
			// Shows the previous session's top set and a +10 kg gain.
