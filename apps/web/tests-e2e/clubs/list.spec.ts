...
test.describe('/clubs', () => {
	...
	test('Browse search narrows public clubs to a name match', async ({
		...
		// "UVA Tempo Tuesday" visible from the seeded public clubs.
		...
		// Tempo Tuesday remains; Richmond Run Club gone (no match for Tempo).
		await expect(
			page.getByRole('heading', { name: 'UVA Tempo Tuesday' })
	...
	test('Browse tab shows public clubs + hides private "Friends of Jared"', async ({
		...
		// Seeded clubs: Richmond Run Club (public open), Tempo Tuesday
	...
	test('seeded Richmond Run Club renders + drill into slug detail', async ({
		page
	}) => {
		...
		await page.getByRole('link', { name: /Richmond Run Club/ }).click();
		await expect(page).toHaveURL(/\/clubs\/richmond-run-club/);
	});
	...
	test('Browse search clearing the box restores the full club list', async ({
		...
		await page.getByPlaceholder(/Search by name/).fill('Tempo');
		// Richmond Run Club is hidden under the Tempo filter.
		await expect(
			page.getByRole('heading', { name: 'Richmond Run Club' })
		).toHaveCount(0);

		// Clear the search; Richmond Run Club returns.
		await page.getByPlaceholder(/Search by name/).fill('');
		await expect(
			page.getByRole('heading', { name: 'Richmond Run Club' })
		).toBeVisible({ timeout: 10_000 });
