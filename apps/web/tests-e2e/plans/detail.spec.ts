 ...
 *
 * The seed provisions a single active training_plan ("Richmond Half
...
test.describe('/plans/[id]', () => {
	...
	test('Edit-plan meta dialog: rename → Save persists across reload, restore', async ({
		...
		const restored = 'Richmond Half 2026';
	...
	test('Richmond Half plan renders multi-week grid + clicking a day opens the workout editor', async ({
		page
	}) => {
	...
	test('publish-as-template: pick a club → Publish → club gets the new template', async ({
		...
		// The cloned template now lives under Richmond Run Club's
		...
		await expect(
			page.locator('.template-row', { hasText: 'Richmond Half 2026' }).first()
		).toBeVisible({ timeout: 10_000 });
	...
	test('day-modal mounts WorkoutEditor with the Mark/Save/Cancel actions', async ({
		...
		// adjacent "Richmond Half plan renders multi-week grid" test.
