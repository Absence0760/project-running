 ...
 * delivered through behaviour changes inside the surface (unlimited
 * messages, wider context budget). See [docs/paywall.md] for the
...
test.describe('paywall — per-feature gates', () => {
	...
	test.describe('Free user reaches /coach directly (no Pro screen-gate)', () => {
		test('free user lands on /coach -> sees the chat surface, not a <ProGate>', async ({
			...
			// chat UI plus a 5-msg/day cap. This test pins the surface
