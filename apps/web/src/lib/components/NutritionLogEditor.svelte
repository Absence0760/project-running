<script lang="ts">
	...
	interface Props {
		/// Fired after a food entry is logged. The host closes the modal +
		/// refreshes (or, on the standalone route wrapper, navigates back).
		oncreated: () => void;
		oncancel: () => void;
	}
	...
	// A picked result switches the editor to an inline portion-confirm view
	// (rather than a nested modal — the editor is itself modal-hosted).
...
<div class="nutrition-editor">
	...
	{#if picked}
		<div class="portion" data-testid="portion-dialog">
			<button type="button" class="back-step" onclick={() => (picked = null)}>
			...
			{/if}
			<div class="actions">
				<button class="btn btn-secondary" type="button" onclick={() => (picked = null)}>{m('nutrition.cancel')}</button>
	...
	{:else}
		...
		<div class="actions">
			<button class="btn btn-secondary" type="button" onclick={oncancel} disabled={saving}>
				{m('nutrition.cancel')}
			</button>
		</div>
	{/if}
...
<style>
	...
	.toolbar-select {
		padding: var(--space-sm) calc(var(--space-md) + var(--space-lg)) var(--space-sm) var(--space-md);
		border: 1px solid var(--color-border);
		border-radius: var(--radius-md);
		background: var(--color-surface);
		color: var(--color-text);
	...
	.back-step:hover { color: var(--color-primary); }
	.back-step .material-symbols { font-size: 1.05rem; }
	...
	.actions { display: flex; justify-content: flex-end; gap: var(--space-sm); padding-top: var(--space-xs); }
</style>
