...
test('pickAllowedRunMetadata — null input returns null', () => {
	assert.equal(pickAllowedRunMetadata(null), null);
});

test('pickAllowedRunMetadata — empty input returns null', () => {
	assert.equal(pickAllowedRunMetadata({}), null);
});

test('pickAllowedRunMetadata — keeps coaching-value keys', () => {
	const meta = {
		activity_type: 'run',
		avg_bpm: 152,
		workout_kind: 'tempo',
	...
	const out = pickAllowedRunMetadata(meta);
...
test('pickAllowedRunMetadata — workout_step_results passes through (planned vs actual)', () => {
	...
	assert.deepEqual(pickAllowedRunMetadata(meta), meta);
});
