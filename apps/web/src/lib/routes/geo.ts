 ...
 *
 * Every local planar frame in this folder — the along-route projection's
 ...
 *
 * TS twin of `apps/mobile_android/lib/geo.dart`, and a port of the firmware's
 ...
 */
export function lonDeltaDeg(fromDeg: number, toDeg: number): number {
	return foldTurns(toDeg - fromDeg);
}
 ...
 * for bit — `floor` is exactly zero there, and a round-trip through a
 * subtraction and an addition is not.
 */
export function unwrapLonDeg(referenceDeg: number, lonDeg: number): number {
	return lonDeg - 360 * Math.floor((lonDeg - referenceDeg + 180) / 360);
}

/** A longitude folded back onto the [-180, 180) meridian range. */
export function wrapLonDeg(lonDeg: number): number {
	return foldTurns(lonDeg);
}
 ...
 */
function foldTurns(deg: number): number {
