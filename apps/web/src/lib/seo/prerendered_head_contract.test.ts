...
test('the built landing page carries every head signal the render map promises', (t) => {
	...
	const titles = [...h.matchAll(/<title[^>]*>([\s\S]*?)<\/title\s*>/gi)].map((m) => m[1].trim());
	assert.equal(titles.length, 1, `expected exactly one title on /, found ${titles.length}`);
	...
	for (const property of ['og:title', 'og:description', 'og:url', 'og:image', 'og:type']) {
		assert.match(h, new RegExp(`property="${property}"`), `/ carries no ${property}`);
	}
...
test('every prerendered page carries a canonical naming its own served path', (t) => {
	...
	for (const file of pages) {
		...
		if (url.pathname !== servedPath(file)) {
			wrong.push(`${file}: canonical ${url.pathname}, served at ${servedPath(file)}`);
		}
	}
	assert.deepEqual(
		wrong,
...
test('every prerendered page carries a title, a description and social meta', (t) => {
	...
	for (const file of builtPages().filter((f) => f !== SHELL)) {
		...
		const description = /<meta name="description" content="([^"]*)"/.exec(h);
		if (!description || description[1].trim() === '') missing.push(`${file}: description`);
