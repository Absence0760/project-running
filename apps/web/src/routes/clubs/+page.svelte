<script lang="ts">
	...
	// Per-loader generation counters. Browse + Mine both fire from
	// onMount and from user actions; a shared gen would have one
	// invalidate the other. Snapshot.restore bumps BOTH so any in-flight
	// load() kicked off on mount discards its result instead of
	// clobbering the captured lists on back-nav from /clubs/[slug].
	...
	function setTab(next: 'browse' | 'mine') {
		tab = next;
		const path = next === 'mine' ? '/clubs' : `/clubs?tab=${next}`;
		goto(path, { replaceState: true, noScroll: true, keepFocus: true });
	}

	onMount(async () => {
		const initial = $page.url.searchParams.get('tab');
		if (initial === 'browse') tab = 'browse';

		// fetchMyClubs() returns [] silently when auth.user is null, so a
		// hard reload during the auth race would render the empty
		// "You haven't joined a club yet" state forever. Poll briefly
		// before kicking off the fetches. Same shape as /settings/account.
		for (let i = 0; i < 20 && (auth.loading || !auth.user); i++) {
			await new Promise((r) => setTimeout(r, 50));
		}
		// Snapshot.restore (below) populates lists synchronously and flips
	...
	export const snapshot: Snapshot<{
		...
		restore: (s) => {
			...
			// rendered, so the page is too short and scroll falls back to 0.
			// Re-apply the captured scrollY after the DOM updates.
...
<div class="page">
	...
	<!-- Single-rail toolbar — search (Browse only) + Create club. Mirrors
	     the .filter-row + space-between pattern used by /routes + /runs. On
	     My clubs the search collapses so the Create button anchors right
	     by itself. -->
		...
		<div class="empty-card">
			{#if tab === 'mine'}
				...
				<p class="empty-text">
					...
					what's public near you, request to join with one tap, or start your
					own.
				</p>
				<div class="empty-actions">
					<button class="btn btn-primary" type="button" onclick={() => setTab('browse')}>
						<span class="material-symbols" aria-hidden="true">search</span>
						Find a club
					</button>
			...
			{:else}
				...
				<p class="empty-text">
					There aren't any public clubs to browse right now. Start one to put
					your area on the map — others can find and join.
				</p>
				<div class="empty-actions">
...
<style>
	...
	/* Single-rail toolbar — search grows to fill, Create anchors right.
	   Mirrors /routes .filter-row pattern. */
	.filter-row {
		display: flex;
		align-items: center;
	...
	/* Request-pending chip — yellow accent so the user-state distinction
	   from "joined" (purple primary-light chip) is obvious at a glance. */
	.chip-pending {
	...
	/* Empty-state card — same shape as /routes, /runs, /plans. Card with
	   icon (or brand mark), h3, explainer, primary CTA + secondary
	   actions. */
	.empty-card {
		display: flex;
		flex-direction: column;
	...
	/* Skeleton — same shimmer language as /routes + /runs. Mirrors the
	   real card layout so the page lands at its true height immediately
	   and the data swap doesn't shift the grid. */
	.skel {
		display: block;
		background: var(--color-bg-tertiary);
	...
	/* <=50rem (small tablet / large phone). Collapse the Create button to
	   its own row so the search has the full rail. */
	...
	/* <=30rem (phone). Create stretches to the full rail. */
