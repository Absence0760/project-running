...
src/
  ...
  routes/
    ...
    plans/new/      # Unified CREATE HUB for all three reusable-plan engines (decisions §146): a Training / Session / Gym-routine chooser swaps in PlanEditor / SessionPlanEditor / RoutineEditor. ?type= picks the initial branch, ?club= targets a club (one-step club-owned create only on the session branch; training+gym stay build-then-publish). Training branch = the editable week-by-week plan wizard (starter picker + club-template adopt + PlanEditor).
