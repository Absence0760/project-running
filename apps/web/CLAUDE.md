...
src/
  ...
  routes/
    ...
    history/        # Run history with source + activity type filters. The canonical History surface (renamed from /runs — F14 cosmetic half, decisions §63 / D3). Multi-modal (data-gated, no flag — §63 amendment): All/Runs/Lifts/Meals kind chips + a unified timeline over the `activities` view (fetchActivities) appear once a second modality exists; the Runs chip keeps the full run UI. Pure runner unchanged. nav glyph is `timeline` (modality-agnostic), not `directions_run`.
