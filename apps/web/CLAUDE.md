...
src/
  ...
  routes/
    +layout.svelte  # App shell with collapsible sidebar (state persisted in localStorage as `sidebar_collapsed`). Sidebar shape: Dashboard · History · Runs · Gym · Nutrition · Coach · Social. Routes is NOT a standalone sidebar item — it lives under the run surface as a sub-tab (Runs · Routes via RunSurfaceTabs.svelte); the /routes URL still resolves.
    ...
    routes/         # Tabbed: My routes (saved) + Explore routes (community discovery via RouteExplorer). ?tab=explore deep-links the second tab. Routes now lives under the run surface — the page renders the shared RunSurfaceTabs (Runs · Routes) strip above its inner My-routes/Explore/Heatmap tablist, and is reached from the /runs Routes sub-tab. The /routes URL keeps resolving for bookmarks, club deep links, and shares.
