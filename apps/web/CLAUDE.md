...
src/
  ...
  routes/
    ...
    learn/          # Public, no-auth, PRERENDERED Learn/guides surface (decisions §161, features/learn.md). /learn hub + /learn/[slug] articles (mdsvex .md bodies) + /learn/category/[category]. All prerender=true (+ entries()); SEO mirrors share/route (canonical / OG / Article+BreadcrumbList JSON-LD via lib/learn/learn_meta.ts). Shell-less + anon in +layout.svelte (path.startsWith('/learn')). Shares the landing page's chrome via PublicHeader (solid variant; the landing uses `overlay`) + PublicFooter — don't hand-roll a header here (issue #212). Content authored in lib/learn/guides/*.md, indexed by import.meta.glob in lib/learn/guides.ts. Web-only (no twin).
