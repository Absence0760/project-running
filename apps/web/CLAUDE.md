...
src/
  lib/
    components/     # Modal + SplitPane (primitives), Avatar (shared img-or-initial avatar; per-site size/font/bg props), RunMap, ElevationProfile (Android-style interactive chart), ImportRoute, RouteBuilder, CoachChat,
                    ...
                    # (modal-hosted creation forms), GymEditor (Phase 4 gym composer — modal-hosted, free-text exercise + inline sets, used by /gym + /gym/[id]), NutritionLogEditor (Phase 4 nutrition capture — modal-hosted on /nutrition, food search → inline confirm-portion → log + manual fallback; /nutrition/log is a thin wrapper), PrivacyZonePicker (MapLibre map picker for owner zones, decisions §33),
    ...
    # training/ routes/ segments/ social/ integrations/ backup/ share/ settings/ runs/ format/ util/ billing/ gym/ nutrition/.
  ...
  routes/
    ...
    dashboard/      # Weekly mileage, PBs, calendar heatmap. "This Week" stat card opens PeriodSummary in a modal. Multi-modal (data-gated, no flag — §63 amendment): self-hiding Today's-lift + Recent-lifts cards, and gym sessions folded into the fitness/fatigue/form curve via gym/lift_load.ts (decisions §63, multi_modal.md).
    ...
    nutrition/      # Phase 4 multi-modal Nutrition (decisions §63; sidebar item always present since the §63 amendment). /nutrition = Mifflin-St Jeor macro rings vs targets (hidden when body metrics absent), meal-slot daily view, water tracker (localStorage per day), 7-day calorie trend, with a capture modal (NutritionLogEditor in the shared Modal, opened from /nutrition per the §210 "Log → a sheet" IA contract — mirrors the gym GymEditor pattern): Open Food Facts search → inline confirm-portion → log, manual macro entry fallback. /nutrition/log stays a thin deep-link wrapper around the same editor; /nutrition/log/[id] is the meal-detail route. Data + body-metrics queries in core/data.ts (food_log + body_metrics). Pure libs in lib/nutrition/: nutrition_targets.ts (BMR, parity pair), food_search.ts (Open Food Facts, injectable fetcher), nutrition_totals.ts (day aggregation). Body metrics (height on user_profiles + body_metrics weight series, migration 20261216_001) + activity/goal prefs entered in Settings → Preferences under the Art 9 health-consent gate. e2e tests-e2e/nutrition/. Mobile twin owns the Dart side.
