...
| Generalising events to types (yoga / pilates / class) or adding paid registration | [docs/features/club_events.md](docs/features/club_events.md) — **proposed, not built**: typed events (`run`/`ride`/`class`/`social`, self-hiding athletic fields) + an orthogonal Stripe Connect marketplace whose payouts go to the host, not the club. Slice E (typing) ships first as the gate probe; paid (P) is gated behind it |
