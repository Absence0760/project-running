...
| Building the gym programming engine (routines, supersets, planned-vs-actual, progression) | [docs/features/gym_programming.md](docs/features/gym_programming.md) — specced four-slice depth tier (P1 routines → P4 progression); relocates the engagement gate into P1, mirrors the run runner. Not yet built; P1 is the only approved slice |
| Building a yoga / pilates session planner (timed pose/movement sequences + follow-along runner) | [docs/features/session_planner.md](docs/features/session_planner.md) — **specced, not built**: a `session_plans` engine (blocks → timed/per-side/cue items) mirroring the gym routine engine's shape; attaches to a `class` event, follow-along runner logs a `gym_workout` (cross-modal). Self-contained for a fresh session to implement; P1 (build/save, no execution) is the only slice to start, no compliance gate |
| Getting a gym / yoga instructor running their **class business** end-to-end (schedule → get paid → author content → teach → log → reconcile) | [docs/features/instructor_business.md](docs/features/instructor_business.md) — **plan, not built**: sequences `club_events.md` + `gym_programming.md` + `session_planner.md` into one ordered path (M0–M10) across three tracks (make-money-real / make-class-teachable / run-the-day) for the `gym-class-instructor` + `yoga-pilates-instructor` personas. Money milestones carry the owner+CISO+counsel compliance gate; content milestones don't. Includes the verified shipped-vs-gated-vs-missing state matrix |
